...
describe('luis:version:list', () => {
  ...
  test
    .nock('https://westus.api.cognitive.microsoft.com', (api) =>
      api.get((uri) => uri.includes('apps')).reply(200, { name: 'version' })
    )
    .stdout()
    .command([
      'luis:version:list',
  ...
  test
    ...
    .it(
      'export a list of application versions to the specified file',
      (ctx) => {
  ...
  test
    ...
    .it(
      'export a list of application versions to the specified directory, using a default file name',
      (ctx) => {
        expect(ctx.stdout).to.contain('List successfully written to file');
  ...
  test
    ...
    .it(
      'displays a list of application versions and a success message in the console (since the target path provided is invalid)',
      (ctx) => {
