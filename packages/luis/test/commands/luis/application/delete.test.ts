...
describe('luis:application:delete', () => {
  ...
  test
    ...
    .it(
      ...
      (ctx) => {
        expect(ctx.stderr).to.contain(
          `Required input property 'endpoint' missing.`
        );
      }
    );
  ...
  test
    .nock('https://westus.api.cognitive.microsoft.com', (api) =>
      api.delete((uri) => uri.includes('apps')).reply(200, { code: 'Success' })
    )
    .stdout()
    .command([
      'luis:application:delete',
  ...
  test
    ...
    .it(
      'fails to delete an app and displays an error message if the endpoint is undefined',
      (ctx) => {
