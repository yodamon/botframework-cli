...
describe('luis:application:create', () => {
  beforeEach(() => {
    ...
    sinon
      .stub(LuisApplicationCreate.prototype, 'saveImportedConfig')
      .returns(true);
  });
    ...
    .it(
      ...
      (ctx) => {
        expect(ctx.stderr).to.contain(
          `Required input property 'name' missing.`
        );
      }
    );
  ...
  test
    ...
    .it(
      'fails to create an app and displays an error message if the endpoint is undefined',
      (ctx) => {
        expect(ctx.stderr).to.contain(
          'Failed to create app: TypeError: Only absolute URLs are supported\n'
        );
      }
    );
});
