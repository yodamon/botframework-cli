...
export default class LuisTrainRun extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    versionId: flags.string({
      description:
        '(required) Version to show training status (defaults to config:LUIS:versionId)',
    }),
    wait: flags.boolean({
      description: 'Wait until training complete and then display status',
    }),
  ...
  async run() {
    ...
    try {
      const trainingRequestStatus = await Train.train(
    ...
    try {
      ...
      const inProgress = trainingStatusData.filter((model: any) => {
        if (model.details && model.details.status) {
          return (
            model.details.status === 'InProgress' ||
            model.details.status === 'Queued'
          );
        }
      });
      if (inProgress.length > 0) {
        ...
        const output = jsonOutput
          ? JSON.stringify({ Status: completionMssg }, null, 2)
          : `${completionMssg} Training is complete`;
        this.log(output);
      }
    } catch (err) {
