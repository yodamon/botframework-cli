...
export default class LuisGenerateCs extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description:
        'Path to the file containing the LUIS application JSON model',
    }),
    out: flags.string({
      char: 'o',
      description:
    ...
    className: flags.string({
      description: 'Name of the autogenerated class (can include namespace)',
    }),
    force: flags.boolean({
      char: 'f',
      description:
  ...
  async run() {
    ...
    const pathPrefix =
      flags.in && path.isAbsolute(flags.in) ? '' : process.cwd();
    let app: any;
    try {
      app = flags.in
        ? await fs.readJSON(path.join(pathPrefix, flags.in))
        : JSON.parse(stdInput as string);
    } catch (err) {
    ...
    const outputPath = flags.out
      ? file.validatePath(flags.out, flags.className + '.cs', flags.force)
      : flags.out;

    this.log(
      `Generating file at ${
        outputPath || 'stdout'
      } that contains class ${space}.${flags.className}.`
    );
    try {
      await LuisToCsConverter.writeFromLuisJson(
        app,
        flags.className,
        space,
        outputPath
