...
export default class LuisVersionExport extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    versionId: flags.string({
      description:
        '(required) Version to export (defaults to config:LUIS:versionId)',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Save exported application to specified file, uses STDOUT if not specified (optional)',
    }),
    force: flags.boolean({
      char: 'f',
      description:
        'Overwrites output file if exists, otherwise creates a parallel numbered file (optional)',
      default: false,
    }),
  ...
  async run() {
    ...
    try {
      const messageData = await Version.export(
      ...
      if (out) {
        const writtenFilePath: string = await utils.writeToFile(
          out,
          messageData,
          force
        );
