...
export default class LuisVersionImport extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    appId: flags.string({
      description:
        '(required) LUIS application Id (defaults to config:LUIS:appId)',
    }),
    versionId: flags.string({
      description: 'Version to import (defaults to config:LUIS:versionId)',
    }),
  ...
  async run() {
    ...
    try {
      ...
      const messageData = await Version.import(
      ...
      const output = flags.json
        ? JSON.stringify({ Status: 'Success', version: messageData }, null, 2)
        : `App version successfully imported as version ${messageData}.`;
      this.log(output);
    } catch (err) {
