...
export default class LuisVersionClone extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    versionId: flags.string({
      description:
        '(required) Source version to clone (defaults to config:LUIS:versionId)',
    }),
    targetVersionId: flags.string({
      description: '(required) Destination version to create',
    }),
    endpoint: flags.string({ description: 'LUIS endpoint hostname' }),
    subscriptionKey: flags.string({
      description: 'LUIS authoring (Ocp-Apim-subscription) key',
    }),
  ...
  async run() {
    ...
    try {
      ...
      const output = flags.json
        ...
        : `App successfully cloned. Latest version is now: ${messageData}`;
      this.log(output);
    } catch (err) {
