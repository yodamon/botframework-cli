...
export default class LuisVersionList extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    out: flags.string({
      char: 'o',
      description:
        'Output results to specified folder and/or file in JSON format, otherwise prints to STDOUT (optional)',
    }),
    skip: flags.string({
      description: 'Number of entries to skip. Default: 0 (no skips)',
    }),
    take: flags.string({
      description:
        'Number of etnries to return. Maximum page size is 500. Default: 100',
    }),
    force: flags.boolean({
      char: 'f',
      description:
  ...
  async run() {
    ...
    try {
      const versionList = await Version.list(
