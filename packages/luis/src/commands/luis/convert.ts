...
export default class LuisConvert extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    sort: flags.boolean({
      description:
        'When set, intent, utterances, entities are alphabetically sorted in .lu files',
      default: false,
    }),
    out: flags.string({
      char: 'o',
    ...
    description: flags.string({
      description: 'Text describing the LUIS applicaion',
    }),
    culture: flags.string({
      description: 'Lang code for the LUIS application',
    }),
    versionid: flags.string({
      description: 'Version ID of the LUIS application',
    }),
    schemaversion: flags.string({
      description: 'Schema version of the LUIS application',
    }),
    force: flags.boolean({
      char: 'f',
      description:
  ...
  async run() {
    try {
      ...
      if (isLu) {
        result.luis_schema_version =
          flags.schemaversion || result.luis_schema_version || '3.2.0';
