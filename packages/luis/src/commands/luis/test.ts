...
export default class LuisTest extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description: 'Source .lu file or LUIS application JSON model for testing',
    }),
    out: flags.string({
      char: 'o',
      description:
    ...
    subscriptionKey: flags.string({
      char: 's',
      description: 'LUIS cognitive services subscription key',
      required: true,
    }),
    endpoint: flags.string({
      description: 'LUIS endpoint hostname',
    ...
    intentOnly: flags.boolean({
      description: 'Only test intent',
      default: false,
    }),
    staging: flags.boolean({
      description:
        'Presence of flag targets the staging app, if no flag passed defaults to production',
      default: false,
    }),
    allowIntentsCount: flags.integer({
      description:
        'Top-scoring intent or top n Intent with score to show in the result',
      default: 1,
    }),
    force: flags.boolean({
      description:
  ...
  private async writeOutput(convertedObject: any, flags: any, log: any) {
    ...
    const validatedLogPath = utils.validatePath(
      filePath + '.log',
      '',
      flags.force
    );
    ...
    try {
      ...
      throw new CLIError(
        'Unable to write file - ' + validatedPath + ' Error: ' + err.message
      );
    }
