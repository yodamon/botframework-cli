...
export default class LuisApplicationDelete extends Command {
  ...
  async run() {
    ...
    const { appId, endpoint, subscriptionKey } = await utils.processInputs(
      flags,
      flagLabels,
      configDir
    );
    ...
    if (flags.appId && !flags.force) {
      const deleteAppConfirmation = await cli.confirm(
        `Are you sure you would like to delete app with id: ${appId}? (Y/N)`
      );
    ...
    try {
      ...
      if (result.code === 'Success') {
        const output = flags.json
          ? JSON.stringify({ Status: 'Success', id: flags.appId }, null, 2)
