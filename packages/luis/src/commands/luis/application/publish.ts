...
export default class LuisApplicationPublish extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    versionId: flags.string({
      description:
        '(required) Version to publish (defaults to config:LUIS:versionId)',
    }),
    staging: flags.boolean({
      description:
        'Publishes application version to Staging slot, otherwise publish to production',
      default: false,
    }),
    direct: flags.boolean({
      description:
        'Available only in direct version query. Do not publish to staging or production',
      default: false,
    }),
  };
  ...
  async run() {
    ...
    try {
      const messageData = await Application.publish(
        { subscriptionKey, appId, endpoint },
