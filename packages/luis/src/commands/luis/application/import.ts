...
export default class LuisApplicationImport extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    endpoint: flags.string({
      description: '(required) LUIS endpoint hostname',
    }),
    subscriptionKey: flags.string({
      description:
        '(required) LUIS cognitive services subscription key (default: config subscriptionKey)',
    }),
    name: flags.string({ description: 'LUIS application name (optional)' }),
    in: flags.string({
      char: 'i',
      description:
        '(required) File path containing LUIS application contents, uses STDIN if not specified',
    }),
    save: flags.boolean({
      description:
        'Save configuration settings from imported app (appId, subscriptionKey & endpoint)',
    }),
  ...
  async run() {
    ...
    let { endpoint, subscriptionKey, name, inVal } = await utils.processInputs(
      flags,
      flagLabels,
      configDir
    );
    ...
    try {
      ...
      let messageData = await Application.import(
      ...
      const output: string = flags.json
        ? JSON.stringify({ Status: 'Success', id: messageData }, null, 2)
        : `App successfully imported with id ${messageData}.`;
      this.log(output);
