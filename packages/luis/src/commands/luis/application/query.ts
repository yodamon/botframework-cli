...
export default class LuisApplicationQuery extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    verbose: flags.boolean({
      description:
        'Returns all intents, otherwise only top scoring intent. (default: false)',
    }),
    timezoneOffset: flags.string({
      description:
        'Timezone offset for the location of the request in minutes (optional)',
    }),
    log: flags.boolean({
      description: 'Logs query operation on service (default: true)',
    }),
  };
  ...
  async run() {
    ...
    try {
      const predictionData = await Application.query(
