...
export default class LuisApplicationCreate extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    subscriptionKey: flags.string({
      description:
        '(required) LUIS cognitive services subscription key (default: config:LUIS:subscriptionKey)',
    }),
    name: flags.string({ description: '(required) Name of LUIS application' }),
    culture: flags.string({
      description: 'Specify culture language (default: en-us)',
    }),
    ...
    versionId: flags.string({
      description:
        '(required) LUIS version Id. (defaults to config:LUIS:versionId)',
    }),
    tokenizerVersion: flags.string({
      description:
        'Version specifies how sentences are tokenized (optional). See also: https://aka.ms/luistokens',
    }),
    save: flags.boolean({
      description:
        'Save configuration settings from imported app (appId & endpoint)',
    }),
  ...
  async run() {
    ...
    try {
      ...
      const output: string = flags.json
        ? JSON.stringify(
            { Status: 'App successfully created', id: response },
            null,
            2
          )
        : `App successfully created with id ${response}.`;
      this.log(output);
      ...
      if (save) {
        ...
        const saveConfigResponse = await this.saveImportedConfig(
          config,
          configDir
        );
        if (saveConfigResponse && !flags.json)
