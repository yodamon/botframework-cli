...
export default class LuisCrossTrain extends Command {
  ...
  static flags: flags.Input<any> = {
    help: flags.help({
      char: 'h',
      description: 'Luis:cross-train command help',
    }),
    in: flags.string({
      char: 'i',
      description: 'Source lu and qna files folder',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Output folder name. If not specified, the cross trained files will be written to cross-trained folder under folder of current command',
    }),
    config: flags.string({
      description: 'Path to config file of mapping rules',
    }),
    intentName: flags.string({
      description: 'Interruption intent name',
      default: '_Interruption',
    }),
  ...
  async run() {
    try {
      ...
      if (flags.config && flags.config !== '') {
        ...
        flags.config = await crossTrain.generateConfig(
          flags.in,
          flags.rootDialog
        );
      } else {
    ...
    if (fileIdToLuResourceMap) {
      ...
      for (const fileId of fileIdToLuResourceMap.keys()) {
        try {
          ...
          await fs.writeFile(
            validatedPath,
            fileIdToLuResourceMap.get(fileId).Content,
            'utf-8'
          );
        } catch (err) {
          throw new CLIError(
            `Unable to write to file ${fileId}. Error: ${err.message}`
          );
        }
      }
    }
