...
const fileExtEnum = require('@microsoft/bf-lu/lib/parser/utils/helpers')
  .FileExtTypeEnum;
...
export default class LuisBuild extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    region: flags.string({
      description: 'LUIS authoring region [westus|westeurope|australiaeast]',
      default: 'westus',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Output folder name to write out .dialog and settings files. If not specified, application setting will be output to console',
    }),
    defaultCulture: flags.string({
      description:
        'Culture code for the content. Infer from .lu if available. Defaults to en-us',
    }),
    fallbackLocale: flags.string({
      description:
        'Locale to be used at the fallback if no locale specific recognizer is found. Only valid if --out is set',
    }),
    suffix: flags.string({
      description:
        'Environment name as a suffix identifier to include in LUIS app name. Defaults to current logged in user alias',
    }),
    dialog: flags.string({
      description: 'Dialog recognizer type [multiLanguage|crosstrained]',
    ...
    luConfig: flags.string({
      description:
        'Path to config for lu build which can contain switches for arguments',
    }),
    deleteOldVersion: flags.boolean({
      description:
        'Deletes old version of LUIS application after building new one.',
    }),
    log: flags.boolean({
      description: 'Writes out log messages to console',
      default: false,
    }),
    endpoint: flags.string({
      description: 'Luis authoring endpoint for publishing',
    }),
    schema: flags.string({
      description: 'Defines $schema for generated .dialog files',
    }),
    isStaging: flags.boolean({
      description:
        'Publishes luis application to staging slot if set. Default to production slot',
      default: false,
    }),
  };
  ...
  async run() {
    try {
      ...
      if (flags.luConfig) {
        ...
        if (await fs.exists(configFilePath)) {
          const configObj = JSON.parse(
            await file.getContentFromFile(configFilePath)
          );
          for (let prop of Object.keys(configObj)) {
            if (prop === 'models') {
              files = configObj.models.map((m: string) =>
                path.isAbsolute(m)
                  ? m
                  : path.join(path.dirname(configFilePath), m)
              );
            } else if (prop === 'out') {
              flags.out = path.isAbsolute(configObj.out)
                ? configObj.out
                : path.join(path.dirname(configFilePath), configObj.out);
            } else {
      ...
      if (!authoringKey) {
        throw new CLIError(
          'Missing LUIS authoring key. Please pass authoring key with --authoringKey flag or specify via bf config:set:luis.'
        );
      }
      ...
      if (
        dialog &&
        dialog !== recognizerType.MULTILANGUAGE &&
        dialog !== recognizerType.CROSSTRAINED
      ) {
        throw new CLIError(
      ...
      defaultCulture =
        defaultCulture && defaultCulture !== '' ? defaultCulture : 'en-us';
      region = region && region !== '' ? region : 'westus';
      suffix =
        suffix && suffix !== '' ? suffix : (await username()) || 'development';
      fallbackLocale =
        fallbackLocale && fallbackLocale !== '' ? fallbackLocale : 'en-us';
      ...
      if ((inVal && inVal !== '') || files.length > 0) {
        ...
        if (inVal && inVal !== '') {
          const luFiles = await file.getLuFiles(
            inVal,
            true,
            fileExtEnum.LUFile
          );
        ...
        const loadedResources = await builder.loadContents(
          files,
          defaultCulture,
          suffix,
          region,
        ...
        multiRecognizers.set(
          ...
          new MultiLanguageRecognizer(
            path.join(process.cwd(), 'stdin.lu.dialog'),
            {}
          )
        );
        settings = new Settings(
          path.join(process.cwd(), `luis.settings.${suffix}.${region}.json`),
          {}
        );
        const recognizer = Recognizer.load(
          content.path,
          content.name,
          path.join(process.cwd(), `${content.name}.dialog`),
          settings.get('stdin'),
          {}
        );
        ...
        crosstrainedRecognizers.set(
          ...
          new CrosstrainedRecognizer(
            path.join(process.cwd(), 'stdin.lu.qna.dialog'),
            {}
          )
        );
      }
      ...
      const dialogContents = await builder.build(
        luContents,
        recognizers,
        authoringKey,
        endpoint,
      ...
      if (out) {
        ...
        const writeDone = await builder.writeDialogAssets(
          dialogContents,
          force,
          outputFolder,
          luConfig
        ...
        this.log(
          JSON.stringify(
            JSON.parse(dialogContents[dialogContents.length - 1].content).luis,
            null,
            4
          )
        );
