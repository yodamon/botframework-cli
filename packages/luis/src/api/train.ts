...
export default {
  async train(param: EndpointParameters, versionId: string) {
    let url =
      buildUrl(param.endpoint) + `/${param.appId}/versions/${versionId}/train`;

    return http.post(url, param.subscriptionKey, {});
  },

  async getStatus(param: EndpointParameters, versionId: string) {
    let url =
      buildUrl(param.endpoint) + `/${param.appId}/versions/${versionId}/train`;
