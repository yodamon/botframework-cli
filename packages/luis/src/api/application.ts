...
export default {
  ...
  async publish(param: EndpointParameters, applicationPublishObject: any) {
  ...
  async rename(param: EndpointParameters, name: string, description: string) {
    let url = buildUrl(param.endpoint) + `/${param.appId}`;
  ...
  async show(param: EndpointParameters) {
    let url = buildUrl(param.endpoint) + `/${param.appId}`;
    return http.get(url, param.subscriptionKey);
  },
};
