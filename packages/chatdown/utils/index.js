...
function initConversation(args, conversationId, activities) {
  ...
  args.accounts = {};
  args.accounts.bot = new ChannelAccount({
    id: getHashCode(args.bot),
    name: args.bot,
    role: 'bot',
  });
  ...
  let conversationUpdate = createConversationUpdate(
    args,
  ...
  for (let user of args.users) {
    user = user.trim();
    args.accounts[user.toLowerCase()] = new ChannelAccount({
      id: getHashCode(user),
      name: user,
      role: 'user',
    });
...
async function readCommandsFromAggregate(args, aggregate) {
  ...
  while ((result = commandRegExp.exec(aggregate))) {
    ...
    let typeOrField =
      split > 0 ? lines[0].substring(0, split).trim() : lines[0].trim();
  ...
  if (
    ...
    (currentActivity.suggestedActions &&
      currentActivity.suggestedActions.actions.length > 0)
  ) {
    newActivities.push(currentActivity);
  }
    ...
    switch (property) {
      ...
      case 'membersadded': {
        ...
        for (let memberAdded of membersAdded) {
          ...
          conversationUpdate.membersAdded.push(
            args.accounts[memberAdded.toLowerCase()]
          );
        }
        break;
      }
      ...
      case 'membersremoved': {
        ...
        for (let memberRemoved of membersRemoved) {
          ...
          conversationUpdate.membersRemoved.push(
            args.accounts[memberRemoved.toLowerCase()]
          );
        }
        break;
      }
...
function addCard(contentType, currentActivity, rest) {
  ...
  for (let line of lines) {
    ...
    switch (property) {
      ...
      default:
        console.warn(
          chalk.red.bold(`Skipping unknown card property ${property}\n${line}`)
        );
        break;
    }
  }
  let attachment = { contentType: contentType, content: card };
  (currentActivity.attachments || (currentActivity.attachments = [])).push(
    attachment
  );
}
...
async function addAttachment(activity, arg) {
  ...
  if (contentType) {
    ...
    contentType =
      mime.lookup(contentUrl) || cardContentTypes[path.extname(contentUrl)];
  ...
  if (contentUrl.indexOf('http') != 0) {
    ...
    if (!isCard(contentType) && charset !== 'UTF-8') {
      // send as base64
      contentUrl = `data:${contentType};base64,${Buffer.from(content).toString(
        'base64'
      )}`;
      content = undefined;
    } else {
  ...
  // send as contentUrl
  return (activity.attachments || (activity.attachments = [])).push(
    new Attachment({ contentType, contentUrl })
  );
}
