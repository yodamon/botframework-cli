...
export default class ChatdownConvert extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description:
        'The path of the chat file or directory to be parsed. A glob expression may be passed containing chat files to be processed all at once, ex. ./**/*.chat. If flag is omitted, stdin will be used. If an output directory is not present (-o), it will default the output to the current working directory.',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Path to the directory where the output of the multiple chat file processing (-o) will be placed.',
    }),
    stamp: flags.boolean({
      char: 's',
      description:
    ...
    prefix: flags.boolean({
      char: 'p',
      description: 'Prefix stdout with package name.',
    }),
    force: flags.boolean({
      char: 'f',
      description:
  ...
  async run() {
    try {
      ...
      let inputIsDirectory = flags.in
        ? flags.in.includes('*') || this.isDir(flags.in)
        : false;
      ...
      if (inputIsDirectory) {
        ...
        if (fileContents) {
          ...
          const writeConfirmation = await this.writeOut(
            activities,
            fileName,
            outputDir,
            flags.force
  ...
  private async writeOut(
    ...
    await new Promise((done) =>
      process.stdout.write(output, 'utf-8', () => done())
    );
    return true;
  }
}
