...
`;
      ...
      assert(ts === 2000);
      ts =
        new Date(activities[3].timestamp).getTime() -
        new Date(activities[2].timestamp).getTime();
      assert(ts === 100);
      ts =
        new Date(activities[4].timestamp).getTime() -
        new Date(activities[3].timestamp).getTime();
      assert(ts === 5000);
    });
...
`;
      ...
      const activities = await chatdown(conversation, {});
      assert.doesNotThrow(() =>
        Buffer.from(activities[5].attachments[0].contentUrl, 'base64')
      );
    });
    ...
    it('with the appropriate messages', async () => {
      ...
      activities.forEach((activity, index) => {
        if (activity.type == 'message') {
          assert(
            activity.text.trim() ===
              conversation[index + 1].replace('user: ', '').replace('bot: ', '')
          );
        }
      });
    });
...
bot: Hello, can I help you?`;
      ...
      assert.equal(
        ...
        new Date(
          new Date(activities[0].timestamp).getTime() + 2000
        ).toISOString()
      );
    });
...
bot->Joe: Hi Joe!`;
      ...
      chatdown(conversation)
        .then((result) => {
          ...
          assert.equal(
            result[0].membersAdded[0].name,
            'bot',
            'bot should be first'
          );
          assert.equal(
            result[1].membersAdded[0].name,
            'Joe',
            'Joe should be first'
          );
          assert.equal(
            result[1].membersAdded[1].name,
            'Susan',
            'Susan should be second'
          );
          assert.equal(
            result[2].membersRemoved[0].name,
            'Susan',
...
[OauthCard=
  ...
  it('support suggested Actions', async () => {
    const conversation = `bot: Hello [Suggestions=option 1|option 2]
