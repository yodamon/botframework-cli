...
const helpers = {
   ...
   */
  findLUFiles: function (
    inputFolder,
    getSubFolders,
    extType = this.FileExtTypeEnum.LUFile
  ) {
    let results = [];
    fs.readdirSync(inputFolder).forEach(function (dirContent) {
      ...
      if (getSubFolders && fs.statSync(dirContent).isDirectory()) {
        results = results.concat(
          helpers.findLUFiles(dirContent, getSubFolders, extType)
        );
      }
  ...
  findConfigFile: function (inputFolder) {
    ...
    try {
      ...
      throw new Error(
        `No config.json file found under folder ${path.resolve(inputFolder)}`
      );
    }
  },
  /**
  ...
  parseLinkURI: async function (utterance, srcId = null, luSearchFn = null) {
    ...
    if (splitReference.groups.fileName && srcId && luSearchFn) {
      let luObjects = await luSearchFn(srcId, [
        { filePath: splitReference.groups.fileName, includeInCollate: false },
      ]);
      if (luObjects && luObjects.length > 0)
        splitReference.groups.fileName = luObjects[0].id;
    }
  ...
  getRolesAndType: function (entityType) {
    ...
    let [parsedEntityType, parsedRoleDefinition] = entityType
      .split(RoleDetectionRegEx)
      .map((item) => item.trim());
    returnValue.entityType = parsedEntityType;
    if (parsedRoleDefinition !== undefined) {
      returnValue.roles = parsedRoleDefinition
        .replace('[', '')
        .replace(']', '')
        .split(RolesSplitRegEx)
        .map((item) => item.trim());
    }
    return returnValue;
  },
  ...
  checkAndUpdateVersion: function (finalLUISJSON) {
    ...
    if (finalLUISJSON.flatListOfEntityAndRoles)
      delete finalLUISJSON.flatListOfEntityAndRoles;
...
const updateToV7 = function (finalLUISJSON) {
  ...
  v7DefFound =
    (finalLUISJSON.entities || []).find((i) => i.children || i.features) ||
  ...
  if (v7DefFound) {
    ...
    (finalLUISJSON.entities || []).forEach((entity) =>
      transformAllEntityConstraintsToFeatures(entity, phraseListsNamesInFinal)
    );
    (finalLUISJSON.intents || []).forEach((intent) =>
      addIsRequiredProperty(intent)
    );
    // do we have nDepthEntities?
    let nDepthEntityExists = (finalLUISJSON.entities || []).find(
      (x) =>
    ...
    if (nDepthEntityExists !== undefined) {
      ...
      const curPath = ['$root$'];
      constructEntityParentTree(
        finalLUISJSON.entities,
        entityParentTree,
        curPath
      );
      ...
      updateModelBasedOnNDepthEntities(
        finalLUISJSON.utterances,
        entityParentTree
      );

      transformUtterancesWithNDepthEntities(finalLUISJSON, entityParentTree);
      verifyPatternsDoNotHaveChildEntityReferences(
        finalLUISJSON,
        entityParentTree
      );
    }
...
const removeDeadMLEntityDefinitions = function (finalLUISJSON) {
  ...
  (finalLUISJSON.entities || []).forEach((entity, idx) => {
    if (
      entity.explicitlyAdded === undefined ||
      entity.explicitlyAdded !== true
    ) {
      ...
      let entityIsPL = (finalLUISJSON.phraselists || []).find(
...
const updateModelBasedOnNDepthEntities = function (
  utterances,
  entityParentTree
) {
    ...
    utterance.entities.forEach((entityInUtterance) => {
      ...
      nonRootParents.forEach((tree) => {
        ...
        tree.forEach((parent) => {
          ...
          if (parentIsLabelled) return;
          let parentInUtterance = utterance.entities.find(
            (x) => x.entity == parent
          );
...
const verifyPatternsDoNotHaveChildEntityReferences = function (
  finalLUISJSON,
  entityParentTree
) {
  if (
    finalLUISJSON.patterns === undefined ||
    !Array.isArray(finalLUISJSON.patterns) ||
    finalLUISJSON.patterns.length === 0
  )
    return;
  (finalLUISJSON.patterns || []).forEach((pattern) => {
    // detect if pattern has an entity definition
    let entitiesRegExp = /{(?<entity>[^{,}]+)}/gim;
    ...
    if (entitiesFound !== null) {
      ...
      entitiesFound.forEach((entity) => {
        ...
        if (entityInTree !== undefined) {
          ...
          let isEntityAlsoRoot = entityInTree.find(
            (item) => item[0] === '$root$'
          );
          if (isEntityAlsoRoot === undefined) {
            throw new exception(
              retCode.errorCode.INVALID_INPUT,
              `Patterns cannot contain references to child entities. Pattern: "${pattern.pattern}" has reference to "{${entity}}".`
            );
          }
        }
      });
...
const updateEntityParentTreeWithAllEntityTypes = function (
  finalLUISJSON,
  entityParentTree
) {
  let collection = [
  ...
  collection.forEach((item) => {
    (finalLUISJSON[item] || []).forEach((entity) =>
      addEntityToParentTree(entityParentTree, entity.name)
    );
  });
};
...
const constructEntityParentTree = function (
  entityCollection,
  entityParentTree,
  curPath
) {
...
const updateTreeWithNode = function (curPath, entityName, entityParentTree) {
  ...
  if (entityParentTree[entityName] === undefined) {
    ...
    if (
      entityParentTree[entityName].find(
        (item) => item.join('->') == revPath.join('->')
      ) === undefined
    )
...
const transformUtterancesWithNDepthEntities = function (
  finalLUISJSON,
  entityParentTree
) {
  (finalLUISJSON.utterances || []).forEach((utt) => {
    if (
      utt.entities !== undefined &&
      Array.isArray(utt.entities) &&
      utt.entities.length !== 0
    ) {
      ...
      utt.entities.forEach((item, idx) => {
        ...
        parentPathsForEntity.forEach((path) => {
          utt.entities.find((i, id) => {
            if (
              i.entity === path[0] &&
              i.startPos <= item.startPos &&
              i.endPos >= item.endPos
            ) {
              parentIdx.push(id);
            }
          });
...
const removeNonRootChildren = function (entitiesList, allEntitiesList) {
  ...
  if (idxToRemove.length !== 0) {
    idxToRemove
      .sort((a, b) => b - a)
      .forEach((id) => entitiesList.splice(id, 1));
    idxToRemove = [];
  }
  // de-dupe children
...
const isRootEntity = function (entityName, entitiesCollection) {
  if (
    (entitiesCollection || []).find(
      (ecEntity) => ecEntity.name == entityName
    ) !== undefined
  )
    return true;
  return false;
...
const findParent = function (
  ...
  entityCollection.forEach((childEntity) => {
    ...
    if (
      ...
      Array.isArray(childEntity.children) &&
      childEntity.children.length !== 0
    ) {
      parentTree[childEntity.name] = {};
      numOfParentsFound += findParent(
        entityInUtt,
...
const transformAllEntityConstraintsToFeatures = function (
  entity,
  phraseListsInFinal
) {
  ...
  if (entity.hasOwnProperty('instanceOf') && entity.instanceOf !== null) {
    if (entity.hasOwnProperty('features') && Array.isArray(entity.features)) {
      let featureFound = (entity.features || []).find(
        (i) => i.modelName == entity.instanceOf
      );
      if (featureFound !== undefined) {
        ...
        entity.features.push(
          new hClasses.entityFeature(entity.instanceOf, true)
        );
      }
    } else {
