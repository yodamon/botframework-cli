...
module.exports = {
  ...
  header: function (space, className, writer) {
    ...
    writer.increaseIndentation();
    //Text
    writer.writeLineIndented(['[JsonProperty("text")]', 'public string Text;']);
    writer.writeLine();
    writer.writeLineIndented([
  ...
  entities: function (app, writer) {
    ...
    if (app.composites.length > 0) {
      ...
      app.composites.forEach((composite) => {
        ...
        composite.attributes.forEach((attr) => {
          writer.writeLineIndented([
            `public InstanceData[] ${parse_multi_platform_luis_1.jsonPropertyName(
              attr
            )};`,
          ]);
        });
        ...
        composite.attributes.forEach((attr) => {
          writer.writeLineIndented(
            this.getEntityWithType(
              attr,
              app.closedLists.includes(attr) ? 'list' : attr
            )
          );
        });
        writer.writeLineIndented([
          '[JsonProperty("$instance")]',
          `public _Instance${lodash.upperFirst(
        ...
        writer.writeLineIndented([
          '}',
          `public ${lodash.upperFirst(composite.compositeName)}Class[] ${
            composite.compositeName
          };`,
        ]);
      });
    ...
    // Instance
    writer.writeLine();
    writer.writeLineIndented(['// Instance', 'public class _Instance', '{']);
    writer.increaseIndentation();
  ...
  getEntityWithType: function (entityNameOrObject, entityType = '') {
    if (
      typeof entityNameOrObject === 'object' &&
      'name' in entityNameOrObject
    ) {
      if ('instanceOf' in entityNameOrObject) {
        ...
        let name = parse_multi_platform_luis_1.jsonPropertyName(
          entityNameOrObject.name
        );
    ...
    return (
      result +
      ` ${parse_multi_platform_luis_1.jsonPropertyName(entityNameOrObject)};`
    );
  },
  converter: function (className, writer) {
    ...
    writer.increaseIndentation();
    writer.writeLineIndented('JsonConvert.SerializeObject(');
    writer.increaseIndentation();
    writer.writeLineIndented([
      'result,',
