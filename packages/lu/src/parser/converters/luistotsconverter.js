...
module.exports = {
  ...
  intents: function (app, writer) {
    ...
    app.intents.forEach((intent) => {
      writer.writeLineIndented(
        `${parse_multi_platform_luis_1.normalizeName(intent)}: IntentData`
      );
    });
  ...
  entities: function (app, writer) {
    ...
    app.composites.forEach((composite) => {
      let name = parse_multi_platform_luis_1.normalizeName(
      ...
      composite.attributes.forEach((attribute) => {
        writer.writeLineIndented(
          `${parse_multi_platform_luis_1.jsonPropertyName(
            attribute
          )}?: InstanceData[]`
        );
      });
      ...
      composite.attributes.forEach((attribute) => {
        writer.writeLineIndented(
          this.getEntityWithType(attribute, this.isList(attribute, app))
        );
      });
