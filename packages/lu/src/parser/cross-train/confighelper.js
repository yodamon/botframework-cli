...
const createConfig = function (rootDialog, dialogs, configPath) {
  ...
  if (!fs.existsSync(rootLuPath)) {
    throw new exception(
      retCode.errorCode.INVALID_INPUT,
      `Failed to parse mapping rules config from file system: ${rootLuPath} does not exist. Please provide config file by --config`
    );
  }
    ...
    if (trigger.$type && trigger.$type === 'Microsoft.OnIntent') {
      ...
      for (const action of actions) {
        ...
        const target = dialogs.find(
          (dialog) => path.basename(dialog.path, dialogExt) === dialogName
        );
        ...
        if (!result[key].triggers[trigger.intent]) {
          ...
          result[key].triggers[trigger.intent] = [
            result[key].triggers[trigger.intent],
            relativePath,
          ];
        } else {
