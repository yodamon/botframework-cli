...
const prebuiltEntityTypes = require('../utils/enums/luisbuiltintypes')
  .consolidatedList;
...
module.exports = {
  ...
  crossTrain: async function (
    ...
    try {
      let { luObjectArray, qnaObjectArray } = pretreatment(
        luContents,
        qnaContents
      );
      ...
      let {
        fileIdToResourceMap: luFileIdToResourceMap,
        allEmpty: allLuEmpty,
      } = await parseAndValidateContent(luObjectArray, verbose, importResolver);

      // parse qna content to LUResource object
      let {
        fileIdToResourceMap: qnaFileIdToResourceMap,
        allEmpty: allQnAEmpty,
      } = await parseAndValidateContent(
        qnaObjectArray,
        verbose,
        importResolver
      );
      ...
      if (!allLuEmpty) {
        ...
        let resources = constructResoureTree(
          luFileIdToResourceMap,
          triggerRules
        );
        ...
        for (const rootObjectId of rootIds) {
          if (
            resources.some(
              (r) => r.id.toLowerCase() === rootObjectId.toLowerCase()
            )
          ) {
            // do cross training for each root at top level
            const result = luCrossTrain(
              rootObjectId,
              resources,
              qnaFileIdToResourceMap,
              intentName
            );
            ...
            throw new exception(
              ...
              `Sorry, root lu file '${rootObjectId}' does not exist`
            );
          }
        }
      }
      ...
      return {
        luResult: luFileIdToResourceMap,
        qnaResult: qnaFileIdToResourceMap,
      };
    } catch (err) {
      throw err;
    }
...
const constructResoureTree = function (fileIdToLuResourceMap, triggerRules) {
  ...
  let lowerCasefileIdsFromInput = Array.from(
    fileIdToLuResourceMap.keys()
  ).map((x) => x.toLowerCase());
  ...
  for (const fileId of fileIdsFromInput) {
    ...
    for (const section of luResource.Sections) {
      if (
        section.SectionType === LUSectionTypes.SIMPLEINTENTSECTION ||
    ...
    const intentToDestLuFiles =
      triggerRules[
        triggerKeys.find((k) => k.toLowerCase() === fileId.toLowerCase())
      ];
    for (const triggerIntent of Object.keys(intentToDestLuFiles)) {
      if (
        triggerIntent !== '' &&
        !intents.some((i) => i.Name === triggerIntent)
      ) {
        throw new exception(
          retCode.errorCode.INVALID_INPUT,
          `Sorry, trigger intent '${triggerIntent}' is not found in lu file: ${fileId}`
        );
      }
      ...
      if (destLuFiles.length > 0) {
        destLuFiles.forEach((destLuFile) => {
          if (
            destLuFile !== '' &&
            !lowerCasefileIdsFromInput.includes(destLuFile.toLowerCase())
          ) {
            throw new exception(
              retCode.errorCode.INVALID_INPUT,
              `Sorry, lu file '${destLuFile}' is not found`
            );
          } else {
            resource.children.push({
              target:
                fileIdsFromInput.find(
                  (x) => x.toLowerCase() === destLuFile.toLowerCase()
                ) || '',
 ...
 */
const luCrossTrain = function (
  rootResourceId,
  resources,
  qnaFileToResourceMap,
  intentName
) {
  ...
  // Parse resources
  let rootResource = resources.filter(
    (r) => r.id.toLowerCase() === rootResourceId.toLowerCase()
  )[0];
  rootResource.visited = true;
  mergeRootInterruptionToLeaves(
    rootResource,
    idToResourceMap,
    qnaFileToResourceMap,
    intentName
...
const mergeRootInterruptionToLeaves = function (
  rootResource,
  result,
  qnaFileToResourceMap,
  intentName
  ...
  for (const child of rootResource.children) {
    ...
    if (childResource && childResource.visited === undefined) {
      let rootQnaFileId = rootResource.id
        .toLowerCase()
        .replace(
          new RegExp(helpers.FileExtTypeEnum.LUFile + '$'),
          helpers.FileExtTypeEnum.QnAFile
        );
      rootQnaFileId = Array.from(qnaFileToResourceMap.keys()).find(
        (x) => x.toLowerCase() === rootQnaFileId
      );
      const rootQnaResource = qnaFileToResourceMap.get(rootQnaFileId);
      const newChildResource = mergeFatherInterruptionToChild(
        rootResource,
        rootQnaResource,
        childResource,
        intentName
...
const mergeBrothersInterruption = function (resource, result, intentName) {
  ...
  for (const child of children) {
    ...
    if (destLuFile !== '')
      intentsWithSameTarget = children
        .filter((c) => c.target === destLuFile && c.intent !== '')
        .map((x) => x.intent);

    const brotherSections = resource.content.Sections.filter(
      (s) =>
        s.Name !== triggerIntent &&
        s.Name !== intentName &&
        (s.SectionType === LUSectionTypes.SIMPLEINTENTSECTION ||
          s.SectionType === LUSectionTypes.NESTEDINTENTSECTION) &&
    ...
    brotherSections.forEach((s) => {
      if (s.SectionType === LUSectionTypes.SIMPLEINTENTSECTION) {
        brotherUtterances = brotherUtterances.concat(
          s.UtteranceAndEntitiesMap.map((u) => u.utterance).filter(
            (i) => !patternWithPrebuiltEntity(i)
          )
        );
      } else {
        s.SimpleIntentSections.forEach((section) => {
          brotherUtterances = brotherUtterances.concat(
            section.UtteranceAndEntitiesMap.map((u) => u.utterance).filter(
              (i) => !patternWithPrebuiltEntity(i)
            )
          );
        });
    ...
    if (targetResource) {
      targetResource = mergeInterruptionIntent(
        brotherUtterances,
        targetResource,
        intentName
      );
...
const mergeFatherInterruptionToChild = function (
  fatherResource,
  fatherQnaResource,
  childResource,
  intentName
  ...
  const fatherInterruptions = fatherResource.content.Sections.filter(
    (s) => s.Name === intentName
  );
  if (fatherInterruptions && fatherInterruptions.length > 0) {
    const fatherInterruption = fatherInterruptions[0];
    fatherUtterances = fatherUtterances.concat(
      fatherInterruption.UtteranceAndEntitiesMap.map((u) => u.utterance)
    );
  }
  ...
  if (fatherQnaResource) {
    const qnaSections = fatherQnaResource.Sections.filter(
      (s) => s.SectionType === LUSectionTypes.QNASECTION
    );
  ...
  if (fatherUtterances.length > 0) {
    childResource = mergeInterruptionIntent(
      fatherUtterances,
      childResource,
      intentName
    );
...
const mergeInterruptionIntent = function (
  fromUtterances,
  toResource,
  intentName
) {
  ...
  let existingUtterances = extractIntentUtterances(
    toResource.content
  ).map((u) => u.toLowerCase());
  const toInterruptions = toResource.content.Sections.filter(
    (section) => section.Name === intentName
  );
  if (toInterruptions && toInterruptions.length > 0) {
    ...
    // update section here
    toResource.content = new SectionOperator(toResource.content).updateSection(
      toInterruption.Id,
      newFileContent
    );
  } else {
    // construct new content here
    const dedupUtterances = dedupFromUtterances.filter(
      (u) => !existingUtterances.includes(u.toLowerCase())
    );
    if (dedupUtterances && dedupUtterances.length > 0) {
      ...
      if (toResource.content.Content !== '') {
        toResource.content = new SectionOperator(toResource.content).addSection(
          newFileContent
        );
      }
    }
...
const removeDupUtterances = function (resource) {
  ...
  resource.Sections.forEach((s) => {
    if (s.SectionType === LUSectionTypes.SIMPLEINTENTSECTION) {
      const intentUtterances = s.UtteranceAndEntitiesMap.map(
        (u) => u.utterance
      );
      ...
      if (intentUtterances.length > dedupedUtterances.length) {
        ...
        newResource = new SectionOperator(newResource).updateSection(
          s.Id,
          newSectionContent
        );
      }
...
const extractIntentUtterances = function (resource, intentName) {
  const intentSections = resource.Sections.filter(
    (s) =>
      s.SectionType === LUSectionTypes.SIMPLEINTENTSECTION ||
      s.SectionType === LUSectionTypes.NESTEDINTENTSECTION
  );
  ...
  if (intentName && intentName !== '') {
    const specificSections = intentSections.filter(
      (s) => s.Name === intentName
    );
    if (specificSections.length > 0) {
      intentUtterances = intentUtterances.concat(
        specificSections[0].UtteranceAndEntitiesMap.map((u) => u.utterance)
      );
    }
  } else {
    intentSections.forEach((s) => {
      if (s.SectionType === LUSectionTypes.SIMPLEINTENTSECTION) {
        intentUtterances = intentUtterances.concat(
          s.UtteranceAndEntitiesMap.map((u) => u.utterance)
        );
      } else {
        s.SimpleIntentSections.forEach((section) => {
          intentUtterances = intentUtterances.concat(
            section.UtteranceAndEntitiesMap.map((u) => u.utterance)
          );
        });
      }
    });
 ...
 * @throws {exception} throws errors
 */
const qnaCrossTrain = function (
  qnaFileIdToResourceMap,
  luFileIdToResourceMap,
  interruptionIntentName,
  allLuEmpty
) {
  try {
    for (const qnaObjectId of Array.from(qnaFileIdToResourceMap.keys())) {
      let luObjectId = qnaObjectId
        .toLowerCase()
        .replace(
          new RegExp(helpers.FileExtTypeEnum.QnAFile + '$'),
          helpers.FileExtTypeEnum.LUFile
        );
      let fileName = path.basename(qnaObjectId, path.extname(qnaObjectId));
      const culture = fileHelper.getCultureFromPath(qnaObjectId);
      fileName = culture
        ? fileName.substring(0, fileName.length - culture.length - 1)
        : fileName;

      luObjectId = Array.from(luFileIdToResourceMap.keys()).find(
        (x) => x.toLowerCase() === luObjectId
      );
      if (luObjectId) {
        const { luResource, qnaResource } = qnaCrossTrainCore(
          luFileIdToResourceMap.get(luObjectId),
          qnaFileIdToResourceMap.get(qnaObjectId),
          fileName,
          interruptionIntentName,
          allLuEmpty
        );
        ...
        let qnaResource = qnaAddMetaData(
          qnaFileIdToResourceMap.get(qnaObjectId),
          fileName
        );
        qnaFileIdToResourceMap.set(qnaObjectId, qnaResource);
 ...
 */
const qnaCrossTrainCore = function (
  luResource,
  qnaResource,
  fileName,
  interruptionIntentName,
  ...
  // extract questions
  const qnaSections = qnaResource.Sections.filter(
    (s) => s.SectionType === LUSectionTypes.QNASECTION
  );
  let questions = [];
  qnaSections.forEach((q) => (questions = questions.concat(q.Questions)));
  ...
  let utterancesOfInterruption = extractIntentUtterances(
    luResource,
    interruptionIntentName
  );

  // extract lu utterances except interruption
  let utterancesOfLocalIntents = utterances
    .filter((u) => !utterancesOfInterruption.includes(u))
    .map((u) => u.toLowerCase());

  // remove questions which are duplicated with local lu utterances
  let dedupedQuestions = questions.filter(
    (q) => !utterancesOfLocalIntents.includes(q.toLowerCase())
  );

  // update interruption intent if there are duplications with questions
  let dedupedQuestionsOfLowerCase = dedupedQuestions.map((u) =>
    u.toLowerCase()
  );
  if (
    utterancesOfInterruption.some((u) =>
      dedupedQuestionsOfLowerCase.includes(u.toLowerCase())
    )
  ) {
    utterancesOfInterruption = utterancesOfInterruption.filter(
      (u) => !dedupedQuestionsOfLowerCase.includes(u.toLowerCase())
    );

    // get section id
    const sectionId = trainedLuResource.Sections.filter(
      (s) => s.Name === interruptionIntentName
    )[0].Id;

    // construct updated interruption intent content
    utterancesOfInterruption = utterancesOfInterruption.map((u) =>
      '- '.concat(u)
    );
    let updatedSectionContent = utterancesOfInterruption.join(NEWLINE);
    if (updatedSectionContent && updatedSectionContent !== '') {
      trainedLuResource = new SectionOperator(trainedLuResource).updateSection(
        sectionId,
        `# ${interruptionIntentName}${NEWLINE}${updatedSectionContent}`
      );
    }
  }

  // construct questions content
  dedupedQuestions = dedupedQuestions
    .map((q) => '- '.concat(q))
    .filter((i) => !patternWithPrebuiltEntity(i));
  let questionsContent = dedupedQuestions.join(NEWLINE);

  // cross training comments
  const crossTrainingComments =
  ...
  if (!allLuEmpty && questionsContent && questionsContent !== '') {
    ...
    trainedLuResource = new SectionOperator(trainedLuResource).addSection(
      questionsToUtterances
    );
  }
  ...
  const utterancesWithoutPatterns = utterances.filter(
    (i) => /{([^}]+)}/g.exec(i) === null
  );

  // remove utterances which are duplicated with local qna questions
  let questionsOfLowerCase = questions.map((q) => q.toLowerCase());
  let dedupedUtterances = utterancesWithoutPatterns.filter(
    (u) => !questionsOfLowerCase.includes(u.toLowerCase())
  );
  ...
  while (dedupedUtterances.length > 0) {
    let subDedupedUtterances = dedupedUtterances.splice(
      0,
      MAX_QUESTIONS_PER_ANSWER
    );
    ...
    trainedQnaResource = new SectionOperator(trainedQnaResource).addSection(
      utterancesToQuestion
    );
  }
...
const qnaAddMetaData = function (qnaResource, fileName) {
  ...
  for (const qnaSection of qnaSections) {
    ...
    qnaSectionContent += `# ? ${Array.from(new Set(qnaSection.Questions)).join(
      NEWLINE + '- '
    )}${NEWLINE}${NEWLINE}**Filters:**${NEWLINE}- ${qnaSection.FilterPairs.map(
      (f) => f.key + '=' + f.value
    ...
    if (qnaSection.promptsText && qnaSection.promptsText.length > 0) {
      qnaSectionContent += `${NEWLINE}${NEWLINE}**Prompts:**${NEWLINE}- ${qnaSection.promptsText.join(
        NEWLINE + '- '
      )}`;
    }
  ...
  if (qnaContents && qnaContents !== '') {
    const modelInfoSections = qnaResource.Sections.filter(
      (s) => s.SectionType === LUSectionTypes.MODELINFOSECTION
    );
    const modelInforContent = modelInfoSections
      .map((m) => m.ModelInfo)
      .join(NEWLINE);
    if (modelInforContent && modelInforContent !== '')
      qnaContents = NEWLINE + qnaContents;

    resultQnaResource = new SectionOperator(
      new LUResource([], modelInforContent, [])
    ).addSection(qnaContents);
  }
 ...
 * @throws {exception} throws errors
 */
const parseAndValidateContent = async function (
  objectArray,
  verbose,
  importResolver
) {
  ...
  for (const object of objectArray) {
    ...
    if (object.content && object.content !== '') {
      if (object.id.toLowerCase().endsWith(fileExtEnum.LUFile)) {
        let result = await LuisBuilderVerbose.build(
          [object],
          verbose,
          undefined,
          importResolver
        ...
        let result = await qnaBuilderVerbose.build(
          [object],
          verbose,
          importResolver
        );
    ...
    if (resource.Errors && resource.Errors.length > 0) {
      if (verbose) {
        var warns = resource.Errors.filter(
          (error) =>
            error &&
            error.Severity &&
            error.Severity === DiagnosticSeverity.WARN
        );
        if (warns.length > 0) {
      ...
      var errors = resource.Errors.filter(
        (error) =>
          error && error.Severity && error.Severity === DiagnosticSeverity.ERROR
      );
      if (errors.length > 0) {
        throw new exception(
          retCode.errorCode.INVALID_INPUT_FILE,
          `Invlid file ${object.Id}: ${errors
