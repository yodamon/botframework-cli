...
module.exports = {
  ...
  Build: async function (luObjArray, verbose, luis_culture, luSearchFn) {
    let allParsedContent = await buildLuJsonObject(
      luObjArray,
      verbose,
      luis_culture,
      luSearchFn
...
const buildRefTree = async function (allParsedContent, luSearchFn) {
  let refs = {};
  await Promise.all(
    allParsedContent.LUISContent.map(async (parserObj, objIdx) => {
      let luObj = {
      ...
      (parserObj.LUISJsonStructure.utterances || []).forEach(
        async (utterance, uttIdx) => {
          parserObj.LUISJsonStructure.uttHash[utterance.text] = '';
          if (helpers.isUtteranceLinkRef(utterance.text)) {
            let parsedLinkUri = await helpers.parseLinkURI(
              utterance.text,
              parserObj.srcFile,
              luSearchFn
            );
  ...
  await Promise.all(
    allParsedContent.QnAContent.map(async (parserObj, objIdx) => {
      let qnaObj = {
      ...
      if (result.utterances !== undefined) {
        result.utterances.forEach((utt) => {
          if (luObj.uttHash[utt] === undefined) {
            luObj.utterances.push(
              new hClasses.uttereances(utt, ref.uttObj.intent)
            );
            luObj.uttHash[utt] = '';
          }
        });
  ...
  entitiesFound.forEach(function (entity) {
    ...
    if (isNewEntity(luObj, entityName)) {
      if (!patternAnyInMaster && roleName !== '') {
        parseFileContents.addItemOrRoleIfNotPresent(
          luObj,
      ...
      if (!patternAnyInMaster) {
        parseFileContents.addItemIfNotPresent(
          luObj,
          LUISObjNameEnum.PATTERNANYENTITY,
          entity
        );
      ...
      if (roleName !== '') {
        !patternAnyInMaster.roles.includes(roleName)
          ? patternAnyInMaster.roles.push(roleName)
          : undefined;
      }
      return;
    }
...
const resolveQuestionRef = function (srcId, ref, refTree) {
  ...
  let newId = path.resolve(
    path.dirname(srcFile ? srcFile : ''),
    ref.parsedLink.fileName
  );
  let tgtId =
    refTree[ref.parsedLink.fileName] &&
    refTree[ref.parsedLink.fileName].qna !== undefined
      ? ref.parsedLink.fileName
      : undefined;
  tgtId =
    tgtId === undefined &&
    refTree[newId] !== undefined &&
    refTree[newId].qna !== undefined
      ? newId
      : tgtId;
  ...
  if (
    tgtObj &&
    ((tgtObj.luis && tgtObj.luis.refs.length !== 0) ||
      (tgtObj.qna && tgtObj.qna.refs.length !== 0))
  )
    resolveRefs(refTree, tgtId);
  let parseLCasePath = ref.parsedLink.path.toLowerCase();
  let qnaObj =
    tgtObj && tgtObj.qna && tgtObj.qna.obj ? tgtObj.qna.obj : undefined;
  let qnaAlt =
    tgtObj && tgtObj.qna && tgtObj.qna.alt ? tgtObj.qna.alt : undefined;
  ...
  if (parseLCasePath.startsWith('*answers*')) {
    parsedQnABlobs.forEach((blob) =>
      blob.qnaList.forEach((item) => utterances.push(item.answer))
    );
  } else if (
    ref.parsedLink.path.length > 1 &&
    parseLCasePath.startsWith('?') &&
    parseLCasePath.endsWith('?')
  ) {
    let itemsFound = undefined;
    let testQuestion = ref.parsedLink.path
      .replace(/\?/g, '')
      .replace(/-/g, ' ')
      .trim();
    ...
    parsedQnABlobs.forEach((blob) => {
      ...
      itemsFound = blob.qnaList.find((item) =>
        item.questions.includes(testQuestion)
      );
    });
    if (itemsFound) {
    ...
    parsedQnAAlterations.forEach((blob) =>
      blob.wordAlterations.forEach((item) =>
    ...
    parsedQnABlobs.forEach((blob) =>
      blob.qnaList.forEach((item) =>
        item.questions.forEach((question) => utterances.push(question))
      )
    );
  }
...
const resolveUttAndPattRef = function (srcId, ref, refTree) {
  ...
  let tgtId =
    refTree[ref.parsedLink.fileName] &&
    refTree[ref.parsedLink.fileName].luis !== undefined
      ? ref.parsedLink.fileName
      : undefined;
  tgtId =
    tgtId === undefined &&
    refTree[newId] !== undefined &&
    refTree[newId].luis !== undefined
      ? newId
      : tgtId;
  let tgtObj = refTree[ref.parsedLink.fileName] || refTree[newId] || undefined;
  if (!tgtObj) {
  ...
  if (parseLCasePath.endsWith('*utterancesandpatterns*')) {
    ...
    referenceIntent = parseLCasePath
      .replace(/-/g, ' ')
      .replace('*utterancesandpatterns*', '')
      .trim();
...
const resolveRefByType = function (srcId, ref, refTree) {
  let filter = ref.parsedLink.path.endsWith('?')
    ? resolveQuestionRef
    : resolveUttAndPattRef;
  return filter(srcId, ref, refTree);
};

const buildLuJsonObject = async function (
  luObjArray,
  log,
  luis_culture,
  luSearchFn = resolveLuContent
) {
  ...
  while (filesToParse.length > 0) {
    ...
    if (haveLUISContent(parsedContent.LUISJsonStructure)) {
      allParsedLUISContent.push(
        parserObject.create(
          parsedContent.LUISJsonStructure,
          undefined,
          undefined,
          luOb.id,
          luOb.includeInCollate
        )
      );
    }

    allParsedQnAContent.push(
      parserObject.create(
        undefined,
        parsedContent.qnaJsonStructure,
        undefined,
        luOb.id,
        luOb.includeInCollate
      )
    );
    allParsedAlterationsContent.push(
      parserObject.create(
        undefined,
        undefined,
        parsedContent.qnaAlterations,
        luOb.id,
        luOb.includeInCollate
      )
    );
    ...
    let foundLuFiles = await luSearchFn(
      luOb.id,
      parsedContent.additionalFilesToParse
    );
    for (let i = 0; i < foundLuFiles.length; i++) {
      if (parsedFiles.includes(foundLuFiles[i].id)) {
        let duplicated = foundLuFiles.splice(i--, 1);
        updateParsedFiles(
          allParsedLUISContent,
          allParsedQnAContent,
          allParsedAlterationsContent,
          duplicated
        );
      }
    }
...
const resolveLuFileContent = function (file, luObjects, srcId) {
  let parentFilePath =
    srcId === 'stdin' ? process.cwd() : path.parse(path.resolve(srcId)).dir;
...
const resolveLuUriContent = async function (srcId, toResolve, luObjects) {
  ...
  if (uri !== undefined) {
    ...
    luObjects.push(
      new luObject(
        encodedRes,
        new luOptions(toResolve.filePath, toResolve.includeInCollate)
      )
    );
  }
};
...
const updateParsedFiles = function (
  allParsedLUISContent,
  allParsedQnAContent,
  allParsedAlterationsContent,
  luobject
  ...
  if (luobject.includeInCollate) {
    let matchInLUIS = allParsedLUISContent.find(
      (item) => item.srcFile == luobject.id
    );
    if (matchInLUIS) matchInLUIS.includeInCollate = true;

    let matchInQnA = allParsedQnAContent.find(
      (item) => item.srcFile == luobject.id
    );
    if (matchInQnA) matchInQnA.includeInCollate = true;

    let matchInAlterations = allParsedAlterationsContent.find(
      (item) => item.srcFile == luobject.id
    );
...
const haveLUISContent = function (blob) {
  ...
  return (
    blob[LUISObjNameEnum.INTENT].length > 0 ||
...
const isNewEntity = function (luisModel, entityName) {
  let simpleEntityInMaster = luisModel.entities.find(
    (item) => item.name == entityName
  );
  let compositeInMaster = luisModel.composites.find(
    (item) => item.name == entityName
  );
  let listEntityInMaster = luisModel.closedLists.find(
    (item) => item.name == entityName
  );
  let regexEntityInMaster = luisModel.regex_entities.find(
    (item) => item.name == entityName
  );
  let prebuiltInMaster = luisModel.prebuiltEntities.find(
    (item) => item.name == entityName
  );
...
const parseLuFile = async function (luOb, log, luis_culture) {
  ...
  if (!luOb.content) {
    let error = BuildDiagnostic({
      message: `Cannot parse empty ${luOb.id}. Please add content to the file or remove it.`,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT_FILE, error.toString());
  }
  try {
    parsedContent = await parseFileContents.parseFile(
      luOb.content,
      log,
      luis_culture
    );
