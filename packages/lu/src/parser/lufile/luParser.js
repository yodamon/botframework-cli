...
const FileContext = require('./generated/LUFileParser').LUFileParser
  .FileContext;
...
class LUParser {
  ...
  static parseWithRef(text, luResource, config) {
    ...
    const sectionEnabled = luResource
      ? this.isSectionEnabled(luResource.Sections)
      : undefined;
  ...
  static extractFileContent(
    ...
    if (
      modelInfoSections &&
      modelInfoSections.length > 0 &&
      !config.enableModelDescription
    ) {
      errors.push(
        BuildDiagnostic({
    ...
    modelInfoSections.forEach(
      (section) => (errors = errors.concat(section.Errors))
    );
    sections = sections.concat(modelInfoSections);

    try {
      let isSectionEnabled =
        sectionEnabled === undefined
          ? this.isSectionEnabled(sections)
          : sectionEnabled;

      let nestedIntentSections = this.extractNestedIntentSections(
        fileContent,
        content
      );
      nestedIntentSections.forEach(
        (section) => (errors = errors.concat(section.Errors))
      );
      if (isSectionEnabled) {
        ...
        nestedIntentSections.forEach((section) => {
          ...
          const range = new Range(
            section.Range.Start,
            new Position(section.Range.Start.Line, endCharacter)
          );
    ...
    try {
      let simpleIntentSections = this.extractSimpleIntentSections(
        fileContent,
        content
      );
      simpleIntentSections.forEach(
        (section) => (errors = errors.concat(section.Errors))
      );
  ...
  static extractNestedIntentSections(fileContext, content) {
    ...
    let nestedIntentSections = fileContext
      .paragraph()
      .map((x) => x.nestedIntentSection())
      .filter((x) => x !== undefined && x !== null);

    let nestedIntentSectionList = nestedIntentSections.map(
      (x) => new NestedIntentSection(x, content)
    );
  ...
  static extractSimpleIntentSections(fileContext, content) {
    ...
    let simpleIntentSections = fileContext
      .paragraph()
      .map((x) => x.simpleIntentSection())
      .filter((x) => x && x.intentDefinition());

    let simpleIntentSectionList = simpleIntentSections.map(
      (x) => new SimpleIntentSection(x, content)
    );
  ...
  static extractEntitiesSections(fileContext) {
    ...
    let entitySections = fileContext
      .paragraph()
  ...
  static extractNewEntitiesSections(fileContext) {
    ...
    let newEntitySections = fileContext
      .paragraph()
      .map((x) => x.newEntitySection())
      .filter((x) => x && x.newEntityDefinition());

    let newEntitySectionList = newEntitySections.map(
      (x) => new NewEntitySection(x)
    );
  ...
  static extractImportSections(fileContext) {
    ...
    let importSections = fileContext
      .paragraph()
  ...
  static extractReferenceSections(fileContext) {
    ...
    let referenceSections = fileContext
      .paragraph()
      .map((x) => x.referenceSection())
      .filter((x) => x !== undefined && x !== null);

    let referenceSectionList = referenceSections.map(
      (x) => new ReferenceSection(x)
    );
  ...
  static extractModelInfoSections(fileContext) {
    ...
    let modelInfoSections = fileContext
      .paragraph()
      .map((x) => x.modelInfoSection())
      .filter((x) => x !== undefined && x !== null);

    let modelInfoSectionList = modelInfoSections.map(
      (x) => new ModelInfoSection(x)
    );
    ...
    for (index = 0; index < sections.length; index++) {
      ...
      if (section.SectionType === SectionType.NESTEDINTENTSECTION) {
        if (
          ...
          simpleIntentSections[simpleIntentSections.length - 1].Entities.push(
            sections[index + 1]
          );
          simpleIntentSections[simpleIntentSections.length - 1].Errors.push(
            ...sections[index + 1].Errors
          );
          index++;
          ...
          while (
            ...
            (sections[index + 1].SectionType === SectionType.ENTITYSECTION ||
              sections[index + 1].SectionType ===
                SectionType.NEWENTITYSECTION ||
              (sections[index + 1].SectionType ===
                SectionType.SIMPLEINTENTSECTION &&
                sections[index + 1].IntentNameLine.includes('##')))
          ) {
            if (
              sections[index + 1].SectionType === SectionType.ENTITYSECTION ||
              sections[index + 1].SectionType === SectionType.NEWENTITYSECTION
            ) {
              simpleIntentSections[
                simpleIntentSections.length - 1
              ].Entities.push(sections[index + 1]);
              simpleIntentSections[simpleIntentSections.length - 1].Errors.push(
                ...sections[index + 1].Errors
              );
            } else {
  ...
  static extractSectionBody(sections, content) {
    ...
    sections.forEach(function (section, index) {
      if (
        section.SectionType === SectionType.SIMPLEINTENTSECTION ||
        ...
        if (section.SectionType === SectionType.NESTEDINTENTSECTION) {
          LUParser.extractSectionBody(
            ...
            originList.slice(0, stopLine).join(NEWLINE)
          );
        }
      }
    });
  ...
  static isSectionEnabled(sections) {
    let modelInfoSections = sections.filter(
