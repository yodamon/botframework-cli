...
class Visitor {
    ...
    for (const node of ctx.children) {
      ...
      switch (innerNode.symbol.type) {
        ...
        case lp.EXPRESSION: {
          ...
          utterance = this.recurselyResolveTokenizedUtterance(
            tokUtt,
            entities,
            errorMsgs,
            utterance.trimLeft()
   ...
   */
  static recurselyResolveTokenizedUtterance(
    tokUtt,
    entities,
    errorMsgs,
    srcUtterance
    ...
    for (const item of tokUtt) {
      if (item === Object(item)) {
        ...
        if (
          ...
          errorMsgs.push(
            `Invalid utterance line, entity name ${entityName} cannot contain any of the following characters: [<, >, *, %, &, :, \\, $]`
          );
          continue;
        }
        ...
        if (item.entityValue === undefined) {
          ...
          const patternStr = item.role
            ? `{${item.entityName}:${item.role}}`
            : `{${item.entityName}}`;
          ...
          if (item.entityValue === undefined) {
            errorMsgs.push(
              `Composite entity "${item.parent.entityName}" includes pattern.any entity "${item.entityName}".\r\n\tComposites cannot include pattern.any entity as a child.`
            );
          } else {
            srcUtterance = this.recurselyResolveTokenizedUtterance(
              item.entityValue,
              entities,
              errorMsgs,
              srcUtterance
    ...
    exp.split('').forEach((char) => {
      switch (char) {
        ...
        case '}':
          ...
          curList =
            curEntity != undefined ? curEntity.entityValue : splitString;
          entityValueCapture = false;
          entityRoleCapture = false;
