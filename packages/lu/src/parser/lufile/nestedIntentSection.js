 ...
 */
const NestedIntentSectionContext = require('./generated/LUFileParser')
  .LUFileParser.NestedIntentSectionContext;
...
class NestedIntentSection extends BaseSection {
  ...
  constructor(parseTree, content) {
    ...
    this.Body = '';
    this.SimpleIntentSections = this.ExtractSimpleIntentSections(
      parseTree,
      content
    );
    this.Errors = [];
  ...
  ExtractSimpleIntentSections(parseTree, content) {
    ...
    for (const subIntentDefinition of parseTree
      .nestedIntentBodyDefinition()
      .subIntentDefinition()) {
      let simpleIntentSection = new SimpleIntentSection(
        subIntentDefinition.simpleIntentSection(),
        content
      );
