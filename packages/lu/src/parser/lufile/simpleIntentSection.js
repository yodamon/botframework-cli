 ...
 */
const SimpleIntentSectionContext = require('./generated/LUFileParser')
...
class SimpleIntentSection extends BaseSection {
  ...
  ExtractUtteranceAndEntitiesMap(parseTree) {
    ...
    let errors = [];
    if (
      parseTree.intentDefinition().intentBody() &&
      parseTree.intentDefinition().intentBody().normalIntentBody()
    ) {
      for (const errorIntentStr of parseTree
        .intentDefinition()
        .intentBody()
        .normalIntentBody()
        .errorString()) {
      ...
      for (const normalIntentStr of parseTree
        .intentDefinition()
        .intentBody()
        .normalIntentBody()
        .normalIntentString()) {
        ...
        try {
          utteranceAndEntities = visitor.visitNormalIntentStringContext(
            normalIntentStr
          );
        } catch (err) {
          errors.push(
            BuildDiagnostic({
              message:
                "Invalid utterance definition found. Did you miss a '{' or '}'?",
        ...
        if (utteranceAndEntities !== undefined) {
          ...
          const startPosition = new Position(
            normalIntentStr.start.line,
            normalIntentStr.start.column
          );
          const stopPosition = new Position(
            normalIntentStr.stop.line,
            normalIntentStr.stop.column + normalIntentStr.stop.text.length
          );
          utteranceAndEntities.range = new Range(startPosition, stopPosition);

          utteranceAndEntitiesMap.push(utteranceAndEntities);
          utteranceAndEntities.errorMsgs.forEach((errorMsg) =>
            errors.push(
              BuildDiagnostic({
                message: errorMsg,
                context: normalIntentStr,
