 ...
 */
const QnaSectionContext = require('./generated/LUFileParser').LUFileParser
  .QnaSectionContext;
...
class QnaSection extends BaseSection {
  ...
  ExtractSourceInfo(parseTree) {
    ...
    if (srcAssignment) {
      let srcRegExp = /^[ ]*\>[ ]*!#[ ]*@qna.pair.source[ ]*=[ ]*(?<sourceInfo>.*?)$/gim;
  ...
  ExtractAssignedId(parseTree) {
    ...
    if (idAssignment) {
      let idTextRegExp = /^\<a[ ]*id[ ]*=[ ]*[\"\'](?<idCaptured>.*?)[\"\'][ ]*>[ ]*\<\/a\>$/gim;
  ...
  ExtractPrompts(parseTree) {
      ...
      for (const errorFilterLineStr of promptSection.errorFilterLine()) {
        if (errorFilterLineStr.getText().trim() !== '') {
          errors.push(
            BuildDiagnostic({
              message:
                "Invalid QnA prompt line, expecting '-' prefix for each line.",
    ...
    for (const promptLine of promptSection.filterLine()) {
      ...
      let promptConfigurationRegExp = /^\[(?<displayText>.*?)]\([ ]*\#[ ]*[ ?]*(?<linkedQuestion>.*?)\)[ ]*(?<contextOnly>\`context-only\`)?.*?$/gim;
      let splitLine = promptConfigurationRegExp.exec(filterLineText);
      if (!splitLine) {
        errors.push(
          BuildDiagnostic({
            message:
              'Invalid QnA prompt definition. Unable to parse prompt. Please verify syntax as well as question link`.',
    ...
    for (const errorQuestionStr of questionsBody.errorQuestionString()) {
      if (errorQuestionStr.getText().trim() !== '') {
        errors.push(
          BuildDiagnostic({
            message:
              "Invalid QnA question line, did you miss '-' at line begin",
        ...
        for (const errorFilterLineStr of filterSection.errorFilterLine()) {
          if (errorFilterLineStr.getText().trim() !== '') {
            errors.push(
              BuildDiagnostic({
                message:
                  "Invalid QnA filter line, did you miss '-' at line begin",
                context: errorFilterLineStr,
              })
            );
          }
