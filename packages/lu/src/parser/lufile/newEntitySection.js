 ...
 */
const NewEntitySectionContext = require('./generated/LUFileParser').LUFileParser
  .NewEntitySectionContext;
const DiagnosticSeverity = require('./diagnostic').DiagnosticSeverity;
const BuildDiagnostic = require('./diagnostic').BuildDiagnostic;
const LUSectionTypes = require('./../utils/enums/lusectiontypes');
const InvalidCharsInIntentOrEntityName = require('./../utils/enums/invalidchars')
...
class NewEntitySection extends BaseSection {
  ...
  constructor(parseTree) {
    ...
    const startPosition = new Position(
      parseTree.start.line,
      parseTree.start.column
    );
    const stopPosition = new Position(
      parseTree.stop.line,
      parseTree.stop.column + parseTree.stop.text.length
    );
  ...
  ExtractName(parseTree) {
    ...
    if (parseTree.newEntityDefinition().newEntityLine().newEntityName()) {
      ...
      parseTree.newEntityDefinition().newEntityLine().newEntityNameWithWS()
    ) {
      entityName = parseTree
        .newEntityDefinition()
        .newEntityLine()
    ...
    if (
      entityName &&
      InvalidCharsInIntentOrEntityName.some((x) => entityName.includes(x))
    ) {
      this.Errors.push(
        BuildDiagnostic({
  ...
  ExtractFeatures(parseTree) {
    if (
      parseTree.newEntityDefinition().newEntityLine().newEntityUsesFeatures()
    ) {
      return parseTree
        .newEntityDefinition()
        .newEntityLine()
  ...
  ExtractCompositeDefinition(parseTree) {
    if (
      parseTree.newEntityDefinition().newEntityLine().newCompositeDefinition()
    ) {
      return parseTree
        .newEntityDefinition()
        .newEntityLine()
  ...
  ExtractSynonymsOrPhraseList(parseTree) {
    ...
    if (parseTree.newEntityDefinition().newEntityListbody()) {
      for (const errorItemStr of parseTree
        .newEntityDefinition()
        .newEntityListbody()
        .errorString()) {
        if (errorItemStr.getText().trim() !== '') {
          this.Errors.push(
            BuildDiagnostic({
              message:
                "Invalid list entity line, did you miss '-' at line begin",
      ...
      for (const normalItemStr of parseTree
        .newEntityDefinition()
        .newEntityListbody()
        .normalItemString()) {
    ...
    if (
      this.Type &&
      this.Type.indexOf('=') > -1 &&
      synonymsOrPhraseList.length === 0
    ) {
      let errorMsg = `no synonyms list found for list entity definition: "${parseTree
        .newEntityDefinition()
        .entityLine()
        .getText()}"`;
