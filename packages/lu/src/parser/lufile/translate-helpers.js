...
const translateHelpers = {
  ...
  parseAndTranslate: async function (
    ...
    let batch_translate_size = batch_translate
      ? parseInt(batch_translate)
      : MAX_TRANSLATE_BATCH_SIZE;
    ...
    for (let lineIndex in linesInFile) {
      ...
      if (currentLine.indexOf(PARSERCONSTS.COMMENT) === 0) {
          ...
          case PARSERCONSTS.INTENT:
            ...
            for (let i = 0; i < content.length; i++) {
              ...
              if (!skipChars.includes(content.charAt(i))) {
                for (
                  let j = i;
                  j < content.length && !skipChars.includes(content.charAt(j));
                  j++
                ) {
        ...
        if (inAnswer) {
          ...
          throw new exception(
            ...
            'Error: Unexpected line encountered when parsing \n' +
              '[' +
              lineIndex +
              ']:' +
              currentLine
      ...
      if (
        ...
        try {
          localizedContent += await batchTranslateText(
            linesToTranslate,
            subscriptionKey,
            to_lang,
            src_lang,
  ...
  translateText: async function (text, subscriptionKey, to_lang, from_lang) {
    ...
    if (!res.ok) {
      throw new exception(
        retCode.errorCode.TRANSLATE_SERVICE_FAIL,
 ...
 * @throws {exception} Throws on errors. exception object includes errCode and text.
 */
const batchTranslateText = async function (
  linesToTranslate,
  subscriptionKey,
  to_lang,
  src_lang,
 ...
 * @returns {void}
 */
const translateAndMap = async function (
  batchRequest,
  subscriptionKey,
  to_lang,
  src_lang,
  linesToTranslateCopy
) {
  if (batchRequest.length === 0) return;
  let data;
  data = await translateHelpers.translateText(
    batchRequest,
    subscriptionKey,
    to_lang,
    src_lang
