  ...
  validateResource: function (originalResource, config) {
    ...
    if (
      resource.Errors.filter(
        (error) =>
          error && error.Severity && error.Severity === DiagnosticSeverity.ERROR
      ).length > 0
    ) {
      return [];
    }
    ...
    try {
      ...
      let enableMergeIntents = parseAndHandleModelInfoSection(
        parsedContent,
        resource,
        false,
        config
      ...
      let featuresToProcess = parseAndHandleEntityV2(
        parsedContent,
        resource,
        false,
        undefined,
 ...
 * @throws {exception} Throws on errors. exception object includes errCode and text.
 */
const parseLuAndQnaWithAntlr = async function (
  parsedContent,
  fileContent,
  log,
  locale,
  ...
  if (luResource.Errors && luResource.Errors.length > 0) {
    if (log) {
      var warns = luResource.Errors.filter(
        (error) =>
          error && error.Severity && error.Severity === DiagnosticSeverity.WARN
      );
      if (warns.length > 0) {
        process.stdout.write(
          warns
            .map((warn) => warn.toString())
            .join('\n')
            .concat('\n')
        );
      }
    ...
    var errors = luResource.Errors.filter(
      (error) =>
        error && error.Severity && error.Severity === DiagnosticSeverity.ERROR
    );
    if (errors.length > 0) {
      throw new exception(
        retCode.errorCode.INVALID_LINE,
  ...
  validateNDepthEntities(
    parsedContent.LUISJsonStructure.entities,
    parsedContent.LUISJsonStructure.flatListOfEntityAndRoles,
    parsedContent.LUISJsonStructure.intents
  );
...
const updateIntentAndEntityFeatures = function (luisObj) {
  let plForAll = luisObj.model_features.filter(
    (item) =>
      item.enabledForAllModels == undefined || item.enabledForAllModels == true
  );
  plForAll.forEach((pl) => {
...
const updateFeaturesWithPL = function (collection, plName) {
  ...
  if (collection.hasOwnProperty('features')) {
    if (
      ...
      collection.features.find((i) => i.featureName == plName) === undefined
    ) {
      collection.features.push(
        new helperClass.featureToModel(
          plName,
          featureProperties.phraseListFeature
        )
      );
    }
  } else {
...
const updateModelBasedOnNDepthEntities = function (utterances, entities) {
  ...
  let utterancesWithLabels = utterances.filter(
    (utterance) => utterance.entities && utterance.entities.length !== 0
  );
  utterancesWithLabels.forEach((utterance) => {
    utterance.entities.forEach((entityInUtterance) => {
      ...
      entityFoundInMaster.forEach((entityInMaster) => {
        let splitPath = entityInMaster.path
          .split('/')
          .filter((item) => item.trim() !== '');
        if (
          entityFoundInMaster.length > 1 &&
          splitPath.length === 0 &&
          (!entityInMaster.entityRoot.children ||
            entityInMaster.entityRoot.children.length === 0)
        ) {
          ...
          if (isParentLabelled === false) {
            ...
            rSplitPath.forEach((parent) => {
              ...
              let parentLabelled = utterance.entities.find(
                (entityUtt) => entityUtt.entity == parent
              );
              if (!parentLabelled) {
                ...
                throw new exception(
                  retCode.errorCode.INVALID_INPUT,
                  error.toString(),
                  [error]
                );
              } else {
...
const validateNDepthEntities = function (
  ...
  (collection || []).forEach((child) => {
    if (child.instanceOf) {
      let baseEntityFound = entitiesAndRoles.find(
        (i) => i.name == child.instanceOf
      );
      if (!baseEntityFound) {
      ...
      if (
        ![
          EntityTypeEnum.LIST,
          EntityTypeEnum.REGEX,
          EntityTypeEnum.PREBUILT,
          EntityTypeEnum.ML,
        ].includes(baseEntityFound.type)
      ) {
    ...
    if (child.features) {
      ...
      (child.features || []).forEach((feature, idx) => {
        ...
        let featureExists = entitiesAndRoles.find(
          (i) => i.name == feature || i.name == `${feature}(interchangeable)`
        );
        if (featureExists) {
          // is feature phrase list?
          if (featureExists.type == EntityTypeEnum.PHRASELIST) {
            child.features[idx] = new helperClass.featureToModel(
              feature,
              featureProperties.phraseListFeature
            );
            featureHandled = true;
            ...
            child.features[idx] = new helperClass.modelToFeature(
              feature,
              featureProperties.entityFeatureToModel[featureExists.type]
            );
            featureHandled = true;
          }
        }
        if (!featureHandled) {
          ...
          let intentFeatureExists = intentsCollection.find(
            (i) => i.name == feature
          );
          if (intentFeatureExists) {
            child.features[idx] = new helperClass.modelToFeature(
              feature,
              featureProperties.intentFeatureToModel
            );
            featureHandled = true;
          } else {
 ...
 * @param {Range} range
 */
const validateFeatureAssignment = function (
  srcItemType,
  srcItemName,
  tgtFeatureType,
  tgtFeatureName,
...
const addFeatures = function (
  ...
  let featureAlreadyDefined = (tgtItem.features || []).find(
    (item) => item.modelName == feature || item.featureName == feature
  );
  switch (featureType) {
    case featureTypeEnum.featureToModel: {
      if (tgtItem.features) {
        if (!featureAlreadyDefined)
          tgtItem.features.push(
            new helperClass.featureToModel(feature, featureProperties)
          );
      } else {
        tgtItem.features = new Array(
          new helperClass.featureToModel(feature, featureProperties)
        );
      }
      break;
    ...
    case featureTypeEnum.modelToFeature: {
      if (tgtItem.features) {
        if (!featureAlreadyDefined)
          tgtItem.features.push(
            new helperClass.modelToFeature(feature, featureProperties)
          );
      } else {
        tgtItem.features = new Array(
          new helperClass.modelToFeature(feature, featureProperties)
...
const parseFeatureSections = function (
  ...
  if (!config.enableFeatures) {
    const error = BuildDiagnostic({
      message:
        'Do not support Features. Please make sure enableFeatures is set to true.',
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
    ]);
  ...
  (featuresToProcess || []).forEach((section) => {
    if (section.Type === INTENTTYPE) {
      ...
      let intentExists = parsedContent.LUISJsonStructure.intents.find(
        (item) => item.name === section.Name
      );
      if (intentExists !== undefined) {
        // verify the list of features requested have all been defined.
        let featuresList = section.Features.split(/[,;]/g).map((item) =>
          item.trim().replace(/^[\'\"]|[\'\"]$/g, '')
        );
        (featuresList || []).forEach((feature) => {
          let entityExists = (
            ...
            (item) =>
              item.name == feature || item.name == `${feature}(interchangeable)`
          );
          let featureIntentExists = (
            parsedContent.LUISJsonStructure.intents || []
          ...
          if (entityExists) {
            if (entityExists.type === EntityTypeEnum.PHRASELIST) {
              ...
              addFeatures(
                intentExists,
                feature,
                featureTypeEnum.featureToModel,
                section.Range,
                featureProperties.phraseListFeature
              );
              // set enabledForAllModels on this phrase list
              let plEnity = parsedContent.LUISJsonStructure.model_features.find(
                (item) => item.name == feature
              );
              if (plEnity.enabledForAllModels === undefined)
              ...
              addFeatures(
                intentExists,
                feature,
                featureTypeEnum.modelToFeature,
                section.Range,
                featureProperties.entityFeatureToModel[entityExists.type]
              );
            }
      ...
      if (section.Features) {
        ...
        let srcEntityInFlatList = (
          parsedContent.LUISJsonStructure.flatListOfEntityAndRoles || []
        ).find((item) => item.name == section.Name);
        let entityType = srcEntityInFlatList
          ? srcEntityInFlatList.type
          : undefined;
        (featuresList || []).forEach((feature) => {
          ...
          let srcEntity = (
            parsedContent.LUISJsonStructure[luisEntityTypeMap[entityType]] || []
          ).find((item) => item.name == section.Name);
          if (featureExists) {
            if (featureExists.type === EntityTypeEnum.PHRASELIST) {
              // de-dupe and add features to intent.
  ...
  (parsedContent.LUISJsonStructure[type] || []).forEach((itemOfType) => {
    ...
    if (itemOfType.features) {
      (itemOfType.features || []).forEach((feature) => {
        ...
        let featureDependencyEx = dependencyList.filter(
          (item) =>
            srcName == (item.value ? item.value.slice(-1)[0] : undefined)
        );
        ...
        featureDependencyEx = dependencyList.find(
          (item) => feature == (item.value ? item.value.slice(0)[0] : undefined)
        );
        if (featureDependencyEx) {
          copySrc = featureDependencyEx.key.split('::')[1];
          copyValue = featureDependencyEx.value.slice(1);
        }
        let dependencyExists = dependencyList.find(
          (item) => item.key == `${srcName}::${feature}`
        );
        if (!dependencyExists) {
          let lKey = copySrc
            ? `${srcName}::${copySrc}`
          ...
          dependencyExists.key = `${
            dependencyExists.key.split('::')[0]
          }::${feature}`;
          dependencyExists.value.push(feature);
        }
        let circularItemFound = dependencyList.find(
          (item) =>
            item.value && item.value.slice(0)[0] == item.value.slice(-1)[0]
        );
...
const verifyNoCircularDependencyForFeatures = function (parsedContent) {
  ...
  updateDependencyList(
    LUISObjNameEnum.CLOSEDLISTS,
    parsedContent,
    dependencyList
  );
  updateDependencyList(
    LUISObjNameEnum.COMPOSITES,
    parsedContent,
    dependencyList
  );
  updateDependencyList(
    LUISObjNameEnum.PATTERNANYENTITY,
    parsedContent,
    dependencyList
  );
 ...
 * @throws {exception} Throws on errors. exception object includes errCode and text.
 */
const parseAndHandleImportSection = async function (
  parsedContent,
  luResource,
  config
) {
  // handle reference
  let luImports = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.IMPORTSECTION
  );
  if (luImports && luImports.length > 0) {
    if (!config.enableExternalReferences) {
      const error = BuildDiagnostic({
        message:
          'Do not support External References. Please make sure enableExternalReferences is set to true.',
      });
      throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
        error,
      ]);
    ...
    let references = luResource.Sections.filter(
      (s) => s.SectionType === SectionType.REFERENCESECTION
    );
    for (const luImport of luImports) {
      ...
      if (parseUrl.host || parseUrl.hostname) {
        ...
        if (!contentType.includes('text/html')) {
          if (
            parseUrl.pathname.toLowerCase().endsWith('.lu') ||
            parseUrl.pathname.toLowerCase().endsWith('.qna')
          ) {
            parsedContent.additionalFilesToParse.push(
              new fileToParse(linkValue)
            );
          } else {
            parsedContent.qnaJsonStructure.files.push(
              new qnaFile(linkValue, linkValueText)
            );
          }
        } else {
        ...
        if (parseInt(linkValue, 10).toString() === linkValue) {
          let foundReference = references.find(
            (refer) => refer.ReferenceId === linkValue
          );
          if (foundReference) {
 ...
 */
const handleAtForPattern = function (
  utterance,
  entitiesFound,
  flatEntityAndRoles
) {
...
const handleAtPrefix = function (entity, flatEntityAndRoles) {
  if (entity.entity.match(/^@/g)) {
    ...
    if (flatEntityAndRoles) {
      ...
      let entityMatch = flatEntityAndRoles.find(
        (item) => item.entityName == entity.entity
      );
      if (entityMatch !== undefined) {
        return entity;
      }
      // find the entity as a match by role
      let roleMatch = flatEntityAndRoles.find((item) =>
        item.roles.includes(entity.entity)
      );
 ...
 * @throws {exception} Throws on errors. exception object includes errCode and text.
 */
const parseAndHandleNestedIntentSection = function (
  luResource,
  enableMergeIntents
) {
  // handle nested intent section
  let sections = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.NESTEDINTENTSECTION
  );
    ...
    sections.forEach((section) => {
      if (enableMergeIntents) {
        ...
        for (let idx = 1; idx < section.SimpleIntentSections.length; idx++) {
          mergedIntentSection.UtteranceAndEntitiesMap = mergedIntentSection.UtteranceAndEntitiesMap.concat(
            section.SimpleIntentSections[idx].UtteranceAndEntitiesMap
          );
          mergedIntentSection.Entities = mergedIntentSection.Entities.concat(
            section.SimpleIntentSections[idx].Entities
          );
        }
...
const parseAndHandleSimpleIntentSection = function (
  ...
  // handle intent
  let intents = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.SIMPLEINTENTSECTION
  );
  let hashTable = {};
  if (intents && intents.length > 0) {
    ...
    for (const intent of intents) {
      ...
      addItemIfNotPresent(
        parsedContent.LUISJsonStructure,
        LUISObjNameEnum.INTENT,
        intentName
      );
        ...
        if (helpers.isUtteranceLinkRef(utterance || '')) {
          if (utterance.endsWith(']')) {
            ...
            let reference = references.find(
              (refer) => refer.ReferenceId === referenceId
            );
            if (!reference) {
          ...
          parsedContent.additionalFilesToParse.push(
            new fileToParse(parsedLinkUriInUtterance.fileName, false)
          );
        }

        if (utteranceAndEntities.entities.length > 0) {
          let entitiesFound = utteranceAndEntities.entities;
          let havePatternAnyEntity = entitiesFound.find(
            (item) => item.type == LUISObjNameEnum.PATTERNANYENTITY
          );
          if (havePatternAnyEntity !== undefined) {
            if (!config.enablePattern) {
              const error = BuildDiagnostic({
                message:
                  'Do not support Pattern. Please make sure enablePattern is set to true.',
                range: utteranceAndEntities.range,
              });
              throw new exception(
                retCode.errorCode.INVALID_INPUT,
            ...
            utterance = handleAtForPattern(
              utterance,
              entitiesFound,
              parsedContent.LUISJsonStructure.flatListOfEntityAndRoles
            );
            let mixedEntity = entitiesFound.filter(
              (item) => item.type != LUISObjNameEnum.PATTERNANYENTITY
            );
            ...
            let prebuiltEntities = entitiesFound.filter((item) =>
              builtInTypes.consolidatedList.includes(item.entity)
            );
            prebuiltEntities.forEach((prebuiltEntity) => {
              if (
                parsedContent.LUISJsonStructure.prebuiltEntities.findIndex(
                  (e) => e.name === prebuiltEntity.entity
                ) < 0
              ) {
            ...
            entitiesFound.forEach((entity) => {
              let simpleEntityInMaster = parsedContent.LUISJsonStructure.entities.find(
                (item) => item.name == entity.entity
              );
              if (simpleEntityInMaster && entity.role) {
                addItemOrRoleIfNotPresent(
                  parsedContent.LUISJsonStructure,
                  LUISObjNameEnum.ENTITIES,
                  entity.entity,
                  [entity.role.trim()]
                );
              }
              let compositeInMaster = parsedContent.LUISJsonStructure.composites.find(
                (item) => item.name == entity.entity
              );
              ...
              let listEntityInMaster = parsedContent.LUISJsonStructure.closedLists.find(
                (item) => item.name == entity.entity
              );
              ...
              let regexEntityInMaster = parsedContent.LUISJsonStructure.regex_entities.find(
                (item) => item.name == entity.entity
              );
              if (regexEntityInMaster && entity.role) {
                addItemOrRoleIfNotPresent(
                  parsedContent.LUISJsonStructure,
                  LUISObjNameEnum.REGEX,
                  entity.entity,
                  [entity.role.trim()]
                );
              }
              let prebuiltInMaster = parsedContent.LUISJsonStructure.prebuiltEntities.find(
                (item) => item.name == entity.entity
              );
              if (prebuiltInMaster && entity.role) {
                addItemOrRoleIfNotPresent(
                  parsedContent.LUISJsonStructure,
                  LUISObjNameEnum.PREBUILT,
                  entity.entity,
                  [entity.role.trim()]
                );
              }
              if (
                !simpleEntityInMaster &&
                !compositeInMaster &&
                !listEntityInMaster &&
                !regexEntityInMaster &&
                !prebuiltInMaster
            ...
            entitiesFound.forEach((entity) => {
              ...
              let patternAnyExists = (
                parsedContent.LUISJsonStructure.patternAnyEntities || []
              ).find((item) => item.name == entity.entity);
              if (
                compositeExists === undefined &&
                listExists === undefined &&
                prebuiltExists === undefined &&
                regexExists === undefined &&
                patternAnyExists === undefined
              ) {
                if (!config.enableMLEntities) {
                  const error = BuildDiagnostic({
                    message:
                      'Do not support ML entity. Please make sure enableMLEntities is set to true.',
                    range: utteranceAndEntities.range,
                  });
                  throw new exception(
                    retCode.errorCode.INVALID_INPUT,
                ...
                if (compositeExists !== undefined) {
                  ...
                  if (entity.value != '') {
                    ...
                    if (patternAnyIdx > -1)
                      parsedContent.LUISJsonStructure.patternAnyEntities.splice(
                        patternAnyIdx,
                        1
                      );
                  } else if (entity.role) {
            ...
            if (hashTable[uttHash]) {
              ...
              utteranceObject = new helperClass.uttereances(
                utterance,
                intentName,
                []
              );
            ...
            entitiesFound.forEach((item) => {
              if (item.startPos > item.endPos) {
                ...
                throw new exception(
                  retCode.errorCode.MISSING_LABELLED_VALUE,
                  error.toString(),
                  [error]
                );
              }

              let utteranceEntity = new helperClass.utteranceEntity(
                item.entity,
                item.startPos,
                item.endPos
              );
              ...
              let priorLabelFound = utteranceObject.entities.find((item) => {
                if (item.entity === utteranceEntity.entity) {
                  if (
                    utteranceEntity.role === undefined ||
                    utteranceEntity.role === ''
                  ) {
                    if (item.role === undefined || item.role === '') {
                      // do the labels overlap?
                      if (
                        (item.startPos >= utteranceEntity.startPos &&
                          item.endPos <= utteranceEntity.endPos) ||
                        (utteranceEntity.startPos >= item.startPos &&
                          utteranceEntity.endPos <= item.endPos)
                      ) {
                        return true;
                      }
                    }
              ...
              if (priorLabelFound === undefined) {
                ...
                if (
                  !utteranceObject.entities.find((item) =>
                    deepEqual(item, utteranceEntity)
                  )
                ) {
                  let overlapHandling =
                    parsedContent.LUISJsonStructure.onAmbiguousLabels ||
                    'takeLongestLabel';
                  switch (overlapHandling.toLowerCase()) {
                    ...
                    case 'takelast':
                      ...
                      break;
                    case 'throwanerror':
                      let oldUtterance = expandUtterance(
                        utterance,
                        priorLabelFound
                      );
                      let newUtterance = expandUtterance(
                        utterance,
                        utteranceEntity
                      );
                    ...
                    default:
                      ...
                      if (
                        utteranceEntity.startPos >= priorLabelFound.startPos &&
                        utteranceEntity.endPos >= priorLabelFound.endPos
                      ) {
                        priorLabelFound.startPos = utteranceEntity.startPos;
                        priorLabelFound.endPos = utteranceEntity.endPos;
                      }
          ...
          if (helpers.isUtterancePattern(utterance)) {
            ...
            if (!hashTable[uttHash]) {
              let utteranceObject = new helperClass.uttereances(
                utterance,
                intentName,
                []
              );
...
const expandUtterance = function (utterance, entity) {
  ...
  utteranceSplit[entity.startPos] = `{@${entity.entity}=${
    utteranceSplit[entity.startPos]
  }`;
...
const isChildEntity = function (entity, entitiesFound) {
  ...
  return (entitiesFound || []).find(
    (item) =>
      item.entity !== entity.entity &&
      entity.startPos >= item.startPos &&
      entity.endPos <= item.endPos
  ) === undefined
    ? false
    : true;
};
...
const getEntityType = function (entityName, entities) {
  let entityFound = (entities || []).find(
    (item) =>
      item.Name == entityName || item.Name == `${entityName}(interchangeable)`
  );
  if (entityFound && entityFound.Type !== undefined) return entityFound.Type;
  // see if this one of the prebuilt type
  if (builtInTypes.consolidatedList.includes(entityName))
    return EntityTypeEnum.PREBUILT;
  return undefined;
};
 ...
 */
const validateAndGetRoles = function (
  parsedContent,
  roles,
  range,
  entityName,
  ...
  let newRoles = roles
    ? roles.split(',').map((item) => item.replace(/[\'\"]/g, '').trim())
    : [];
  // de-dupe roles
  ...
  if (parsedContent.LUISJsonStructure.flatListOfEntityAndRoles) {
    ...
    newRoles.forEach((role) => {
      let roleFound = parsedContent.LUISJsonStructure.flatListOfEntityAndRoles.find(
        (item) => item.roles.includes(role) || item.name === role
      );
      if (roleFound !== undefined) {
        ...
        let hasBadNonPLRoles = (roleFound.roles || []).filter(
          (item) =>
            item.toLowerCase() !== PLCONSTS.INTERCHANGEABLE &&
            item.toLowerCase() !== PLCONSTS.ENABLEDFORALLMODELS &&
            item.toLowerCase() !== PLCONSTS.DISABLED &&
            item.toLowerCase() !== PLCONSTS.DISABLEDFORALLMODELS
        );
        if (hasBadNonPLRoles.length !== 0) {
          let errorMsg = `Roles must be unique across entity types. Invalid role definition found "${entityName}". Prior definition - '@ ${
            roleFound.type
          } ${roleFound.name}${
            roleFound.roles.length > 0
              ? ` hasRoles ${roleFound.roles.join(',')}`
              : ``
          }'`;
          let error = BuildDiagnostic({
            message: errorMsg,
    ...
    let oldEntity = parsedContent.LUISJsonStructure.flatListOfEntityAndRoles.find(
      (item) => item.name === entityName && item.type === entityType
    );
    if (oldEntity !== undefined) {
      oldEntity.addRoles(newRoles);
    } else {
      parsedContent.LUISJsonStructure.flatListOfEntityAndRoles.push(
        new helperClass.entityAndRoles(entityName, entityType, newRoles)
      );
    }
  } else {
...
const parseAndHandleEntityV2 = function (
  ...
  let entities = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.NEWENTITYSECTION
  );
      ...
      if (entity.Type !== INTENTTYPE) {
        if (entity.Name.endsWith(PLCONSTS.INTERCHANGEABLE)) {
          entity.Name =
            entity.Name.slice(
              0,
              entity.Name.length - PLCONSTS.INTERCHANGEABLE.length
            )
              .trim()
              .replace(/^[\'\"]|[\'\"]$/g, '') + PLCONSTS.INTERCHANGEABLE;
        } else {
          entity.Name = entity.Name.replace(/^[\'\"]|[\'\"]$/g, '');
        }

        let entityName = entity.Name.endsWith('=')
          ? entity.Name.slice(0, entity.Name.length - 1)
          : entity.Name;
        let entityType = !entity.Type
          ? getEntityType(entity.Name, entities)
          : entity.Type;
        if (!entityType) {
          let errorMsg = `No type definition found for entity "${entityName}". Supported types are ${Object.values(
            EntityTypeEnum
          ).join(', ')}. Note: Type names are case sensitive.`;
          let error = BuildDiagnostic({
            message: errorMsg,
            range: entity.Range,
          });
        ...
        let entityRoles = validateAndGetRoles(
          parsedContent,
          entity.Roles,
          entity.Range,
          entityName,
        ...
        let PAEntityRoles = RemoveDuplicatePatternAnyEntity(
          parsedContent,
          entityName,
          entityType,
          entity.Range
        ...
        switch (entityType) {
          ...
          case EntityTypeEnum.COMPOSITE:
            ...
            if (entity.CompositeDefinition) {
              entity.CompositeDefinition.replace(/[\[\]]/g, '')
                .split(/[,;]/g)
                .map((item) => item.trim())
                .forEach((item) => candidateChildren.push(item));
            }
            if (entity.ListBody) {
          ...
          case EntityTypeEnum.LIST:
            handleClosedList(
              parsedContent,
              entityName,
              entity.ListBody.map((item) => item.trim()),
              entityRoles,
              entity.Range,
              config
            );
          ...
          case EntityTypeEnum.REGEX:
            if (entity.ListBody[0]) {
              handleRegExEntity(
                parsedContent,
                entityName,
                entity.ListBody[0].trim().substr(1).trim(),
                entityRoles,
                entity.Range,
                config
              );
          ...
          case EntityTypeEnum.PHRASELIST:
            handlePhraseList(
              ...
              entity.Type,
              entityRoles,
              entity.ListBody.map((item) => item.trim().substr(1).trim()),
              entity.Range,
              config
            );
          default:
 ...
 * @param {Object} range
 */
const handleNDepthEntity = function (
  parsedContent,
  entityName,
  entityRoles,
  entityLines,
  ...
  let rootEntity = parsedContent.LUISJsonStructure.entities.find(
  ...
  entityLines.forEach((child) => {
    ...
    let childFeatures = groupsFound.groups.features
      ? groupsFound.groups.features
          .trim()
          .split(/[,;]/g)
          .map((item) => item.trim())
      : undefined;
    ...
    let tabLevel =
      Math.ceil(
        groupsFound.groups.leadingSpaces !== undefined
          ? groupsFound.groups.leadingSpaces.length / SPACEASTABS
          : 0
      ) ||
      (groupsFound.groups.leadingTabs !== undefined
        ? groupsFound.groups.leadingTabs.length
        : 0);
    ...
    currentParentEntity = entityIdxByLevel
      .reverse()
      .find((item) => item.level == tabLevel - baseTabLevel);
    entityIdxByLevel.reverse();
    if (!currentParentEntity) {
      let errorMsg = `[ERROR] line ${defLine}: Invalid definition found for child "${child.trim()}". Parent of each child entity must be of type "${
        EntityTypeEnum.ML
      }".`;
      const error = BuildDiagnostic({
        message: errorMsg,
    ...
    if (
      groupsFound.groups.instanceOf.toLowerCase().trim() ===
      EntityTypeEnum.SIMPLE
    ) {
      pushNDepthChild(
      ...
      groupsFound.groups.instanceOf.toLowerCase().trim() === EntityTypeEnum.ML
    ) {
      pushNDepthChild(
        currentParentEntity.entity,
        childEntityName,
      ...
      let newParent = currentParentEntity.entity.children.find(
        (item) => item.name == childEntityName
      );
      // Push the ID of the parent since we are proessing the first child entity
      entityIdxByLevel.push({
        level: tabLevel - baseTabLevel + 1,
        entity: newParent,
      });
    } else {
      pushNDepthChild(
 ...
 */
const pushNDepthChild = function (
  entity,
  childEntityName,
  context,
  childFeatures,
  ...
  if (!entity.children) {
    ...
    let childExists = (entity.children || []).find(
      (item) => item.name == childEntityName
    );
    if (!childExists) {
 ...
 */
const verifyUniqueEntityName = function (
  parsedContent,
  entityName,
  entityType,
  range,
  ...
  // Entity name cannot be same as a role name
  let matchType = '';
  let entityFound = parsedContent.LUISJsonStructure.flatListOfEntityAndRoles.find(
    (item) => {
  ...
  if (entityFound !== undefined) {
    let errorMsg = `${matchType} Prior definition - '@ ${entityFound.type} ${
      entityFound.name
    }${
      entityFound.roles.length > 0
        ? ` hasRoles ${entityFound.roles.join(',')}`
        : ``
    }'`;
    let error = BuildDiagnostic({
      message: errorMsg,
 ...
 */
const handlePatternAny = function (
  parsedContent,
  entityName,
  entityRoles,
  range,
  ...
  try {
    let rolesImport = VerifyAndUpdateSimpleEntityCollection(
      parsedContent,
      entityName,
      'Pattern.Any'
    );
    if (rolesImport.length !== 0) {
      rolesImport.forEach((role) =>
        !entityRoles.includes(role) ? entityRoles.push(role) : undefined
      );
    }
  } catch (err) {
    throw err;
  ...
  let PAExists = parsedContent.LUISJsonStructure.patternAnyEntities.find(
    (item) => item.name == entityName
  );
  if (PAExists === undefined) {
    parsedContent.LUISJsonStructure.patternAnyEntities.push(
      new helperClass.patternAnyEntity(entityName, [], entityRoles)
    );
  } else {
 ...
 * @param {String} range range
 */
const RemoveDuplicatePatternAnyEntity = function (
  parsedContent,
  pEntityName,
  entityType,
  range
  ...
  let PAIdx = -1;
  let entityRoles = [];
  let PAEntityFound = parsedContent.LUISJsonStructure.patternAnyEntities.find(
    function (item, idx) {
  ...
  if (
    PAEntityFound !== undefined &&
    PAIdx !== -1 &&
    entityType != EntityTypeEnum.PATTERNANY
  ) {
...
const handlePhraseList = function (
  ...
  if (!config.enablePhraseLists) {
    const error = BuildDiagnostic({
      message:
        'Do not support Phrase Lists. Please make sure enablePhraseLists is set to true.',
      range: range,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
  ...
  if (entityRoles.length !== 0) {
    ...
    entityRoles.forEach((item) => {
      if (item.toLowerCase() === PLCONSTS.DISABLED) {
        ...
        let errorMsg = `Phrase list entity ${entityName} has invalid role definition with roles = ${entityRoles.join(
          ', '
        )}. Roles are not supported for Phrase Lists`;
        let error = BuildDiagnostic({
          message: errorMsg,
          context: range,
        });
  ...
  for (const phraseListValues of valuesList) {
    phraseListValues
      .split(/[,;]/g)
      .map((item) => item.trim())
      .forEach((item) => pLValues.push(item));
  }

  let pLEntityExists = parsedContent.LUISJsonStructure.model_features.find(
    (item) => item.name == entityName
  );
  if (pLEntityExists) {
    if (entityType) {
    ...
    pLValues.forEach(function (plValueItem) {
      if (!wordsSplit.includes(plValueItem))
        pLEntityExists.words +=
          (pLEntityExists.words !== '' ? ',' : '') + plValueItem;
    });
  } else {
    pLEntityExists = new helperClass.modelObj(
      entityName,
      intc,
      pLValues.join(','),
      true
  ...
  if (isPLEnabledForAllModels !== undefined)
    pLEntityExists.enabledForAllModels = isPLEnabledForAllModels;
};
 ...
 * @param {String} range range
 */
const handlePrebuiltEntity = function (
  parsedContent,
  entityName,
  entityType,
  entityRoles,
  ...
  if (!config.enablePrebuiltEntities) {
    const error = BuildDiagnostic({
      message:
        'Do not support Prebuilt entity. Please make sure enablePrebuiltEntities is set to true.',
      range: range,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
  ...
  if (prebuiltCheck === null) {
    if (log) {
      process.stdout.write(
        chalk.default.yellowBright(
          '[WARN]: Requested PREBUILT entity "' +
            entityType +
            ' is not available for the requested locale: ' +
            locale +
            '\n'
        )
      );
      process.stdout.write(
        chalk.default.yellowBright('  Skipping this prebuilt entity..\n')
      );
    } else {
    ...
    if (log) {
      ...
      process.stdout.write(
        chalk.default.yellowBright(
          '  Switching to ' +
            builtInTypes.perLocaleAvailability[locale][entityType] +
            ' instead.\n'
        )
      );
    }
...
const handleComposite = function (
  ...
  range,
  inlineChildRequired,
  isEntityTypeDefinition,
  config
) {
  if (!config.enableCompositeEntities) {
    const error = BuildDiagnostic({
      message:
        'Do not support Composite entity. Please make sure enableCompositeEntities is set to true.',
      range: range,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
  ...
  let simpleEntityExists = (
    parsedContent.LUISJsonStructure.entities || []
  ).find((item) => item.name == entityName);
  if (simpleEntityExists !== undefined) {
    ...
    for (
      var idx = 0;
      idx < parsedContent.LUISJsonStructure.entities.length;
      idx++
    ) {
      if (
        parsedContent.LUISJsonStructure.entities[idx].name ===
        simpleEntityExists.name
      ) {
  ...
  // drop [] and trim
  let childDefinition = entityType
    .trim()
    .replace('[', '')
    .replace(']', '')
    .trim();
  if (childDefinition.length === 0 && inlineChildRequired) {
    ...
    throw new exception(
      retCode.errorCode.INVALID_COMPOSITE_ENTITY,
      error.toString(),
      [error]
    );
  }
  ...
  let compositeChildren =
    childDefinition !== ''
      ? childDefinition.split(new RegExp(/[,;]/g)).map((item) => item.trim())
      : [];
  // add this composite entity if it does not exist
  let compositeEntity = (parsedContent.LUISJsonStructure.composites || []).find(
    (item) => item.name == entityName
  );
  if (compositeEntity === undefined) {
    // add new composite entity
    parsedContent.LUISJsonStructure.composites.push(
      new helperClass.compositeEntity(
        entityName,
        compositeChildren,
        entityRoles
      )
    ...
    if (isEntityTypeDefinition) {
      if (
        compositeEntity.children.length !== 0 &&
        JSON.stringify(compositeChildren.sort()) !==
          JSON.stringify(compositeEntity.children.sort())
      ) {
...
const handleClosedList = function (
  ...
  if (!config.enableListEntities) {
    const error = BuildDiagnostic({
      message:
        'Do not support List entity. Please make sure enableListEntities is set to true.',
      range: range,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
  ...
  let closedListExists = parsedContent.LUISJsonStructure.closedLists.find(
    (item) => item.name == entityName
  );
  let addCL = false;
  ...
  listLines.forEach((line) => {
    ...
    if (line.toLowerCase().endsWith(':')) {
      ...
      nvExists = closedListExists.subLists.find(
        (item) => item.canonicalForm == normalizedValue
      );
      ...
      line.split(/[,;]/g).forEach((item) => {
        ...
        if (!nvExists || !nvExists.list) {
          ...
          throw new exception(
            retCode.errorCode.SYNONYMS_NOT_A_LIST,
            error.toString(),
            [error]
          );
        }
  ...
  // merge roles
  entityRoles.forEach((item) => {
    if (!closedListExists.roles.includes(item))
      closedListExists.roles.push(item);
  });
...
const parseAndHandleEntitySection = function (
  ...
  // handle entity
  let entities = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.ENTITYSECTION
  );
  if (entities && entities.length > 0) {
    for (const entity of entities) {
      ...
      let pEntityName =
        entityName.toLowerCase() === 'prebuilt' ? entityType : entityName;
      ...
      if (builtInTypes.consolidatedList.includes(entityType)) {
        ...
        if (entityType.includes(PARSERCONSTS.QNAALTERATIONS)) {
          ...
          if (
            entity.SynonymsOrPhraseList &&
            entity.SynonymsOrPhraseList.length > 0
          ) {
            alterationlist = alterationlist.concat(entity.SynonymsOrPhraseList);
            parsedContent.qnaAlterations.wordAlterations.push(
              (new qnaAlterations().wordAlterations = {
                alterations: alterationlist,
              })
            );
          } else {
          ...
          let normalizedValue = entityType
            .substring(0, entityType.length - 1)
            .trim();
          let synonymsList = entity.SynonymsOrPhraseList;
          let closedListExists = helpers.filterMatch(
            parsedContent.LUISJsonStructure.closedLists,
            'name',
            entityName
          );
          if (closedListExists.length === 0) {
            parsedContent.LUISJsonStructure.closedLists.push(
              new helperClass.closedLists(
                entityName,
                [new helperClass.subList(normalizedValue, synonymsList)],
                entityRoles
              )
            );
          } else {
            // closed list with this name already exists
            let subListExists = helpers.filterMatch(
              closedListExists[0].subLists,
              'canonicalForm',
              normalizedValue
            );
            if (subListExists.length === 0) {
              ...
              synonymsList.forEach(function (listItem) {
                if (!subListExists[0].list.includes(listItem))
                  subListExists[0].list.push(listItem);
              });
            }
        ...
        if (entityType.endsWith('/')) {
          ...
          throw new exception(
            retCode.errorCode.INVALID_REGEX_ENTITY,
            error.toString(),
            [error]
          );
        }
 ...
 * @param {String} range range
 */
const handleRegExEntity = function (
  parsedContent,
  entityName,
  entityType,
  entityRoles,
  ...
  if (!config.enableRegexEntities) {
    const error = BuildDiagnostic({
      message:
        'Do not support Regex entity. Please make sure enableRegexEntities is set to true.',
      range: range,
    });
    throw new exception(retCode.errorCode.INVALID_INPUT, error.toString(), [
      error,
  ...
  let regExEntity = (parsedContent.LUISJsonStructure.regex_entities || []).find(
    (item) => item.name == entityName
  );
  if (regExEntity === undefined) {
    ...
    if (
      regExEntity.regexPattern !== '' &&
      regex !== '' &&
      regExEntity.regexPattern !== regex
    ) {
...
const parseAndHandleQnaSection = async function (parsedContent, luResource) {
  // handle QNA
  let qnas = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.QNASECTION
  );
    ...
    for (const qna of qnas) {
      ...
      if (filterPairs && filterPairs.length > 0) {
        filterPairs.forEach((pair) =>
          metadata.push(new qnaMetaDataObj(pair.key, pair.value))
        );
      }
      ...
      parsedContent.qnaJsonStructure.qnaList.push(
        new qnaListObj(
          qna.QAPairId || 0,
          answer.trim(),
          qna.source,
...
const parseAndHandleModelInfoSection = function (
  ...
  let modelInfos = luResource.Sections.filter(
    (s) => s.SectionType === SectionType.MODELINFOSECTION
  );
  if (modelInfos && modelInfos.length > 0) {
    ...
    for (const modelInfo of modelInfos) {
      ...
      let kvPair = line
        .split(
          /@(app|kb|intent|entity|enableSections|enableMergeIntents|patternAnyEntity|parser).(.*)=/g
        )
        .map((item) => item.trim());
      ...
      if (kvPair.length === 4) {
        ...
        kvPair.forEach((item) => {
          if (item.trim() === '') {
            if (log) {
              process.stdout.write(
                chalk.default.yellowBright(
                  '[WARN]: Invalid model info found. Skipping "' + line + '"\n'
                )
              );
            }
        ...
        if (kvPair[1].toLowerCase() === 'parser') {
          if (kvPair[2].toLowerCase().startsWith('onambiguouslabels')) {
            parsedContent.LUISJsonStructure.onAmbiguousLabels =
              kvPair[3].toLowerCase() || 'takeLongestLabel';
            continue;
          }
        }
        if (kvPair[1].toLowerCase() === 'app') {
          if (kvPair[2].toLowerCase().startsWith('settings')) {
            let settingsRegExp = /^settings.(?<property>.*?$)/gim;
            let settingsPair = settingsRegExp.exec(kvPair[2]);
            if (
              settingsPair &&
              settingsPair.groups &&
              settingsPair.groups.property
            ) {
              if (!parsedContent.LUISJsonStructure.settings) {
                parsedContent.LUISJsonStructure.settings = [
                  {
                    name: settingsPair.groups.property,
                    value: kvPair[3].toLowerCase() === 'true',
                  },
                ];
              } else {
                // find the setting
                let sFound = parsedContent.LUISJsonStructure.settings.find(
                  (setting) => setting.name == settingsPair.groups.property
                );
                if (sFound) {
                  sFound.value = kvPair[3] === 'true';
                } else {
                  parsedContent.LUISJsonStructure.settings.push({
                    name: settingsPair.groups.property,
                    value: kvPair[3].toLowerCase() === 'true',
                  });
                }
          ...
          if (kvPair[2].toLowerCase() === 'inherits') {
            let inheritsProperties = kvPair[3]
              .split(/[:;]/g)
              .map((item) => item.trim());
            if (inheritsProperties.length !== 6) {
              process.stdout.write(
                chalk.default.yellowBright(
                  '[WARN]: Invalid intent inherits information found. Skipping "' +
                    line +
                    '"\n'
                )
              );
              ...
              // find the intent
              let intent = parsedContent.LUISJsonStructure.intents.find(
                (item) => item.name == inheritsProperties[1]
              );
              if (intent === undefined) {
                let newIntent = {
                ...
                newIntent['inherits'][inheritsProperties[2]] =
                  inheritsProperties[3];
                newIntent['inherits'][inheritsProperties[4]] =
                  inheritsProperties[5];
                parsedContent.LUISJsonStructure.intents.push(newIntent);
              } else {
                if (intent['inherits'] === undefined) intent['inherits'] = {};
                intent['inherits'][inheritsProperties[2]] =
                  inheritsProperties[3];
                intent['inherits'][inheritsProperties[4]] =
                  inheritsProperties[5];
              }
            }
          } else {
          ...
          if (kvPair[2].toLowerCase() === 'inherits') {
            ...
            if (inheritsProperties.length !== 6) {
              process.stdout.write(
                chalk.default.yellowBright(
                  '[WARN]: Invalid entity inherits information found. Skipping "' +
                    line +
                    '"\n'
                )
              );
              ...
              // find the intent
              let entity = parsedContent.LUISJsonStructure.entities.find(
                (item) => item.name == inheritsProperties[1]
              );
              if (entity === undefined) {
                let newEntity = {
                ...
                newEntity['inherits'][inheritsProperties[2]] =
                  inheritsProperties[3];
                newEntity['inherits'][inheritsProperties[4]] =
                  inheritsProperties[5];
                parsedContent.LUISJsonStructure.entities.push(newEntity);
              } else {
                if (entity['inherits'] === undefined) entity['inherits'] = {};
                entity['inherits'][inheritsProperties[2]] =
                  inheritsProperties[3];
                entity['inherits'][inheritsProperties[4]] =
                  inheritsProperties[5];
              }
            }
          } else {
          ...
          if (kvPair[2].toLowerCase() === 'inherits') {
            ...
            if (inheritsProperties.length !== 6) {
              process.stdout.write(
                chalk.default.yellowBright(
                  '[WARN]: Invalid Pattern.Any inherits information found. Skipping "' +
                    line +
                    '"\n'
                )
              );
              ...
              // find the intent
              let entity = parsedContent.LUISJsonStructure.patternAnyEntities.find(
                (item) => item.name == inheritsProperties[1]
              );
              if (entity === undefined) {
                let newEntity = new helperClass.patternAnyEntity(
                  inheritsProperties[1]
                );
                newEntity.inherits = {};
                newEntity['inherits'][inheritsProperties[2]] =
                  inheritsProperties[3];
...
const VerifyAndUpdateSimpleEntityCollection = function (
  ...
  let entityExistsInUtteranceLabel = (
    parsedContent.LUISJsonStructure.utterances || []
  ).find((item) => {
    let entityMatch = (item.entities || []).find(
  ...
  if (entityExistsInUtteranceLabel !== undefined) {
    let entityMatch = entityExistsInUtteranceLabel.entities.filter(
    ...
    entityMatch.forEach((entity) => {
      ...
      } else if (entityType !== 'Phrase List') {
        // Fix for # 1151. Phrase lists can have same name as other entities.
...
const flattenLists = function (list, retObj, parentIdx) {
  ...
  retObj.entitiesFound.push(
    new helperClass.parserEntity(
      undefined,
      list.startPos,
      list.entity,
      retValue,
...
const mergeRoles = function (srcEntityRoles, tgtEntityRoles) {
  const rolesMap = srcEntityRoles.reduce(
    (map, role) => ((map[role] = true), map),
    {}
  );
