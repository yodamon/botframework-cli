...
export class Builder {
  ...
  async loadContents(
    ...
    for (const file of files) {
      ...
      if (cultureFromPath) {
        ...
        fileName = fileNameWithCulture.substring(
          0,
          fileNameWithCulture.length - cultureFromPath.length - 1
        );
      } else {
        fileCulture = culture;
      ...
      let qnaFiles = await fileHelper.getLuObjects(
        undefined,
        file,
        true,
        fileExtEnum.QnAFile
      ...
      if (settings === undefined) {
        const settingsPath = path.join(
          fileFolder,
          `qnamaker.settings.${suffix}.${region}.json`
        );
        let settingsContent = {};
      ...
      if (!qnaContents.has(fileCulture)) {
        let contentPerCulture = new Content(
          '',
          new qnaOptions(botName, true, fileCulture, file)
        );
    ...
    await this.resolveMergedQnAContentIds(
      qnaContents,
      qnaObjects,
      importResolver
    );
    ...
    return {
      qnaContents: [...qnaContents.values()],
      recognizers,
      multiRecognizers,
      settings,
  ...
  async build(
    ...
    if (!isAllQnAEmpty) {
      ...
      while (qnaContents.length > 0) {
        ...
        await Promise.all(
          subQnaContents.map(async (content) => {
            // init current kb object from qna content
            const qnaObj = await this.initQnaFromContent(
              content,
              botName,
              suffix
            );
            ...
            let recognizerWithKBId = recognizersOfContentCulture.find(
              (r: Recognizer) => r.getKBId() !== ''
            );
            if (recognizerWithKBId !== undefined) {
              // To see if need update the model
              needPublish = await this.updateKB(
                currentKB,
                qnaBuildCore,
                recognizerWithKBId,
                delayDuration
              ...
              // create a new kb
              needPublish = await this.createKB(
                currentKB,
                qnaBuildCore,
                recognizersOfContentCulture,
                delayDuration
              );
            }

            const publishRecognizer =
              recognizerWithKBId || recognizersOfContentCulture[0];

            if (needPublish) {
              // train and publish kb
              await this.publishKB(
                qnaBuildCore,
                publishRecognizer,
                currentKB.name,
                delayDuration
            ...
            if (hostName === '')
              hostName = (await qnaBuildCore.getKB(publishRecognizer.getKBId()))
                .hostName;
            ...
            if (
              currentAlt.wordAlterations &&
              currentAlt.wordAlterations.length > 0
            ) {
            ...
            for (const recognizer of recognizersOfContentCulture) {
              ...
              const dialogName = path.basename(
                recognizer.getDialogPath(),
                `.${culture}.qna.dialog`
              );
              const dialogFileName = path.basename(
                recognizer.getDialogPath(),
                '.dialog'
              );
              ...
              if (
                crosstrainedRecognizers &&
                crosstrainedRecognizers.has(dialogName)
              ) {
                let crosstrainedRecognizer = crosstrainedRecognizers.get(
                  dialogName
                ) as CrossTrainedRecognizer;
                if (
                  !crosstrainedRecognizer.recognizers.includes(
                    dialogName + '.qna'
                  )
                ) {
              ...
              if (settings) {
                settings.qna[
                  dialogFileName.split('.').join('_').replace(/-/g, '_')
                ] = recognizer.getKBId();
  ...
  async initQnaFromContent(content: any, botName: string, suffix: string) {
    ...
    if (!currentQna.kb.name)
      currentQna.kb.name = `${botName}(${suffix}).${content.language}.qna`;
  ...
  async updateKB(
    currentKB: any,
    qnaBuildCore: QnaBuildCore,
    recognizer: Recognizer,
    delayDuration: number
  ) {
    await delay(delayDuration);
    const existingKB = await qnaBuildCore.exportKB(
      recognizer.getKBId(),
      'Prod'
    );
  ...
  async createKB(
    ...
    try {
      const opResult = await this.getKBOperationStatus(
        qnaBuildCore,
        operationId,
        delayDuration
      );
    ...
    while (isGetting) {
      ...
      if (opResult.operationState === 'Failed') {
        throw new exception(
          ...
          JSON.stringify(opResult, null, 4)
        );
      }
