...
export class QnaBuildCore {
  ...
  constructor(subscriptionkey: string, endpoint: string) {
    ...
    if (!absoluteUrlPattern.test(endpoint)) {
      throw new exception(
        ...
        `Only absolute URLs are supported. "${endpoint}" is not an absolute qnamaker endpoint URL.`
      );
    }
  ...
  public async getKB(kbId: string) {
    const response = await this.service.createRequest(
      `/knowledgebases/${kbId}`,
      'GET'
    );
    const text = await response.text();
  ...
  private parseToQnAContent(qnaList: any) {
    ...
    qnaList.forEach((qnaItem: any) => {
      ...
      fileContent +=
        '> !# @qna.pair.source = ' + qnaItem.source + NEWLINE + NEWLINE;
      ...
      if (
        qnaItem.context &&
        qnaItem.context.prompts &&
        qnaItem.context.prompts.length !== 0
      ) {
        ...
        qnaItem.context.prompts.forEach((prompt: any) => {
          ...
          if (promptQnA) {
            fileContent +=
              promptQnA.context.isContextOnly === true ? ' `context-only`' : '';
          }
          fileContent += NEWLINE;
        });
      }
