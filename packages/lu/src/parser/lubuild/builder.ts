...
export class Builder {
  ...
  async loadContents(
    ...
    for (const file of files) {
      ...
      if (cultureFromPath) {
        ...
        fileName = fileNameWithCulture.substring(
          0,
          fileNameWithCulture.length - fileCulture.length - 1
        );
      } else {
        fileCulture = culture;
      ...
      const crossTrainedRecognizerPath = path.join(
        fileFolder,
        crossTrainedFileName
      );
      if (!crosstrainedRecognizers.has(fileName)) {
        ...
        if (fs.existsSync(crossTrainedRecognizerPath)) {
          let crosstrainedRecognizerObject = JSON.parse(
            await fileHelper.getContentFromFile(crossTrainedRecognizerPath)
          );
          crosstrainedRecognizerContent =
            crosstrainedRecognizerObject.recognizers;
          crosstrainedRecognizerSchema =
            crosstrainedRecognizerSchema ||
            crosstrainedRecognizerObject.$schema;
          this.handler(`${crossTrainedRecognizerPath} loaded\n`);
        }

        crosstrainedRecognizers.set(
          fileName,
          new CrossTrainedRecognizer(
            crossTrainedRecognizerPath,
            crosstrainedRecognizerContent,
            crosstrainedRecognizerSchema as string
          )
        );
      }
      ...
      let result;
      let luisObj;
      let luFiles = await fileHelper.getLuObjects(
        undefined,
        file,
        true,
        fileExtEnum.LUFile
      ...
      try {
        result = await LuisBuilderVerbose.build(
          luFiles,
          true,
          fileCulture,
          importResolver
      ...
      const multiRecognizerPath = path.join(
        fileFolder,
        `${fileName}.lu.dialog`
      );
      if (!multiRecognizers.has(fileName)) {
        ...
        if (fs.existsSync(multiRecognizerPath)) {
          let multiRecognizerObject = JSON.parse(
            await fileHelper.getContentFromFile(multiRecognizerPath)
          );
          multiRecognizerContent = multiRecognizerObject.recognizers;
          multiRecognizerSchema =
            multiRecognizerSchema || multiRecognizerObject.$schema;
          this.handler(`${multiRecognizerPath} loaded\n`);
        }

        multiRecognizers.set(
          fileName,
          new MultiLanguageRecognizer(
            multiRecognizerPath,
            multiRecognizerContent,
            multiRecognizerSchema as string
          )
        );
      }
      ...
      if (settings === undefined) {
        const settingsPath = path.join(
          fileFolder,
          `luis.settings.${suffix}.${region}.json`
        );
        let settingsContent = {};
        if (fs.existsSync(settingsPath)) {
          settingsContent = JSON.parse(
            await fileHelper.getContentFromFile(settingsPath)
          ).luis;
      ...
      const content = new Content(
        fileContent,
        new LUOptions(fileName, true, fileCulture, file)
      );
      ...
      if (fs.existsSync(dialogFile)) {
        existingDialogObj = JSON.parse(
          await fileHelper.getContentFromFile(dialogFile)
        );
  ...
  async build(
      ...
      while (luContents.length > 0) {
        ...
        await Promise.all(
          subLuContents.map(async (content) => {
            ...
            let currentApp = await this.initApplicationFromLuContent(
              content,
              botName,
              suffix
            );
            ...
            if (recognizer.getAppId() && recognizer.getAppId() !== '') {
              ...
              const versionCount =
                keptVersionCount && keptVersionCount > 0
                  ? keptVersionCount
                  : maxVersionCount;
              needTrainAndPublish = await this.updateApplication(
                currentApp,
                luBuildCore,
                recognizer,
                timeBucket,
              ...
              needTrainAndPublish = await this.createApplication(
                currentApp,
                luBuildCore,
                recognizer,
                timeBucket
            ...
            if (needTrainAndPublish) {
              ...
              await this.trainAndPublishApplication(
                luBuildCore,
                recognizer,
                timeBucket,
                isStaging
            ...
            if (multiRecognizers && multiRecognizers.has(content.id)) {
              let multiRecognizer = multiRecognizers.get(
                content.id
              ) as MultiLanguageRecognizer;
              multiRecognizer.recognizers[currentApp.culture] = path.basename(
                recognizer.getDialogPath(),
                '.dialog'
              );
              if (
                currentApp.culture.toLowerCase() ===
                fallbackLocale.toLowerCase()
              ) {
                multiRecognizer.recognizers[''] = path.basename(
                  recognizer.getDialogPath(),
                  '.dialog'
                );
              }
            ...
            if (
              crosstrainedRecognizers &&
              crosstrainedRecognizers.has(content.id)
            ) {
              let crosstrainedRecognizer = crosstrainedRecognizers.get(
                content.id
              ) as CrossTrainedRecognizer;
              if (
                !crosstrainedRecognizer.recognizers.includes(content.id + '.lu')
              ) {
            ...
            if (settings) {
              settings.luis[
                content.name.split('.').join('_').replace(/-/g, '_')
              ] = {
    ...
    if (dialogType === recognizerType.CROSSTRAINED && crosstrainedRecognizers) {
      crosstrainedRecognizerValues = Array.from(
        crosstrainedRecognizers.values()
      );
    }

    const dialogContents = this.generateDeclarativeAssets(
      recognizerValues,
      multiRecognizerValues,
      settingsValue,
      crosstrainedRecognizerValues
    );

    return dialogContents;
  }

  async writeDialogAssets(
    contents: any[],
    force: boolean,
    out: string,
    luconfig: string
    ...
    if (settingsContents && settingsContents.length > 0) {
      ...
      if (luconfig) {
        outPath = path.join(
          path.resolve(path.dirname(luconfig)),
          settingsContents[0].id
        );
      } else if (out) {
    ...
    for (const content of writeContents) {
      ...
      if (fileExists && outFilePath.endsWith('.lu.qna.dialog')) {
        let existingCTRecognizerObject = JSON.parse(
          await fileHelper.getContentFromFile(outFilePath)
        );
        let currentCTRecognizerObject = JSON.parse(content.content);
        let ctRecognizerToBeMerged = currentCTRecognizerObject.recognizers.filter(
          (r: string) => !existingCTRecognizerObject.recognizers.includes(r)
        );
        existingCTRecognizerObject.recognizers = existingCTRecognizerObject.recognizers.concat(
          ctRecognizerToBeMerged
        );
  ...
  async initApplicationFromLuContent(
    content: any,
    botName: string,
    suffix: string
  ) {
    ...
    currentApp.culture =
      currentApp.culture &&
      currentApp.culture !== '' &&
      currentApp.culture !== 'en-us'
        ? currentApp.culture
        : (content.language as string);
    currentApp.desc =
      currentApp.desc && currentApp.desc !== ''
        ? currentApp.desc
        : `Model for ${botName} app, targetting ${suffix}`;
  ...
  async updateApplication(
    ...
    recognizer.versionId =
      appInfo.activeVersion || appInfo.endpoints.PRODUCTION.versionId;

    await delay(timeBucket);
    const existingApp = await luBuildCore.exportApplication(
      recognizer.getAppId(),
      recognizer.versionId
    );
    ...
    if (needUpdate) {
      ...
      this.handler(
        `${recognizer.getLuPath()} creating version=${newVersionId}\n`
      );
      await delay(timeBucket);
      await luBuildCore.importNewVersion(
        recognizer.getAppId(),
        currentApp,
        options
      );
      ...
      await delay(timeBucket);
      const versionObjs = await luBuildCore.listApplicationVersions(
        recognizer.getAppId()
      );
      if (keptVersionCount < versionObjs.length) {
        const versionObjsToDelete = versionObjs
          .reverse()
          .splice(0, versionObjs.length - keptVersionCount);
        for (const versionObj of versionObjsToDelete) {
          if (versionObj.version !== newVersionId) {
            this.handler(
              `${recognizer.getLuPath()} deleting old version=${
                versionObj.version
              }`
            );
            await delay(timeBucket);
            await luBuildCore.deleteVersion(
              recognizer.getAppId(),
              versionObj.version
            );
          }
  ...
  async createApplication(
    ...
    currentApp.versionId =
      currentApp.versionId && currentApp.versionId !== ''
        ? currentApp.versionId
        : '0.1';
    recognizer.versionId = currentApp.versionId;
    this.handler(
      `Creating LUIS.ai application: ${currentApp.name} version:${currentApp.versionId}\n`
    );
    await delay(timeBucket);
  ...
  async trainAndPublishApplication(
    luBuildCore: LuBuildCore,
    recognizer: Recognizer,
    timeBucket: number,
    isStaging: boolean
    ...
    this.handler(
      `${recognizer.getLuPath()} training version=${recognizer.versionId}\n`
    );
    await delay(timeBucket);
    await luBuildCore.trainApplication(
      recognizer.getAppId(),
      recognizer.versionId
    );
    this.handler(
      `${recognizer.getLuPath()} waiting for training for version=${
        recognizer.versionId
      }...\n`
    );
    let done = true;
    do {
      ...
      let trainingStatus = await luBuildCore.getTrainingStatus(
        recognizer.getAppId(),
        recognizer.versionId
      );
      done = true;
      for (let status of trainingStatus) {
        if (status.details) {
          if (
            status.details.status === 'InProgress' ||
    ...
    this.handler(
      `${recognizer.getLuPath()} publishing version=${recognizer.versionId}\n`
    );
    await delay(timeBucket);
    await luBuildCore.publishApplication(
      recognizer.getAppId(),
      recognizer.versionId,
      isStaging
    );
    this.handler(
      `${recognizer.getLuPath()} publishing finished for ${
        isStaging ? 'Staging' : 'Production'
      } slot\n`
    );
  }

  generateDeclarativeAssets(
    recognizers: Array<Recognizer>,
    multiRecognizers: Array<MultiLanguageRecognizer>,
    settings: Settings,
    crosstrainedRecognizers: Array<CrossTrainedRecognizer>
  ): Array<any> {
    ...
    for (const crosstrainedRecognizer of crosstrainedRecognizers) {
      const crosstrainedContent = new Content(
        crosstrainedRecognizer.save(),
        new LUOptions(
          path.basename(crosstrainedRecognizer.getDialogPath()),
          true,
          '',
          crosstrainedRecognizer.getDialogPath()
        )
      );
  ...
  filterEmptyIntents(app: any) {
    ...
    const emptyIntents = intents.filter(
      (intent: any) =>
        !utterances.some(
          (utterance: any) => utterance.intent === intent.name
    ...
    if (emptyIntents && emptyIntents.length > 0) {
      const filteredIntents = intents.filter(
        (intent: any) =>
          !emptyIntents.some(
            (emptyIntent: any) => emptyIntent.name === intent.name
          )
      );
      this.handler(
        `[WARN]: empty intent(s) ${emptyIntents
          .map((intent: any) => '# ' + intent.name)
          .join(', ')} are filtered when handling luis application`
      );
