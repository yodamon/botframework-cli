...
export class LuBuildCore {
  ...
  constructor(
    ...
    if (!absoluteUrlPattern.test(endpoint)) {
      throw new exception(
        retCode.errorCode.INVALID_URI,
        `Only absolute URLs are supported. "${endpoint}" is not an absolute LUIS endpoint URL.`
      );
    }
  ...
  public async importApplication(currentApp: any): Promise<any> {
    ...
    const url =
      this.endpoint + '/luis/authoring/v3.0-preview/apps/import' + name;
    const headers = {
    ...
    while (retryCount > 0) {
      if (error === undefined || error.code === rateLimitErrorCode.toString()) {
        ...
        throw new exception(
          retCode.errorCode.LUIS_API_CALL_FAILED,
          error.message
        );
      }
    }
  ...
  public async exportApplication(appId: string, versionId: string) {
    const url =
      this.endpoint +
      '/luis/authoring/v3.0-preview/apps/' +
      appId +
      '/versions/' +
      versionId +
      '/export?format=json';
  ...
  public compareApplications(currentApp: any, existingApp: any) {
    currentApp.desc =
      currentApp.desc &&
      currentApp.desc !== '' &&
      currentApp.desc !== existingApp.desc
        ? currentApp.desc
        : existingApp.desc;
    currentApp.culture =
      currentApp.culture &&
      currentApp.culture !== '' &&
      currentApp.culture !== existingApp.culture
        ? currentApp.culture
        : existingApp.culture;
    currentApp.versionId =
      currentApp.versionId &&
      currentApp.versionId !== '' &&
      currentApp.versionId > existingApp.versionId
        ? currentApp.versionId
        : existingApp.versionId;
    ...
    if (
      currentAppToCompare.intents &&
      !currentAppToCompare.intents.some((x: any) => x.name === 'None')
    ) {
      const existingNoneIntentIndex = existingApp.intents.findIndex(
        (x: any) => x.name === 'None'
      );
      if (existingNoneIntentIndex > -1)
        existingApp.intents.splice(existingNoneIntentIndex, 1);
    }

    // compare lu contents converted from luis josn
    const isApplicationEqual = this.isApplicationEqual(
      currentAppToCompare,
      existingApp
    );
