...
export class Recognizer {
  static load(
    ...
    dialogPath: string,
    luisSettings: Settings,
    existingRecognizer: any,
    schema?: string
  ): Recognizer {
    if (existingRecognizer) {
      ...
      recognizer.setAppId(
        luisSettings.luis[path.basename(luFile).split('.').join('_')]
      );
  ...
  constructor(
    private readonly luFile: string,
    targetFileName: string,
    schema?: string
  ) {
    this.appId = '';
    this.id = `LUIS_${targetFileName.split('.')[0]}`;
    this.applicationId = `=settings.luis.${targetFileName
      .split('.')
      .join('_')
      .replace(/-/g, '_')}.appId`;
    this.endpoint = '=settings.luis.endpoint';
    this.endpointKey = '=settings.luis.endpointKey';
    this.version = `=settings.luis.${targetFileName
      .split('.')
      .join('_')
      .replace(/-/g, '_')}.version`;
    this.versionId = '0.1';
