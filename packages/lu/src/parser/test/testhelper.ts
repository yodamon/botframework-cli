...
export async function test(
  ...
  while (index < luisObject.utterances.length) {
    const subUtterances = luisObject.utterances.slice(
      index,
      index + concurrency
    );
    await Promise.all(
      subUtterances.map(async (utterance: any) => {
        const predictionRequest: any = {};
        predictionRequest.query = utterance.text;
        let predictedResult = await client.prediction.getSlotPrediction(
          appId,
          slotName,
          predictionRequest,
          options
        ...
        for (const intent of Object.keys(predictedResult.prediction.intents)) {
          ...
          predictedIntents.score =
            predictedResult.prediction.intents[intent].score;
        ...
        if (!intentOnly) {
          ...
          result.EntityPass = compareEntity(
            result.predictedEntities,
            utterance
          );
        }
  ...
  luisObject.utterances.forEach((utterance: any) => {
    let intent = luisObject.intents.find(
      (item: any) => item.name === utterance.intent
    );
    intent.count++;
    if (
      utterance.predictedResult.IntentPass &&
      utterance.predictedResult.EntityPass !== false
    ) {
      intent.passNumber++;
    }
  });
...
function compareEntity(predictedEntities: any[], utterance: any) {
  ...
  for (const entity of utterance.entities) {
    if (
      !predictedEntities.find(
        (x: any) =>
          ((entity.role !== undefined && entity.role === x.role) ||
            (entity.role === undefined && entity.entity === x.entity)) &&
    ...
    if (name === '$instance') {
      for (const entityType of Object.keys(entities.$instance)) {
        for (const entity of entities.$instance[entityType]) {
          if (
            entity.modelTypeId === 1 ||
            entity.modelTypeId === 4 ||
            (entity.role !== undefined && entity.role !== '')
          ) {
