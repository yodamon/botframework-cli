...
const parseIntentsToLu = function (luisObj, luisJSON) {
  ...
  if (luisJSON.test === true) {
    fileContent +=
      `> Total passed: ${luisJSON.passNumber}/${luisJSON.count}` +
      NEWLINE +
      NEWLINE;
  }
  ...
  luisObj.intents.forEach(function (intent) {
    ...
    if (luisJSON.test === true) {
      fileContent +=
        `> Utterance passed in this intent: ${intent.intent.passNumber}/${intent.intent.count}` +
        NEWLINE;
    }
...
const parseUtterancesToLu = function (utterances, luisJSON) {
  ...
  utterances.forEach(function (utterance) {
    ...
    if (utterance.entities.length >= 0) {
      ...
      // flatten entities
      let flatEntities = [];
      Object.assign([], utterance.entities).forEach((entity) =>
        flattenEntities(entity, flatEntities)
      );
...
const updateTokenizedTextByEntity = function (tokenizedText, entity) {
  if (entity.role !== undefined) {
    tokenizedText[parseInt(entity.startPos)] = `{@${entity.role}=${
      tokenizedText[parseInt(entity.startPos)]
    }`;
  } else {
    tokenizedText[parseInt(entity.startPos)] = `{@${entity.entity}=${
      tokenizedText[parseInt(entity.startPos)]
    }`;
  }
...
const parsePredictedResultToLu = function (utterance, luisJSON) {
  ...
  if (
    utterance.predictedResult.predictedIntents !== undefined &&
    utterance.predictedResult.predictedIntents.length > 0
  ) {
  ...
  fileContent +=
    passText + ' Predicted intent: ' + intents.join(', ') + NEWLINE;

  if (utterance.predictedResult.predictedEntities !== undefined) {
    if (utterance.predictedResult.predictedEntities.length > 0) {
      // update utterance for each entity
      let text = utterance.text;
      let sortedEntitiesList = objectSortByStartPos(
        utterance.predictedResult.predictedEntities
      );
      let tokenizedText = text.split('');
      let nonCompositesInUtterance = sortedEntitiesList.filter(
        (entity) =>
          luisJSON.composites.find(
            (composite) => composite.name == entity.entity
          ) == undefined
      );
      ...
      let compositeEntitiesInUtterance = sortedEntitiesList.filter(
        (entity) =>
          luisJSON.composites.find(
            (composite) => composite.name == entity.entity
          ) != undefined
    ...
    if (updatedText)
      fileContent += passText + ' Predicted entities: ' + updatedText + NEWLINE;
...
const parseEntitiesToLu = function (luisJson) {
  ...
  luisJson.entities.forEach(function (entity) {
    if (!entity.children || entity.children.length === 0) {
      ...
      fileContent += `@ ${getEntityType(entity.features)} ${writeEntityName(
        entity.name
      )}`;
...
const parseToLuClosedLists = function (luisJson) {
  ...
  luisJson.closedLists.forEach(function (ListItem) {
    fileContent += `@ list `;
    fileContent += ListItem.name.includes(' ')
      ? `"${ListItem.name}"`
      : `${ListItem.name}`;
...
const parseRegExEntitiesToLu = function (regex_entities) {
  ...
  regex_entities.forEach(function (regExEntity) {
    fileContent += `@ regex `;
    fileContent += regExEntity.name.includes(' ')
      ? `"${regExEntity.name}"`
      : regExEntity.name;
...
const parseCompositesToLu = function (luisJson) {
  ...
  luisJson.composites.forEach((composite) => {
    ...
    fileContent += composite.name.includes(' ')
      ? `"${composite.name}"`
      : composite.name;
    fileContent += addRolesAndFeatures(composite);
    if (composite.children.length > 0) {
      fileContent +=
        typeof composite.children[0] == 'object'
          ? ` = [${composite.children.map((item) => item.name).join(', ')}]`
          : ` = [${composite.children.join(', ')}]`;
    }
    fileContent += NEWLINE;
  });
  return fileContent;
...
const parsePatternAnyEntitiesToLu = function (luisJson) {
  ...
  luisJson.patternAnyEntities.forEach((patternAnyEntity) => {
    ...
    if (patternAnyEntity.inherits !== undefined) {
      // > !# @intent.inherits = {name = Web.WebSearch; domain_name = Web; model_name = WebSearch}
      fileContent +=
        '> !# @patternAnyEntity.inherits = name : ' + patternAnyEntity.name;
      if (patternAnyEntity.inherits.domain_name !== undefined) {
        fileContent +=
          '; domain_name : ' + patternAnyEntity.inherits.domain_name;
      }
    ...
    fileContent += patternAnyEntity.name.includes(' ')
      ? `"${patternAnyEntity.name}"`
      : patternAnyEntity.name;
...
const handlePhraseLists = function (collection) {
  ...
  collection.forEach(function (entity) {
    ...
    fileContent += entity.name.includes(' ')
      ? `"${entity.name}"`
      : `${entity.name}`;
    fileContent += `${entity.mode ? `(interchangeable)` : ``}`;
    if (entity.activated !== undefined && !entity.activated)
      flags += `disabled`;
    if (entity.enabledForAllModels !== undefined) {
      if (entity.enabledForAllModels === true) {
        ...
        flags +=
          flags !== '' ? `, disabledForAllModels` : `disabledForAllModels`;
      }
    }
...
const addAppMetaData = function (LUISJSON) {
  let fileContent = '';
  if (LUISJSON.name)
    fileContent += `> !# @app.name = ${LUISJSON.name}` + NEWLINE;
  if (LUISJSON.desc)
    fileContent += `> !# @app.desc = ${LUISJSON.desc}` + NEWLINE;
  if (LUISJSON.versionId)
    fileContent += `> !# @app.versionId = ${LUISJSON.versionId}` + NEWLINE;
  if (LUISJSON.culture)
    fileContent += `> !# @app.culture = ${LUISJSON.culture}` + NEWLINE;
  if (LUISJSON.luis_schema_version)
    fileContent +=
      `> !# @app.luis_schema_version = ${LUISJSON.luis_schema_version}` +
      NEWLINE;
  if (LUISJSON.settings) {
    LUISJSON.settings.forEach((setting) => {
      fileContent +=
        `> !# @app.settings.${setting.name} = ${setting.value}` + NEWLINE;
    });
  }
  if (LUISJSON.tokenizerVersion)
    fileContent +=
      `> !# @app.tokenizerVersion = ${LUISJSON.tokenizerVersion}` + NEWLINE;
  return fileContent === ''
    ? fileContent
...
const handleNDepthEntity = function (entity) {
  ...
  fileContent += NEWLINE;
  fileContent +=
    addNDepthChildDefinitions(entity.children, BASE_TAB_STOP, fileContent) +
    NEWLINE +
    NEWLINE;
  return fileContent;
};
 ...
 */
const addNDepthChildDefinitions = function (
  childCollection,
  tabStop,
  fileContent
) {
  ...
  (childCollection || []).forEach((child) => {
    ...
    myFileContent += `- @ ${getEntityType(child.features)} ${writeEntityName(
      child.name
    )}`;
    myFileContent += addRolesAndFeatures(child);
    myFileContent += NEWLINE;
    if (child.children && child.children.length !== 0) {
      myFileContent += addNDepthChildDefinitions(
        child.children,
        tabStop + 1,
        myFileContent
      );
...
const getEntityType = function (features) {
  // find constraint
  let constraint = (features || []).find(
    (feature) => feature.isRequired == true
  );
...
const addRolesAndFeatures = function (entity) {
  ...
  if (entity.roles && entity.roles.length > 0) {
    roleAndFeatureContent += ` ${
      entity.roles.length > 1 ? `hasRoles` : `hasRole`
    } `;
    entity.roles.forEach((item) => {
      roleAndFeatureContent += item.includes(' ') ? `"${item}",` : `${item},`;
    });
  }
  roleAndFeatureContent = roleAndFeatureContent.substring(
    0,
    roleAndFeatureContent.length - 1
  );
  ...
  entity.features.forEach((item) => {
    ...
    if (item.modelName) {
      if (item.isRequired !== undefined) {
        if (item.isRequired !== true) featuresList.push(item.modelName);
      } else {
  ...
  if (featuresList.length > 0) {
    roleAndFeatureContent += ` ${
      featuresList.length > 1 ? `usesFeatures` : `usesFeature`
    } `;
    featuresList.forEach((feature) => {
      roleAndFeatureContent += feature.includes(' ')
        ? `"${feature}",`
        : `${feature},`;
    });
    roleAndFeatureContent = roleAndFeatureContent.substring(
 ...
 * @returns {void}
 */
const updateUtterancesList = function (
  srcCollection,
  tgtCollection,
  attribute
) {
  (srcCollection || []).forEach((srcItem) => {
    let matchInTarget = tgtCollection.find(
      (item) => item.intent.name == srcItem.intent
    );
    ...
    if (
      !matchInTarget.utterances.find((item) => item.text == srcItem[attribute])
    ) {
      addUtteranceToCollection(attribute, srcItem, matchInTarget);
      return;
    }
...
const addUtteranceToCollection = function (attribute, srcItem, matchInTarget) {
  if (attribute === 'text') {
    ...
    matchInTarget.utterances.push(
      new helperClasses.uttereances(
        srcItem.pattern.replace('{', '{@'),
        srcItem.intent,
        []
      )
    );
...
const objectSortByStartPos = function (objectArray) {
  ...
  ObjectByStartPos.sort(function (a, b) {
    if (a.startPos === b.startPos) return a.endPos - b.endPos;
