...
const validateBoundaries = function (luisJSON) {
  ...
  if (luisJSON.intents.length > retCode.boundaryLimits.MAX_NUM_INTENTS) {
    validationError(
      retCode.errorCode.BOUNDARY_INTENTS,
      `${luisJSON.intents.length} intents found in application. At most ${retCode.boundaryLimits.MAX_NUM_INTENTS} is allowed.`
    );
  }
  ...
  if (luisJSON.utterances.length > retCode.boundaryLimits.MAX_NUM_UTTERANCES) {
    validationError(
      retCode.errorCode.BOUNDARY_UTTERANCES,
      `${luisJSON.utterances.length} utterances found in application. At most ${retCode.boundaryLimits.MAX_NUM_UTTERANCES} is allowed.`
    );
  }

  // pattern.any entities - 100 per application
  if (
    luisJSON.patternAnyEntities.length >
    retCode.boundaryLimits.MAX_NUM_PATTERNANY_ENTITIES
  ) {
    validationError(
      retCode.errorCode.BOUNDARY_PATTERNANYENTITY,
      `${luisJSON.patternAnyEntities.length} pattern.any entities found in application. At most ${retCode.boundaryLimits.MAX_NUM_PATTERNANY_ENTITIES} is allowed.`
    );
  }
  ...
  luisJSON.utterances.forEach((utt) => {
    if (utt.text.length > retCode.boundaryLimits.MAX_CHAR_IN_UTTERANCE) {
      validationError(
        retCode.errorCode.BOUNDARY_UTTERANCE_CHAR_LENGTH,
        `utterance '${utt.text}' under intent '${utt.intent}' has ${utt.text.length} characters. At most ${retCode.boundaryLimits.MAX_CHAR_IN_UTTERANCE} is allowed.`
      );
    }
  });
  ...
  if (luisJSON.patterns.length > retCode.boundaryLimits.MAX_NUM_PATTERNS) {
    validationError(
      retCode.errorCode.BOUNDARY_PATTERNS,
      `${luisJSON.patterns.length} patterns found in application. At most ${retCode.boundaryLimits.MAX_NUM_PATTERNS} is allowed.`
    );
  }
  ...
  luisJSON.patterns.forEach((utt) => {
    if (utt.pattern.length > retCode.boundaryLimits.MAX_CHAR_IN_PATTERNS) {
      validationError(
        retCode.errorCode.BOUNDARY_PATTERN_CHAR_LIMIT,
        `Pattern '${utt.pattern}' under intent '${utt.intent}' has ${utt.pattern.length} characters. At most ${retCode.boundaryLimits.MAX_CHAR_IN_PATTERNS} characters are allowed in any pattern.`
      );
    }
  });
  ...
  if (
    luisJSON.regex_entities.length >
    retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES
  ) {
    validationError(
      retCode.errorCode.BOUNDARY_REGEX_ENTITY,
      `${luisJSON.regex_entities.length} regex entities found in application. At most ${retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES} is allowed.`
    );
  }
  ...
  luisJSON.regex_entities.forEach((utt) => {
    if (
      utt.regexPattern.length >
      retCode.boundaryLimits.MAX_CHAR_REGEX_ENTITY_PATTERN
    ) {
      validationError(
        retCode.errorCode.BOUNDARY_REGEX_CHAR_LIMIT,
        `Regex entity '${utt.name}' with pattern /${utt.regexPattern}/ has ${utt.regexPattern.length} characters. At most ${retCode.boundaryLimits.MAX_CHAR_REGEX_ENTITY_PATTERN} is allowed.`
      );
    }
  });
  ...
  luisJSON.closedLists.forEach((listEntity) => {
    listEntity.subLists.forEach((subList) => {
      if (
        subList.list.length > retCode.boundaryLimits.MAX_LIST_ENTITY_SYNONYMS
      ) {
        validationError(
          retCode.errorCode.BOUNDARY_SYNONYMS_LENGTH,
          `'${listEntity.name}' list entity for parent (normalized value) '${subList.canonicalForm}' has ${subList.list.length} synonyms. At most ${retCode.boundaryLimits.MAX_LIST_ENTITY_SYNONYMS} is allowed.`
        );
      }
    });
  });
  ...
  if (phraseLists.length > retCode.boundaryLimits.MAX_NUM_PHRASE_LISTS) {
    validationError(
      retCode.errorCode.BOUNDARY_PHRASE_LIST_LIMIT,
      `${phraseLists.length} phrase lists found in application. At most ${retCode.boundaryLimits.MAX_NUM_PHRASE_LISTS} is allowed.`
    );
  }
  ...
  phraseLists.forEach(
    (item) => (totalPhrasesInApp += item.words.split(',').length)
  );
  if (
    totalPhrasesInApp >
    retCode.boundaryLimits.MAX_NUM_PHRASES_IN_ALL_PHRASE_LIST
  ) {
    validationError(
      retCode.errorCode.BOUNDARY_TOTAL_PHRASES,
      `${totalPhrasesInApp} phrases found across all phrase list definitions. At most ${retCode.boundaryLimits.MAX_NUM_PHRASES_IN_ALL_PHRASE_LIST} is allowed.`
    );
  }
  ...
  phraseLists
    ...
    .forEach((item) => {
      if (
        item.words.split(',').length >
        retCode.boundaryLimits.MAX_INTERCHANGEABLE_PHRASES
      ) {
        validationError(
          retCode.errorCode.BOUNDARY_INTC_PHRASES_LIMIT,
          `${totalPhrasesInApp} phrases found across all interchangeable phrase list definitions. At most ${retCode.boundaryLimits.MAX_INTERCHANGEABLE_PHRASES} is allowed.`
        );
      }
    });
  ...
  phraseLists
    ...
    .forEach((item) => {
      if (
        item.words.split(',').length >
        retCode.boundaryLimits.MAX_NON_INTERCHANGEABLE_PHRASES
      ) {
        validationError(
          retCode.errorCode.BOUNDARY_NINTC_PHRASES_LIMIT,
          `${totalPhrasesInApp} phrases found across all non-interchangeable phrase list definitions. At most ${retCode.boundaryLimits.MAX_NON_INTERCHANGEABLE_PHRASES} is allowed.`
        );
      }
    });
  ...
  phraseLists.forEach((item) => {
    ...
    if (
      ...
      item.words.trim() === '' ||
      item.words.split(',').length === 0
    ) {
      validationError(
        retCode.errorCode.BOUNDARY_MINMUM_PHRASE_LIMIT,
        `0 phrases found in phrase list: ${item.name}. Empty phrase list is not allowed.`
      );
    }
  });
  ...
  [
    ...
    luisJSON[scope].forEach((item) => {
      if (item.roles.length > retCode.boundaryLimits.MAX_ROLES_PER_ENTITY) {
        validationError(
          retCode.errorCode.BOUNDARY_ROLES_PER_ENTITY,
          `${scope.substring(0, scope.length - 1)} ${item.name} has ${
            item.roles.length
          } roles. At most ${
            retCode.boundaryLimits.MAX_ROLES_PER_ENTITY
          } is allowed.`
        );
      }
  ...
  if (totalRoles > retCode.boundaryLimits.MAX_NUM_ROLES) {
    validationError(
      retCode.errorCode.BOUNDARY_TOTAL_ROLES,
      `${totalRoles} role definitions found across all entity types. At most ${retCode.boundaryLimits.MAX_NUM_ROLES} is allowed.`
    );
  }
    ...
    luisJSON[scope].forEach((item) => {
      if (
        ...
        item.features.length >
          retCode.boundaryLimits.MAX_NUM_DESCRIPTORS_PER_MODEL
      ) {
        validationError(
          retCode.errorCode.BOUNDARY_FEATURE_PER_MODEL,
          `${scope.substring(0, scope.length - 1)} ${item.name} has ${
            item.features.length
          } descriptors (feature). At most ${
            retCode.boundaryLimits.MAX_NUM_DESCRIPTORS_PER_MODEL
          } is allowed.`
        );
      }
  ...
  if (numberOfParentEntities > retCode.boundaryLimits.MAX_NUM_PARENT_ENTITIES) {
    validationError(
      retCode.errorCode.BOUNDARY_PARENT_ENTITY_LIMIT,
      `${numberOfParentEntities} parent ml entities found in application. At most ${retCode.boundaryLimits.MAX_NUM_PARENT_ENTITIES} is allowed.`
    );
  }

  if (
    totalNumberOfEntitiesAndRoles >
    retCode.boundaryLimits.MAX_TOTAL_ENTITES_AND_ROLES
  ) {
    validationError(
      retCode.errorCode.BOUNDARY_TOTAL_ENTITIES_AND_ROLES,
      `${totalNumberOfEntitiesAndRoles} combined roles and entity definitions found. At most ${retCode.boundaryLimits.MAX_TOTAL_ENTITES_AND_ROLES} is allowed.`
    );
  }

  // up to 50 list entities
  if (
    luisJSON.closedLists.length > retCode.boundaryLimits.MAX_NUM_CLOSED_LISTS
  ) {
    validationError(
      retCode.errorCode.BOUNDARY_TOTAL_CLOSED_LISTS,
      `${luisJSON.closedLists.length} list entity definitions found. At most ${retCode.boundaryLimits.MAX_NUM_CLOSED_LISTS} is allowed.`
    );
  }
};
...
const addEntities = function (luisJSON, entitiesList) {
  luisJSON.entities.forEach(function (entity) {
    entitiesList.push(
      new validateLUISBlobEntity(entity.name, ['simple'], entity.roles)
    );
  });
};
const addClosedLists = function (luisJSON, entitiesList, entityFound) {
  luisJSON.closedLists.forEach(function (entity) {
    ...
    if (entityFound.length === 0) {
      entitiesList.push(
        new validateLUISBlobEntity(entity.name, ['list'], entity.roles)
      );
    } else {
...
const addPatternAnyEntities = function (luisJSON, entitiesList, entityFound) {
  luisJSON.patternAnyEntities.forEach(function (entity) {
    ...
    if (entityFound.length === 0) {
      entitiesList.push(
        new validateLUISBlobEntity(entity.name, ['patternAny'], entity.roles)
      );
    } else {
...
const addregexEntities = function (luisJSON, entitiesList, entityFound) {
  luisJSON.regex_entities.forEach(function (entity) {
    ...
    if (entityFound.length === 0) {
      entitiesList.push(
        new validateLUISBlobEntity(
          entity.name,
          [`regEx:/${entity.regexPattern}/`],
          entity.roles
        )
      );
    } else {
...
const addCompositeEntities = function (luisJSON, entitiesList, entityFound) {
  (luisJSON.composites || []).forEach((entity) => {
    ...
    if (entityFound.length === 0) {
      entitiesList.push(
        new validateLUISBlobEntity(entity.name, ['composite'], entity.roles)
      );
    } else {
...
const addPrebuiltEntities = function (luisJSON, entitiesList, entityFound) {
  (luisJSON.prebuiltEntities || []).forEach((entity) => {
    ...
    if (entityFound.length === 0) {
      entitiesList.push(
        new validateLUISBlobEntity(entity.name, ['prebuilt'], entity.roles)
      );
    } else {
...
const validateEntities = function (entitiesList) {
  entitiesList.forEach(function (entity) {
    if (entity.type.length > 1) {
      let errorMsg =
        `Entity ${entity.name} has duplicate definitions.\r\n\t` +
        JSON.stringify(entity.type, 2, null);
      let error = BuildDiagnostic({ message: errorMsg });

      throw new exception(
        retCode.errorCode.DUPLICATE_ENTITIES,
        error.toString(),
        [error]
      );
    }
...
const validateComposites = function (luisJSON, entitiesList) {
  ...
  luisJSON.composites.forEach((composite) => {
    ...
    let patternAnyEntityInComposite = (luisJSON.patternAnyEntities || []).find(
      (patternAnyEntity) => {
...
const validateCompositeChildren = function (composite, entitiesList) {
  composite.children.forEach((child) => {
    ...
    let compositeChildEntityFound = (entitiesList || []).find(
      (entity) => entity.name == childEntityName
    );
...
const verifyAndExtractFeatures = function (
  collection = [],
  verifyList = [],
  featureCandidates = [],
  verifiedList = []
