...
const build = async function (luArray, verbose, luis_culture, luSearchFn) {
  let mergedContent = await mergeLuFiles(
    luArray,
    verbose,
    luis_culture,
    luSearchFn
  );
  let parsedLUISList = mergedContent.LUISContent.filter(
    (item) => item.includeInCollate
  );
...
const collate = function (luisList) {
  ...
  for (let i = 1; i < luisList.length; i++) {
    ...
    mergeResultsWithHash(
      blob,
      luisObject,
      LUISObjNameEnum.UTTERANCE,
      hashTable
    );
...
const cleanupEntities = function (luisObject) {
  ...
  luisObject.entities.forEach((item, idx) => {
    if (consolidatedList.find((e) => e.name == item.name) !== undefined)
      idxToRemove.push(idx);
  });
  idxToRemove
    .sort((a, b) => a - b)
    .forEach((idx) => luisObject.entities.splice(idx, 1));
    ...
    if (!hashTable[hashCode]) {
      ...
      if (
        ...
        (blobItem.roles || []).forEach((blobRole) => {
          if (item.roles && !item.roles.includes(blobRole)) {
...
const mergeNDepthEntities = function (blob, finalCollection) {
  ...
  nDepthInBlob.forEach((item) => {
    let itemExistsInFinal = (finalCollection || []).find(
      (x) => x.name == item.name
    );
    if (itemExistsInFinal === undefined) {
      ...
      if (
        ...
        recursivelyMergeChildrenAndFeatures(
          item.children,
...
const recursivelyMergeChildrenAndFeatures = function (
  ...
  if (
    tgtChildren === undefined ||
    !Array.isArray(tgtChildren) ||
    tgtChildren.length === 0
  ) {
  ...
  (srcChildren || []).forEach((item) => {
    // find child in tgt
    let itemExistsInFinal = (tgtChildren || []).find(
      (x) => x.name == item.name
    );
    if (itemExistsInFinal === undefined) {
      ...
      if (item.features !== undefined && item.features.length !== 0) {
        ...
        let typeForFinalItem = (itemExistsInFinal.features || []).find(
          (t) => t.isRequired == true
        );
        let typeForItem = (item.features || []).find(
          (t1) => t1.isRequired == true
        );
          ...
          if (typeForItem !== undefined) {
            if (typeForFinalItem.modelName !== typeForItem.modelName) {
              throw new exception(
                ...
                `Child entity ${item.name} does not have consistent type definition. Please verify all definitions for this entity.`
              );
            }
          }
        }
        item.features.forEach((f) => {
          let featureInFinal = (itemExistsInFinal.features || []).find(
            (itFea) => {
              return (
                (itFea.featureName !== undefined &&
                  itFea.featureName == f.featureName) ||
                (itFea.modelName !== undefined &&
                  itFea.modelName == f.modelName)
              );
            }
          );
          if (featureInFinal === undefined) {
            ...
            if (featureInFinal.isRequired !== f.isRequired) {
              throw new exception(
                retCode.errorCode.INVALID_REGEX_ENTITY,
                `Feature ${f.featureName} does not have consistent definition for entity ${item.name}. Please verify all definitions for this feature for this entity.`
              );
            }
          }
        });
...
const mergeResults = function (blob, finalCollection, type) {
  ...
  blob[type].forEach(function (blobItem) {
    ...
    for (let fIndex in finalCollection[type]) {
      ...
      (blobItem.roles || []).forEach((blobRole) => {
        if (
          finalCollection[type][fIndex].roles &&
...
const mergeResults_closedlists = function (blob, finalCollection, type) {
  ...
  blob[type].forEach(function (blobItem) {
    let listInFinal = helpers.filterMatch(
      finalCollection[type],
      'name',
      blobItem.name
    );
    if (listInFinal.length === 0) {
      ...
      blobItem.subLists.forEach(function (blobSLItem) {
        ...
        let slInFinal = helpers.filterMatch(
          listInFinal[0].subLists,
          'canonicalForm',
          blobSLItem.canonicalForm
        );
        if (slInFinal.length === 0) {
          ...
          blobSLItem.list.forEach(function (listItem) {
            if (!slInFinal[0].list.includes(listItem))
              slInFinal[0].list.push(listItem);
          });
        }
      });
...
const buildRegex = function (blob, FinalLUISJSON) {
  ...
  blob.regex_entities.forEach(function (regexEntity) {
    ...
    let entityExistsInFinal = (FinalLUISJSON.regex_entities || []).find(
      (item) => item.name == regexEntity.name
    );
    if (entityExistsInFinal === undefined) {
      ...
      if (entityExistsInFinal.regexPattern !== regexEntity.regexPattern) {
        throw new exception(
          retCode.errorCode.INVALID_REGEX_ENTITY,
          `[ERROR]: RegEx entity : ${regexEntity.name} has inconsistent pattern definitions. \n 1. ${regexEntity.regexPattern} \n 2. ${entityExistsInFinal.regexPattern}`
        );
      }
      // merge roles
...
const buildPrebuiltEntities = function (blob, FinalLUISJSON) {
  ...
  if (
    blob.prebuiltEntities === undefined ||
...
const buildModelFeatures = function (blob, FinalLUISJSON) {
  ...
  blobScope.forEach(function (modelFeature) {
    let modelFeatureInMaster = helpers.filterMatch(
      finalScope,
      'name',
      modelFeature.name
    );
    if (modelFeatureInMaster.length === 0) {
      ...
      if (modelFeatureInMaster[0].mode !== modelFeature.mode) {
        ...
        throw new exception(
          ...
          '[ERROR]: Phrase list : "' +
            modelFeature.name +
            '" has conflicting definitions. One marked interchangeable and another not interchangeable'
        );
      } else {
        modelFeature.words.split(',').forEach(function (word) {
          if (!modelFeatureInMaster[0].words.includes(word))
            modelFeatureInMaster[0].words += ',' + word;
        });
      }
    }
  });
...
const buildComposites = function (blob, FinalLUISJSON) {
  ...
  (blob.composites || []).forEach((composite) => {
    let compositeInMaster = helpers.filterMatch(
      FinalLUISJSON.composites,
      'name',
      composite.name
    );
    if (compositeInMaster.length === 0) {
      ...
      if (
        JSON.stringify(composite.children.sort()) !==
        JSON.stringify(compositeInMaster[0].children.sort())
      ) {
        composite.children.forEach((child) => {
          if (!compositeInMaster[0].children.includes(child))
            compositeInMaster[0].children.push(child);
        });
      } else {
        // merge roles
...
const buildPatternAny = function (blob, FinalLUISJSON) {
  ...
  (blob.patternAnyEntities || []).forEach((patternAny) => {
    let paIdx = -1;
    let patternAnyInMaster = FinalLUISJSON.patternAnyEntities.find(
      (item, idx) => {
    ...
    let simpleEntityInMaster = FinalLUISJSON.entities.find(
      (item) => item.name == patternAny.name
    );
    let compositeInMaster = FinalLUISJSON.composites.find(
      (item) => item.name == patternAny.name
    );
    let listEntityInMaster = FinalLUISJSON.closedLists.find(
      (item) => item.name == patternAny.name
    );
    let regexEntityInMaster = FinalLUISJSON.regex_entities.find(
      (item) => item.name == patternAny.name
    );
    let prebuiltInMaster = FinalLUISJSON.prebuiltEntities.find(
      (item) => item.name == patternAny.name
    );
    if (
      ...
      if (patternAnyInMaster) {
        (patternAny.roles || []).forEach((role) =>
          !patternAnyInMaster.roles.includes(role)
            ? patternAnyInMaster.roles.push(role)
            : undefined
        );
      } else {
...
const initializeHash = function (LuisJSON, hashTable = undefined) {
  for (let prop in LuisJSON) {
    if (
      hashTable !== undefined &&
      (prop === LUISObjNameEnum.UTTERANCE || prop === LUISObjNameEnum.PATTERNS)
    ) {
      (LuisJSON[prop] || []).forEach(
        (item) => (hashTable[helpers.hashCode(JSON.stringify(item))] = item)
      );
    }
  }
};
