...
const build = async function (qnaObjArray, verbose, qnaSearchFn) {
  ...
  let parsedQnAList = mergedContent.QnAContent.filter(
  ...
  let allParsedQnAAlterations = mergedContent.QnAAlterations.filter(
    (item) => item.includeInCollate
  );
...
const resolveMultiTurnReferences = function (qnaList) {
  let qnaPairsWithMultiTurn = qnaList.filter(
    (item) => item.context.prompts.length !== 0
  );
  // find the largetst auto-id
  let largestAutoIdxList = qnaList.filter(
    (item) => item.id !== 0 && item.id.toString().startsWith('*auto*')
  );
  let largestAutoIdx = 0;
  if (largestAutoIdxList.length !== 0) {
    let idx = largestAutoIdxList.reduce(function (max, obj) {
      return parseInt(obj.id.replace('*auto*', '')) >
        parseInt(max.id.replace('*auto*', ''))
        ? obj
        : max;
    });
  ...
  (qnaPairsWithMultiTurn || []).forEach((item) => {
    ...
    (item.context.prompts || []).forEach((prompt) => {
      // find by ID first
      let qnaId = qnaList.find(
        (x) => x.id === prompt.qnaId || x.id === parseInt(prompt.qnaId)
      );
      if (!qnaId) {
        // find by question match
        qnaId = qnaList.find(
          (x) =>
            x.source.trim() !== 'crosstrained' &&
            (x.questions.includes(prompt.qnaId) ||
              x.questions.includes(prompt.qnaId.replace(/-/g, ' ').trim()))
        );
      }
      if (qnaId === undefined) {
        throw new exception(
          retCode.INVALID_INPUT,
          `[ERROR]: Cannot find follow up prompt definition for '- [${prompt.displayText}](#?${prompt.qnaId}).`
        );
      } else {
        if (qnaId.id === 0) {
        ...
        prompt.qna = null;
        qnaId.context.isContextOnly = !qnaId.context.isContextOnly
          ? prompt.contextOnly
          : true;
...
const resolveQnAIds = function (qnaList) {
  ...
  let qnasWithId = qnaList.filter(
    (pair) => pair.id !== 0 && !pair.id.toString().startsWith('*auto*')
  );
  qnasWithId.forEach((qna) => {
    ...
    if (isNaN(qna.id))
      throw new exception(
        retCode.INVALID_INPUT,
        `[Error]: Explicitly assigned QnA Ids must be numbers. '${qna.id}' is not a number.`
      );
  ...
  let qnasWithAutoId = qnaList.filter(
    (pair) =>
      pair.id !== 0 && isNaN(pair.id) && pair.id.toString().startsWith('*auto*')
  );
...
const collateFiles = function (instance, qnaObject) {
  ...
  qnaObject.files.forEach(function (qnaFile) {
    if (
      instance.files.filter((item) => {
        return item.fileUri == qnaFile.fileUri;
      }).length === 0
    ) {
