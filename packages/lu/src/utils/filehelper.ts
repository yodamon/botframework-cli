...
export async function getLuObjects(
  stdin: string,
...
export async function getLuFiles(
  input: string | undefined,
  recurse = false,
  extType: string | undefined
): Promise<Array<any>> {
  ...
  if (!fileStat.isDirectory()) {
    throw new exception(
      ...
      'Sorry, ' + input + ' is not a folder or does not exist'
    );
  }
...
export async function getContentFromFile(file: string) {
  ...
  if (fs.lstatSync(file).isDirectory()) {
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      'Sorry, "' + file + '" is a directory! Unable to read as a file'
    );
  }
  ...
  try {
    ...
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      'Sorry, error reading file: ' + file
    );
  }
  return fileContent;
}

export async function generateNewFilePath(
  outFileName: string,
  inputfile: string,
  isLu: boolean,
  prefix = '',
  extType: string = helpers.FileExtTypeEnum.LUFile
): Promise<string> {
...
export async function generateNewTranslatedFilePath(
  fileName: string,
  ...
  if (extension) {
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      'Output can only be writen to a folder'
    );
  }
...
export function validatePath(
  ...
  if (!fs.existsSync(containingDir))
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      `Containing directory path doesn't exist: ${containingDir}`
    );
  ...
  if (baseElement.includes('.')) {
    return pathAlreadyExist && !forceWrite
      ? enumerateFileName(completePath)
      : completePath;
  }
  ...
  if (!pathAlreadyExist)
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      `Target directory path doesn't exist: ${completePath}`
    );
  completePath = path.join(completePath, defaultFileName);
  return fs.existsSync(completePath) && !forceWrite
    ? enumerateFileName(completePath)
    : completePath;
}
...
export async function detectLuContent(stdin: string, input: string) {
  if (!stdin && !input) {
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      'Missing input. Please use stdin or pass a file location with --in flag'
    );
  }
  ...
  if (!stdin) {
    if (!fs.existsSync(path.resolve(input))) {
      throw new exception(
        retCode.errorCode.INVALID_INPUT_FILE,
        `Sorry unable to open [${input}]`
      );
    }

    let inputStat = await fs.stat(input);
    return !inputStat.isFile()
      ? true
      : path.extname(input) === '.lu' || path.extname(input) === '.qna';
  }
...
async function getConfigFile(input: string): Promise<string> {
  ...
  if (defaultConfigFile === undefined || defaultConfigFile === '') {
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      `Sorry, no config file found in folder ${input}.`
    );
  }
...
export function getConfigObject(
  ...
  if (luConfigContent && luConfigContent !== '') {
    try {
        ...
        for (const triggerObjKey of Object.keys(triggerObj)) {
          if (triggerObjKey === 'rootDialog') {
            ...
            for (const triggerKey of Object.keys(triggers)) {
              const destLuFiles =
                triggers[triggerKey] instanceof Array
                  ? triggers[triggerKey]
                  : [triggers[triggerKey]];
              for (const destLuFile of destLuFiles) {
                const destLuFileFullPath =
                  destLuFile && destLuFile !== ''
                    ? path.resolve(configFileDir, destLuFile)
                    : destLuFile;
                if (rootLuFileFullPath in finalLuConfigObj) {
                  const finalIntentToDestLuFiles =
                    finalLuConfigObj[rootLuFileFullPath];
                  if (finalIntentToDestLuFiles[triggerKey]) {
                    finalIntentToDestLuFiles[triggerKey].push(
                      destLuFileFullPath
                    );
                  } else {
      ...
      throw new exception(
        retCode.errorCode.INVALID_INPUT_FILE,
        `Sorry, invalid cross training config: ${err}`
      );
    }
  }
  ...
  if (rootLuFiles.length > 0) {
    ...
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      'rootDialog property is required in config file'
    );
  }
}
...
export function parseJSON(input: string, appType: string) {
  try {
    ...
    throw new exception(
      retCode.errorCode.INVALID_INPUT_FILE,
      `Sorry, error parsing content as ${appType} JSON`
    );
  }
}
  ...
  for (const content of contents) {
    ...
    if (
      resource.Sections.filter(
        (s: any) => s.SectionType !== LUSectionTypes.MODELINFOSECTION
      ).length > 0
    ) {
