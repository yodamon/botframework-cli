...
const rootDir = path.join(
  __dirname,
  './../../fixtures/testcases/import-resolver/qna-import-resolver'
);
  ...
  it('should load contents sucessfully after resolving imports', async () => {
    let importResolver = async function (srcId, idsToFind) {
      ...
      for (let idx = 0; idx < idsToFind.length; idx++) {
        ...
        if (
          file.filePath.endsWith('.qna') &&
          !file.filePath.endsWith('en-us.qna')
        ) {
          file.filePath =
            file.filePath.slice(0, file.filePath.length - 3) + 'en-us.qna';
        }

        luObjects.push(
          new qnaObject(
            txtfile.readSync(file.filePath),
            new qnaOptions(file.filePath, file.includeInCollate)
          )
        );
      }
      return luObjects;
    ...
    assert.isTrue(
      result.qnaContents[0].content.includes(
...
describe('builder: build function can catch relative endpoint exception successfully', () => {
  it('should throw exception for non absolute endpoint', async () => {
    ...
    try {
      await builder.build(
        [
          new qnaObject(
            `# ? Greeting${NEWLINE}\`\`\`${NEWLINE}hello${NEWLINE}\`\`\``
          ),
        ],
        undefined,
      ...
      assert.equal(
        e.text,
        `Only absolute URLs are supported. "http:fsd" is not an absolute qnamaker endpoint URL.`
      );
    }
  });
  ...
  it('should throw exception for non absolute endpoint', async () => {
    ...
    try {
      ...
      assert.equal(
        e.text,
        `Only absolute URLs are supported. "fsd" is not an absolute qnamaker endpoint URL.`
      );
    }
  });
});
