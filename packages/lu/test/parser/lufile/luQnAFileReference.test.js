...
- [all answers](qna1#*answers*?)
        `;
    luMerger
      .Build([new luObj(luContent)], false, undefined, findLuFiles)
      .then((res) => {
        assert.equal(res.LUISContent[0].LUISJsonStructure.utterances.length, 2);
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.utterances[0].text,
          'answer1'
        );
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.utterances[1].text,
          'answer2'
        );
        done();
      })
...
- [all.qna](qna2#?-book-flight?)
    ...
    luMerger
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.utterances[3].text,
          "I'm looking to fly"
        );
        done();
      })
...
- [all.qna](qna3#*alterations*?)
    ...
    luMerger
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.utterances[2].text,
...
- [all.lu](lu1#*utterancesandpatterns*)
    ...
    luMerger
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.patterns[0].pattern,
          'pattern {one}'
        );
        done();
      })
...
- [all.lu](lu1#*patterns*)
    ...
    luMerger
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISContent[0].LUISJsonStructure.patterns[0].intent,
          'intent1'
        );
...
- [refl1](refl1#*)`;
    luMerger
      .Build(
        [new luObj(luContent, new luOptions('tloop', true))],
        false,
        undefined,
        findLuFiles
      )
...
@ ml test1 usesFeature phraseList1
    ...
    luisBuilder
      .fromLUAsync(
        [new luObj(luContent, new luOptions('main.lu', true))],
        findLuFiles
      )
      .then((res) => {
...
- pl 2
- pl 2 2`,
            new luOptions(`phrases.lumodule`, true)
          )
        );
        break;
      case 'qna1':
