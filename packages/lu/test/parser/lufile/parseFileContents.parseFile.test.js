...
- phone call`;
    parseFile
      .parseFile(luFile, false, 'en-us')
      .then(function (parsedContent) {
        validateLUISBlob(parsedContent.LUISJsonStructure)
          .then(() =>
            done('Test fail. validateLUISBlob did not throw when expected!')
          )
          .catch(() => done());
      })
      .catch(() => done());
      ...
      .then(function (parsedContent) {
        try {
          validateLUISBlob(parsedContent.LUISJsonStructure);
            ...
            'Test fail. validateLUISBlob did not throw when expected!' + error
          );
        }
      })
      .catch((err) =>
        done('Test fail. validateLUISBlob did not throw when expected!' + err)
      );
  });
...
`;
    parseFile
      ...
      .catch(() =>
        done('Test fail. parseFile threw when it was not expected!')
      );
  });
  ...
  it('Test for #1166 ', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        assert.equal(res.LUISJsonStructure.closedLists.length, 1);
        assert.equal(
          res.LUISJsonStructure.closedLists[0].name,
          'Country|Office'
        );
        assert.equal(
          res.LUISJsonStructure.closedLists[0].subLists[0].canonicalForm,
  ...
  it('correctly parses an utterance with one labelled entity', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[0].startPos,
          9
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[0].endPos,
          14
        );
        assert.equal(res.LUISJsonStructure.entities.length, 1);
        done();
  ...
  it('correctly parses a pattern with pattern.any entity', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        assert.equal(res.LUISJsonStructure.patterns.length, 1);
        assert.equal(
          res.LUISJsonStructure.patterns[0].pattern,
          'I want {foodType}'
        );
  ...
  it('correctly parses an utterance with multiple labelled entities', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].text,
          'I want a tomato and orange'
        );
        assert.equal(res.LUISJsonStructure.utterances[0].entities.length, 2);
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[1].startPos,
          20
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[1].endPos,
          25
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[0].startPos,
  ...
  it('correctly parses an utterance with single curly bracket', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[1].text,
          'userName= vishwac}'
        );
        done();
      })
  ...
  it('correctly parses utterance with composite entities with multiple children', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].text,
          'I want another shirt and pant please'
        );
        assert.equal(res.LUISJsonStructure.composites.length, 1);
        assert.equal(res.LUISJsonStructure.composites[0].name, 'productOrder');
        assert.deepEqual(res.LUISJsonStructure.composites[0].children, [
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[2].startPos,
          7
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[2].endPos,
          35
        );
        done();
      })
  ...
  it('correctly parses utterance with composite entities', function (done) {
    ...
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].text,
          'I want x y and a with tomato and orange'
        );
        assert.equal(res.LUISJsonStructure.composites.length, 1);
        assert.equal(res.LUISJsonStructure.composites[0].name, 'p');
        assert.deepEqual(res.LUISJsonStructure.composites[0].children, [
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[3].startPos,
          22
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[3].endPos,
          27
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[4].startPos,
          33
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[4].endPos,
          38
        );
        done();
      })
...
`;
    parseFile
      ...
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].text,
          'no, i already have meeting 3pm tomorrow afternoon'
        );
        assert.equal(res.LUISJsonStructure.utterances[0].entities.length, 2);
        assert.equal(
        ...
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[1].entity,
          'FromDate'
        );
        assert.equal(
          res.LUISJsonStructure.utterances[0].entities[1].startPos,
