...
$deviceTemperature : [foo]`;
  ...
  it('Parser correctly parses composite entity definition with comma delimiter', function (done) {
    ...
    let testCompositeEntity = new hClasses.compositeEntity(
      'deviceTemperature',
      ['child1', 'child2']
    );
    parseFile(luFileContent, false)
      .then((res) => {
        assert.deepEqual(
          res.LUISJsonStructure.composites[0],
          testCompositeEntity
        );
        done();
      })
...
$deviceTemperature:simple`;
  ...
  @ ml outer 
    ...
    assert.deepEqual(
      parsedLUContent.utterances[0].entities[0].children[0].entity,
      'inner'
    );
    assert.deepEqual(
      parsedLUContent.utterances[0].entities[0].children[0].startPos,
      20
    );
    assert.deepEqual(
      parsedLUContent.utterances[0].entities[0].children[0].endPos,
      23
    );
  });
