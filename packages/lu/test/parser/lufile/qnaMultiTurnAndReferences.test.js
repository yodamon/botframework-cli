  ...
  describe('Negative tests', function () {
    it('QnA deep reference cannot pull in patterns.', function (done) {
      let qnaContent = `# ? [test](lu1#intent1)
            ...
            `;
      luMerger
        .Build([new luObj(qnaContent)], false, undefined, findLuFiles)
        .then((res) => done(res))
        .catch((err) => {
    ...
    it('Missing deep reference throws', function (done) {
      ...
      luMerger
        .Build(
          [new luObj(qnaContent, new luOptions('stdin'))],
          false,
          undefined,
          findLuFiles
        )
        ...
        .catch((err) => {
          assert(
            err.text.includes('Cannot find reference.') ||
              err.text.includes('Sorry unable to open')
          );
          done();
        });
    });
    ...
    it('Filters must be a list', function (done) {
      ...
      luMerger
        ...
        .catch((err) => {
          assert(
            err.text.includes('line 7:0 - line 8:12: Invalid QnA filter line')
          );
          done();
        });
    });
    ...
    it("'markdown' is optional for answer", function (done) {
      ...
      luMerger
        .Build(
          [new luObj(qnaContent1, new luOptions('stdin'))],
          false,
          undefined,
          findLuFiles
        )
        .then((res) => {
          luMerger
            .Build(
              [new luObj(qnaContent2, new luOptions('stdin'))],
              false,
              undefined,
              findLuFiles
            )
