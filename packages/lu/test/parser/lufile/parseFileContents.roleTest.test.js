  ...
  it('In new entity notation, prebuilt types do not need to be defined to add roles', function (done) {
    ...
    parseFile(fileContent, false, null)
      .then((res) => {
        ...
        assert.equal(
          res.LUISJsonStructure.prebuiltEntities[0].roles[0],
          'userAge'
        );
        done();
      })
...
$test:[fromTime]`;
  ...
  it('simple entity definitions with roles are collated correctly', function (done) {
    ...
    parseFile(testLU, false, null)
      .then((res) => {
        ...
        assert.deepEqual(LUISJSon.entities[0].roles, [
          'firstname',
          'lastname',
          'middlename',
        ]);
  ...
  it('prebuilt entity definitions with roles are parsed correctly', function (done) {
    ...
    parseFile(testLU, false, null)
      .then((res) => {
        ...
        assert.deepEqual(LUISJSon.prebuiltEntities[0].roles, [
          'fromDate',
          'toDate',
        ]);
        done();
  ...
  it('List entities with explicit role definitions are parsed correctly', function (done) {
    ...
    parseFile(testLU, false, null)
      .then((res) => {
        ...
        assert.equal(
          LUISJson.closedLists[0].subLists[0].canonicalForm,
          'seattle'
        );
  ...
  it('RegEx entities with explicit role definitions are parsed correctly', function (done) {
    ...
    parseFile(testLU, false, null)
      .then((res) => {
        ...
        assert.deepEqual(LUISJson.regex_entities[0].roles, [
          'fromNumber',
          'toNumber',
        ]);
        done();
  ...
  it('parser correctly throws for Pharse lists with explicit role definition', function (done) {
    ...
    parseFile(testLU, false, null)
      .then((res) =>
        done(`Test failed. Did not throw when expected- ${JSON.stringify(err)}`)
      )
      .catch((err) => done());
  });
        ...
        parseFile(testLU2, false)
          .then((res2) => {
            try {
              ...
              assert.deepEqual(luisObj.entities[0].roles, [
                'firstName',
                'lastName',
              ]);
              done();
        ...
        parseFile(testLU2, false)
          .then((res2) => {
            try {
              ...
              assert.deepEqual(luisObj.prebuiltEntities[0].roles, [
                'fromDate',
                'toDate',
              ]);
              done();
        ...
        parseFile(testLU2, false)
          .then((res2) => {
            try {
              ...
              assert.deepEqual(luisObj.closedLists[0].roles, [
                'fromCity',
                'toCity',
              ]);
              done();
        ...
        parseFile(testLU2, false)
          .then((res2) => {
            try {
              ...
              assert.deepEqual(luisObj.regex_entities[0].roles, [
                'fromNumber',
                'toNumber',
              ]);
              done();
        ...
        parseFile(testLU2, false)
          .then((res2) => {
            try {
              ...
              assert.deepEqual(luisObj.patternAnyEntities[0].roles, [
                'fromTime',
                'toTime',
              ]);
              done();
  ...
  it('Ludown refresh successfully converts a luis model with role definition into a .lu file', function (done) {
    ...
    try {
      ...
      assert.isTrue(
        res.includes(`@ composite location hasRoles fromCity,toCity = [city]`)
      );
      done();
    } catch (error) {
