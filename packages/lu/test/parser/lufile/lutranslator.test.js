...
describe('With the parseAndTranslate method', function () {
  it('Translating comments can be skipped', function (done) {
    translate
      .parseAndTranslate(
        `> This is a comment`,
        TRANSLATE_KEY,
        testData.tests.intentsAndUtterancesNC.langCode,
        '',
        false,
        false,
        false
      ...
      .then(function (res) {
        assert.equal(
          helpers.sanitizeNewLines(res),
          helpers.sanitizeNewLines(`> This is a comment\n`)
        );
        done();
      })
      .catch((err) => done(err));
  ...
  it('Bad lu file input throws', function (done) {
    ...
    translate
      .parseAndTranslate(
        testData.tests.badLu.luFile,
        TRANSLATE_KEY,
        testData.tests.badLu.langCode,
        '',
        false,
        false,
        false
  ...
  xit('References can be skipped from being translated', async function (done) {
    ...
    await translate
      .parseAndTranslate(
        testData.tests.fileRef.luFile,
        TRANSLATE_KEY,
        testData.tests.fileRef.langCode,
        '',
        false,
        false,
        false
      ...
      .then(function (res) {
        assert.equal(
          helpers.sanitizeNewLines(res),
          helpers.sanitizeNewLines(testData.tests.fileRef.luFile + NEWLINE)
        );
        done();
      })
      .catch((err) => done(err));
  ...
  xit('Invalid key throws', async function (done) {
    ...
    try {
      await translate.parseAndTranslate(
        `# Greeting
            -hi
            `,
        TRANSLATE_KEY + '2',
