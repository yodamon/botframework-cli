...
describe('Validations for LU content (based on LUIS boundaries)', function () {
  ...
  it(`At most ${retCode.boundaryLimits.MAX_CHAR_IN_UTTERANCE} characters in any utterance`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(new luObj(getMaxUtteranceCharLimit(), 'stdin', true))
    )
      .then((res) => done(res))
      .catch((err) => {
        assert.equal(
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_CHAR_IN_UTTERANCE} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_PATTERNS} patterns in LU content`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(new luObj(getMaxPatterns(), 'stdin', true))
    )
      .then((res) => done(res))
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `patterns found in application. At most ${retCode.boundaryLimits.MAX_NUM_PATTERNS} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_CHAR_IN_PATTERNS} characters in any pattern`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(new luObj(getMaxPatternCharLimit(), 'stdin', true))
    )
      .then((res) => done(res))
      .catch((err) => {
        assert.equal(
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_CHAR_IN_PATTERNS} characters are allowed in any pattern.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES} regex entities`, function (done) {
    LuisBuilder.fromLUAsync(
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_CHAR_REGEX_ENTITY_PATTERN} characters in regex entity pattern`, function (done) {
    LuisBuilder.fromLUAsync(
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_CHAR_REGEX_ENTITY_PATTERN} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_LIST_ENTITY_SYNONYMS} synonyms under any parent for a list entity`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(
        new luObj(
          getListEntity(0, retCode.boundaryLimits.MAX_LIST_ENTITY_SYNONYMS),
          'stdin',
          true
        )
      )
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_LIST_ENTITY_SYNONYMS} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_PHRASE_LISTS} phrase lists`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(new luObj(getMaxPhraseLists(), 'stdin', true))
    )
      .then((res) => done(res))
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_PHRASE_LISTS} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_INTERCHANGEABLE_PHRASES} phrases across all interchangeable phrase lists`, function (done) {
    LuisBuilder.fromLUAsync(
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_INTERCHANGEABLE_PHRASES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NON_INTERCHANGEABLE_PHRASES} phrases across all non-interchangeable phrase lists`, function (done) {
    LuisBuilder.fromLUAsync(
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NON_INTERCHANGEABLE_PHRASES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_ROLES_PER_ENTITY} roles per entity`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getEntityWithRoles())))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_ROLES_PER_ENTITY} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_ROLES} roles across all entities per LU content`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getEntityWithRoles(51, 6))))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_ROLES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_DESCRIPTORS_PER_MODEL} descriptors per model`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getEntityWithFeatures())))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_DESCRIPTORS_PER_MODEL} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_PARENT_ENTITIES} parent nodes in an ML entitiy`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getMLEntity())))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_PARENT_ENTITIES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_TOTAL_ENTITES_AND_ROLES} total entities and roles in given LU content`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getMaxEntityAndRoles())))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_TOTAL_ENTITES_AND_ROLES} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_CLOSED_LISTS} list entities`, function (done) {
    LuisBuilder.fromLUAsync(new Array(new luObj(getMaxListEntity())))
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_CLOSED_LISTS} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At most ${retCode.boundaryLimits.MAX_NUM_PHRASES_IN_ALL_PHRASE_LIST} phrases across all phrase lists`, function (done) {
    LuisBuilder.fromLUAsync(
      ...
      .catch((err) => {
        ...
        assert(
          err.text.includes(
            `At most ${retCode.boundaryLimits.MAX_NUM_PHRASES_IN_ALL_PHRASE_LIST} is allowed.`
          )
        );
        done();
      });
  ...
  it(`At Least one phrase in any phrase list`, function (done) {
    LuisBuilder.fromLUAsync(
      new Array(new luObj(getMaxPhraseLists(1, 0), 'stdin', true))
    )
      .then((res) => done(res))
      .catch((err) => {
        assert.equal(
        ...
        assert(
          err.text.includes(
            `0 phrases found in phrase list: PL0. Empty phrase list is not allowed.`
          )
        );
        done();
      });
...
@ ml entityBase${i} hasRoles role${i}`;
  ...
  for (
    var j = retCode.boundaryLimits.MAX_NUM_ROLES;
    j <= retCode.boundaryLimits.MAX_TOTAL_ENTITES_AND_ROLES;
    j++
  ) {
    fc += `
@ ml entityFillUp${j}`;
...
const getEntityWithFeatures = function () {
  ...
  for (
    var i = 0;
    i <= retCode.boundaryLimits.MAX_NUM_DESCRIPTORS_PER_MODEL;
    i++
  ) {
...
const getEntityWithRoles = function (
  numEntities = 1,
  rolesPerEntity = retCode.boundaryLimits.MAX_ROLES_PER_ENTITY
) {
  let fc = '';
  let roleCounter = 1;
...
const getMaxPhraseLists = function (
  numPhraseLists = retCode.boundaryLimits.MAX_NUM_PHRASE_LISTS,
  numPhrases = null,
  intc = null
) {
  let fc = '';
...
const getListEntity = function (
  numParents = retCode.boundaryLimits.MAX_LIST_ENTITY_CANONICAL_FORM,
  numSynonyms = 0
) {
  let fc = `
@ list entity1 = 
`;
  for (var i = 0; i <= numParents; i++) {
    ...
    for (var j = 0; j < numSynonyms; j++) {
      fc += `synonym${i}.${j}.${POSSIBLE_CHARS.charAt(
...
const getMaxRegeExEntityDefinition = function (
  entityNum = retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES,
  charLimit = retCode.boundaryLimits.MAX_NUM_REGEX_ENTITIES
) {
  let fc = '';
...
const getMaxPatternAnyEntities = function () {
  ...
  for (
    var i = 0;
    i <= retCode.boundaryLimits.MAX_NUM_PATTERNANY_ENTITIES;
    i++
  ) {
    fc += `
@ patternany entity${i}`;
...
- `;
  for (
    var i = 0;
    i <= retCode.boundaryLimits.MAX_CHAR_IN_UTTERANCE + 100;
    i++
  ) {
    fc += POSSIBLE_CHARS.charAt(
      Math.floor(Math.random() * POSSIBLE_CHARS.length)
