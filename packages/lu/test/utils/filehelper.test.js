...
describe('utils/filehelper test', () => {
  ...
  it('File helper correctly build cross train config object', function () {
    ...
    let configObject = fileHelper.getConfigObject(
      {
        id: path.join(__dirname, 'config.json'),
    ...
    assert.equal(
      triggerRuleValues[0]['dia1_trigger'][0].includes('dia1.lu'),
      true
    );
    assert.equal(
      triggerRuleValues[0]['dia1_trigger'][1].includes('dia2.lu'),
      true
    );
    assert.equal(triggerRuleValues[1]['dia3_trigger'][0], '');
    assert.equal(triggerRuleValues[1][''][0].includes('dia4.lu'), true);
    assert.equal(
      triggerRuleValues[2]['dia1_trigger'][0].includes('dia1.fr-fr.lu'),
      true
    );
