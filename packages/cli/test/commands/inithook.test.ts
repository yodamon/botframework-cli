...
describe('Check if telemetry is not changed if config is not null', () => {
  beforeEach(function () {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ telemetry: true }, null, 2)
    );
  });
...
describe('Update available to stdout', () => {
  beforeEach(function () {
    ...
    fs.writeFileSync(
      path.join(pathToConfigJsonUpdate, 'config.json'),
      JSON.stringify({ telemetry: true }, null, 2)
    );
  });
  ...
  test
    ...
    .it(
      'it should write to message stdout if update is available and last checked timestamp is expired'
    );
});
...
describe("bypass version update if it's already been checked today", async () => {
  ...
  beforeEach(async () => {
    ...
    let userConfig = await fs.readJSON(
      path.join(lastversioncheck, 'package.json')
    );
