...
export default class ConfigSetLuis extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    authoringKey: flags.string({
      description:
        'LUIS cognitive services authoring key (aka Ocp-Apim-Subscription-Key).',
    }),
    subscriptionKey: flags.string({
      description:
        'LUIS cognitive services subscription key (aka Ocp-Apim-Subscription-Key)',
    }),
    versionId: flags.string({ description: 'LUIS version Id' }),
    endpoint: flags.string({
      description:
        'LUIS application endpoint hostname, ex: <region>.api.cognitive.microsoft.com',
    }),
  };
