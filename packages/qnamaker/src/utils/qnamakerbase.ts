...
export async function updateQnAMakerConfig(config: any, configfile: string) {
  ...
  userConfig.qnamaker__subscriptionKey = config.subscriptionKey
    ? config.subscriptionKey
    : userConfig.qnamaker__subscriptionKey;
  userConfig.qnamaker__kbId = config.kbId
    ? config.kbId
    : userConfig.qnamaker__kbId;
  userConfig.qnamaker__endpointKey = config.endpointKey
    ? config.endpointKey
    : userConfig.qnamaker__endpointKey;
  userConfig.qnamaker__hostname = config.hostname
    ? config.hostname
    : userConfig.qnamaker__hostname;
  await fs.writeJson(path.join(configfile, 'config.json'), userConfig, {
    spaces: 2,
  });
}
...
export async function processFlags(
  ...
  let config = filterByAllowedConfigValues(
    await getUserConfig(configDir),
    configPrefix
  );
  ...
  flagLabels
    ...
    .map((flag: string) => {
      ...
      input[flag] =
        flags[flag] || (config ? config[configPrefix + flag] : null);
    });
