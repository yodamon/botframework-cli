...
export default class QnamakerTranslate extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description: 'Source .qna file(s) or QnA maker application JSON model',
    }),
    recurse: flags.boolean({
      char: 'r',
      description:
        'Indicates if sub-folders need to be considered to find .qna file(s)',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Output folder name. If not specified stdout will be used as output',
    }),
    ...
    translate_comments: flags.boolean({
      description: 'When set, machine translate comments found in .qna file',
    }),
    translate_link_text: flags.boolean({
      description: 'When set, machine translate link description in .qna file',
    }),
    force: flags.boolean({
      char: 'f',
      description:
  ...
  async run() {
    try {
      ...
      let result: any = {};
      if (isLu) {
        const luFiles = await fileHelper.getLuObjects(
          stdin,
          flags.in,
          flags.recurse,
          fileExtEnum.QnAFile
        );
        const translatedLuFiles = await luTranslator.translateQnAList(
          luFiles,
          flags.translatekey,
          flags.tgtlang,
          flags.srclang,
          flags.translate_comments,
          flags.translate_link_text
        );
        ...
        const json = flags.in
          ? await fileHelper.getContentFromFile(flags.in)
          : stdin;
        const qnaM = new QnaMaker(fileHelper.parseJSON(json, 'QnA'));
        const qna = new QnA(qnaM.parseToLuContent());
        const qnaTranslation = await luTranslator.translateQnA(
          qna,
          flags.translatekey,
          flags.tgtlang,
          flags.srclang,
    ...
    try {
      for (const file in translatedObject) {
        for (const lng in translatedObject[file]) {
          filePath = await fileHelper.generateNewTranslatedFilePath(
            file,
            lng,
            out
          );
          const content = isLu
            ? translatedObject[file][lng]
            : JSON.stringify(translatedObject[file][lng], null, 2);
      ...
      throw new CLIError(
        'Unable to write file - ' + filePath + ' Error: ' + error.message
      );
    }
  }
}
