...
import {
  Inputs,
  processInputs,
  updateQnAMakerConfig,
} from '../../../utils/qnamakerbase';
...
export default class QnamakerKbCreate extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description:
        'File path to the CreateKbDTO object to send in the body of the request.',
    }),
    name: flags.string({
      description:
        'Name of the kb you want to create. This will override the name of KB that might be present in the CreateKb DTO',
    }),
    ...
    help: flags.help({
      char: 'h',
      description: 'qnamaker:kb:create command help',
    }),
  };
  ...
  async run() {
    ...
    if (!input.requestBody.name) {
      let answer = readlineSync.question(
        'What would you like to name your new knowledgebase?'
      );
