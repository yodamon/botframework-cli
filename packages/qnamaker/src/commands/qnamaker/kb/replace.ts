...
export default class QnamakerKbReplace extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description:
        'File path to the ReplaceKbDTO object to send in the body of the request. Alternately this can be path to a .qna file',
    }),
    kbId: flags.string({
      description:
        'Knowledgebase id. Overrides the knowledge base id present in the config',
    }),
    subscriptionKey: flags.string({
      description:
        'Specifies the qnamaker Ocp-Apim-Subscription Key (found in Keys under Resource Management section for your Qna Maker cognitive service). Overrides the subscriptionkey value present in the config',
    }),
    endpoint: flags.string({
      description:
        'Overrides public endpoint https://westus.api.cognitive.microsoft.com/qnamaker/v4.0/',
    }),
    help: flags.help({
      char: 'h',
      description: 'qnamaker:kb:replace command help',
    }),
  };
  ...
  async run() {
    ...
    let input: Inputs = await processInputs(
      flags,
      replaceKbJSON,
    ...
    if (input.requestBody.qnaDocuments && !input.requestBody.qnaList) {
      ...
      const qnaFiles = await fileHelper.getLuObjects(
        stdin,
        flags.in,
        false,
        fileExtEnum.QnAFile
      ...
      const qnamaker = await qnaMakerBuilder.fromContent(
        result.parseToQnAContent()
      );
      input.requestBody = qnamaker
        ? JSON.parse(JSON.stringify(qnamaker.kb))
        : undefined;
      flags.qnaFormat = false;
    }

    const result = await qnamaker(
      input.config,
