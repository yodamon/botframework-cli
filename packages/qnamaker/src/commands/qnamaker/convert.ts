...
export default class QnamakerConvert extends Command {
  ...
  static flags: flags.Input<any> = {
    in: flags.string({
      char: 'i',
      description: 'Source .qna file(s) or QnA KB JSON file',
    }),
    alterations: flags.boolean({
      description: 'Indicates if files is QnA Alterations',
    }),
    log: flags.boolean({ description: 'Enables log messages', default: false }),
    sort: flags.boolean({
      description:
        'When set, questions collections are alphabetically sorted are alphabetically sorted in .qna files',
      default: false,
    }),
    recurse: flags.boolean({
      char: 'r',
      description:
        'Indicates if sub-folders need to be considered to file .qna file(s)',
    }),
    out: flags.string({
      char: 'o',
      description:
  ...
  async run() {
    try {
      ...
      let result: any;
      if (isQnA) {
        const luFiles = await file.getLuObjects(
          stdin,
          flags.in,
          flags.recurse,
          fileExtEnum.QnAFile
        );
        result = await QnAMakerBuilder.build(
          [...luFiles],
          false,
          flags.luis_culture
        );
      } else {
        const qnaContent = flags.in
          ? await file.getContentFromFile(flags.in)
          : stdin;
        let QnA;
        let sortFucntion;
  ...
  private async writeOutput(convertedObject: any, flags: any, isQnA: boolean) {
    const filePath = await file.generateNewFilePath(
      flags.out,
      flags.in,
      isQnA,
      '',
    ...
    try {
      if (isQnA) {
        await fs.writeFile(
          validatedPath,
          JSON.stringify(convertedObject.kb, null, 2),
          'utf-8'
        );
        if (convertedObject.alterations) {
          const filePathAlterations = await file.generateNewFilePath(
            flags.out,
            flags.in,
            isQnA,
            'alterations_',
          ...
          const validatedPathAlter = utils.validatePath(
            filePathAlterations,
            '',
            flags.force
          );
          await fs.writeFile(
            validatedPathAlter,
            JSON.stringify(convertedObject.alterations, null, 2),
            'utf-8'
          );
        }
      } else {
      ...
      throw new CLIError(
        'Unable to write file - ' + validatedPath + ' Error: ' + error.message
      );
    }
