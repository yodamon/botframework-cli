...
export default class QnamakerBuild extends Command {
  ...
  static flags: any = {
    ...
    subscriptionKey: flags.string({
      char: 's',
      description: 'QnA maker subscription key',
    }),
    botName: flags.string({ char: 'b', description: 'Bot name' }),
    region: flags.string({
      description:
        'Overrides public endpoint https://<region>.api.cognitive.microsoft.com/qnamaker/v4.0/',
      default: 'westus',
    }),
    out: flags.string({
      char: 'o',
      description:
        'Output folder name to write out .dialog and settings files. If not specified, knowledge base setting will be output to console',
    }),
    defaultCulture: flags.string({
      description:
        'Culture code for the content. Infer from .qna if available. Defaults to en-us if not set',
    }),
    ...
    suffix: flags.string({
      description:
        'Environment name as a suffix identifier to include in qnamaker kb name. Defaults to current logged in user alias',
    }),
    dialog: flags.string({
    ...
    qnaConfig: flags.string({
      description:
        'Path to config for qna build which can contain switches for arguments',
    }),
    log: flags.boolean({
    ...
    endpoint: flags.string({
      description: 'Qnamaker authoring endpoint for publishing',
    }),
    schema: flags.string({
  ...
  async run() {
    try {
      ...
      if ((inVal && inVal !== '') || files.length > 0) {
        ...
        if (inVal && inVal !== '') {
          const qnaFiles: string[] = await file.getLuFiles(
            inVal,
            true,
            fileExtEnum.QnAFile
          );
        ...
        multiRecognizers.set(
          ...
          new MultiLanguageRecognizer(
            path.join(process.cwd(), `stdin.qna.dialog`),
            {}
          )
        );
      ...
      // get endpointKeys
      const endpointKeysInfo = await builder.getEndpointKeys(
        subscriptionKey,
        endpoint
      );
      ...
      if (out) {
        ...
        this.log(
          JSON.stringify(
            JSON.parse(dialogContents[dialogContents.length - 1].content).qna,
            null,
            4
          )
        );
