...
export default class QnamakerQuery extends Command {
  ...
  static flags: flags.Input<any> = {
    question: flags.string({
      description: 'Query to get a prediction for',
      required: true,
    }),
    top: flags.integer({
      description: 'Specifies the number of matching results',
    }),
    test: flags.boolean({
      description: 'Query against the test index',
      default: false,
    }),
    scorethreshold: flags.integer({
      description:
        'Specifies the confidence score threshold for the returned answer.',
    }),
    strictfilters: flags.string({
      description:
        'Path to json file with MetadataDTO[] e.g {"strictfilters": MetadataDTO[]}',
    }),
    qnaId: flags.integer({
      description:
    ...
    context: flags.string({
      description: 'Path to Context object json file with previous QnA',
    }),
    hostname: flags.string({
      description:
        'Specifies the url for your private QnA service. Overrides the value present in config',
    }),
    endpointKey: flags.string({
      description:
        'Specifies the endpoint key for your private QnA service (From qnamaker.ai portal user settings page). Overrides the value present in config',
    }),
    kbId: flags.string({
      description:
        'Specifies the active qnamaker knowledgebase id. Overrides the value present in the config',
    }),
