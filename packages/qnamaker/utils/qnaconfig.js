...
const config = {
  ...
  composeConfig: async function (args, configfile) {
    ...
    try {
      if (fs.existsSync(path.join(configfile, 'config.json'))) {
        qnamakerrcJson = await fs.readJSON(
          path.join(configfile, 'config.json')
        );
      }
    } catch (e) {
      // Do nothing
      ...
      config.subscriptionKey =
        subscriptionKey ||
        qnamakerrcJson.qnamaker__subscriptionKey ||
        QNAMAKER_SUBSCRIPTION_KEY;
      config.hostname =
        hostname || qnamakerrcJson.qnamaker__hostname || QNAMAKER_HOSTNAME;
      config.endpointKey =
        endpointKey ||
        qnamakerrcJson.qnamaker__endpointKey ||
        QNAMAKER_ENDPOINTKEY;
  ...
  waitForOperationSucceeded: async function (config, result) {
    while (true) {
      ...
      if (result.operationState === 'Succeeded') break;
      await Delay(1000);
    }
    cli.action.stop();
    return result;
