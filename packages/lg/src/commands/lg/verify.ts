...
export default class VerifyCommand extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    recurse: flags.boolean({
      char: 'r',
      description: 'Considere sub-folders to find .lg file(s)',
    }),
    out: flags.string({
      char: 'o',
      description:
  ...
  private fileDiagnostics(
    ...
    const outputContent = diagnostics
      .map((u) => u.source + ':' + u.toString())
      .join('\n');
    Helper.writeContentIntoFile(outputPath, outputContent, force);
    this.log(
      `Diagnostic messages of ${filePath} have been written into file ${outputPath}`
    );
  }
  ...
  private getOutputFile(
    ...
    const diagnosticName =
      path.basename(filePath).replace('.lg', '') + '.diagnostic.txt';
