...
export default class TranslateCommand extends Command {
  ...
  static flags: flags.Input<any> = {
    ...
    tgtlang: flags.string({
      description: 'Comma separated list of target languages.',
      required: true,
    }),
    translatekey: flags.string({
      description: 'Machine translation endpoint key.',
      required: true,
    }),
    ...
    srclang: flags.string({
      description: 'Source lang code. Auto detect if missing.',
    }),
    translate_comments: flags.boolean({
      description: 'Machine translate all comments found in .lg file',
    }),
    translate_link_text: flags.boolean({
      description: 'Machine translate link description in .lg file',
    }),
  ...
  private async translateLGFile(filePath: string, flags: any) {
    ...
    const translateParts = new TranslateParts(
      Boolean(flags.translate_comments),
      Boolean(flags.translate_link_text)
    );
    ...
    for (const toLang of toLangs) {
      ...
      if (tgt_lang !== '') {
        const translateOption = new TranslateOption(
          flags.translatekey,
          toLang,
          src_lang,
          flags.region
        );
        const result = await this.translateLGFileToSpecificLang(
          filePath,
          translateOption,
          translateParts
        );
        const outputFilePath = this.getOutputFile(
          filePath,
          tgt_lang,
          flags.out
        );
        if (!outputFilePath) {
          ...
          this.log(
            `Translate result of file ${filePath} to ${tgt_lang} has been written to ${outputFilePath}`
          );
        }
      }
    }
  ...
  private getOutputFile(
    ...
    // // a.lg -> a.en-us.lg
    const newFileName =
      path.basename(filePath).replace('.lg', '') + '.' + language + '.lg';
  ...
  private async translateLGFileToSpecificLang(
    ...
    const parsedLocContent = await this.parseAndTranslate(
      fileContent,
      translateOption,
      translateParts
    );
  ...
  private async parseAndTranslate(
    ...
    for (const currentLine of linesInFile) {
      ...
      if (inStructure) {
        if (currentLine.trim() === PARSERCONSTS.RIGHT_SQUARE_BRACKET) {
          ...
          if (equalIndex >= 0) {
            this.addSegment(
              linesToTranslate,
              currentLine.substr(0, equalIndex + 1),
              false
            );
        ...
        currentLine.trim().startsWith(PARSERCONSTS.DASH) &&
        currentLine
          .trim()
          .substr(PARSERCONSTS.DASH.length)
          .trim()
          .startsWith(PARSERCONSTS.MULTILINE)
      ) {
        // meet - ``` abc, start multi line
        this.addSegment(linesToTranslate, currentLine, false);
        inMultiLine = true;
      } else if (
        currentLine.trim().startsWith(PARSERCONSTS.LEFT_SQUARE_BRACKET)
      ) {
        // enter structure
        this.addSegment(linesToTranslate, currentLine, false);
        inStructure = true;
        ...
        if (translateParts.comments) {
          ...
          this.addSegment(
            linesToTranslate,
            currentLine.substr(0, commentIndex + 1) + ' ',
            false
          );
          this.addSegment(
            linesToTranslate,
            currentLine.trim().substr(commentIndex + 1),
            true
          );
        } else {
          this.addSegment(linesToTranslate, currentLine, false);
        ...
        this.addSegment(
          linesToTranslate,
          currentLine.substr(0, dashIndex + 1) + ' ',
          false
        );
      ...
      if (
        ...
        lineCtr % this.MAX_TRANSLATE_BATCH_SIZE === 0
      ) {
  ...
  private addSegmentWithExpression(
