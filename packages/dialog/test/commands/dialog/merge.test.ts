...
async function merge(
  ...
  schemaPath?: string,
  checkOnly?: boolean
): Promise<[merger.Imports | undefined, string[]]> {
  let lines: string[] = [];
  let logger = (msg) => {
    console.log(msg);
    lines.push(msg);
  ...
  let mergeClass = new merger.SchemaMerger(
    patterns,
    outputDir,
    undefined,
...
async function compareToOracle(
  ...
  oraclePath = oraclePath
    ? ppath.join(tempDir, oraclePath)
    : ppath.join('oracles', name);
  ...
  if (oracles !== generateds) {
    ...
    assert(
      false,
      `${ppath.resolve(generatedPath)} does not match oracle ${ppath.resolve(
        oraclePath
      )}`
    );
  }
...
describe('dialog:merge', async () => {
  ...
  it('app.schema', async () => {
    ...
    assert(
      countMatches(/error|warning/i, lines) === 0,
      'Error merging schemas'
    );
  ...
  it('bad json', async () => {
    console.log('\nStart bad json');
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/badJson.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
      'Wrong number of errors or warnings'
    );
    assert(
      countMatches('Unexpected token', lines) === 1,
      'Did not detect bad JSON'
    );
  });
  ...
  it('schema mismatch', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/schemaMismatch.schema',
    ]);
    ...
    assert(
      countMatches('does not match', lines) === 1,
      'Did not detect schema mismatch'
    );
  });
  ...
  it('no allof', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/allof.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
    ...
    assert(
      countMatches('allOf', lines) === 1,
      'Did not detect allOf in schema'
    );
  });
  ...
  it('missing extends', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/missingExtends.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
    ...
    assert(
      countMatches('it is not included', lines) === 1,
      'Did not detect missing extends in schema'
    );
  });
  ...
  it('missing schema reference', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/missingSchemaRef.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
    ...
    assert(
      countMatches('does not exist', lines) === 1,
      'Did not detect missing schema ref'
    );
  });
  ...
  it('bad role', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/badRole.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 2,
      'Wrong number of errors or warnings'
    );
    assert(
      countMatches('is not valid for component', lines) === 1,
      'Did not detect bad component $role'
    );
    assert(
      countMatches('is not valid in properties/foo', lines) === 1,
      'Did not detect bad property $role'
    );
  });
  ...
  it('duplicate $kind', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/prompt.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
    ...
    assert(
      countMatches('prompt.schema', lines) === 3,
      'Did not detect duplicate $kind'
    );
  });
  ...
  it('missing implementation', async () => {
    ...
    let [merged, lines] = await merge([
      'schemas/*.schema',
      'schemas/badSchemas/missingImplementation.schema',
    ]);
    assert(!merged, 'Merging should have failed');
    assert(
      countMatches(/error|warning/i, lines) === 1,
    ...
    assert(
      countMatches('no implementations', lines) === 1,
      'Did not detect missing implementations'
    );
  });
  ...
  it('csproj', async () => {
    console.log('\nStart csproj');
    let [merged, lines] = await merge(
      ['projects/project3/project3.csproj'],
      'project3.schema',
      true
    );
    assert(
      countMatches(/error|warning/i, lines) === 0,
      'Should not have got errors'
    );
    assert(merged, 'Could not merge');
    assert(
      countMatches(/Following.*project3/, lines) === 1,
      'Did not follow project3'
    );
    assert(
      countMatches(/Following nuget.*nuget3.*1.0.0/, lines) === 1,
      'Did not follow nuget3'
    );
    assert(
      countMatches(/Parsing.*nuget3.component1.schema/, lines) === 1,
      'Missing nuget3.component1.schema'
    );
    assert(countMatches(/Copying/i, lines) === 2, 'Wrong number of copies');
    assert(countMatches(/Copy /i, lines) === 6, 'Did not copy resources');
    assert(
      await fs.pathExists(
        ppath.join(tempDir, 'ImportedAssets', 'nuget3', 'stuff', 'nuget3.qna')
      ),
      'Did not copy directory'
    );
  ...
  it('csproj-errors', async () => {
    ...
    assert(
      countMatches(/error|warning/i, lines) === 3,
      'Wrong number of errors or warnings'
    );
    assert(
      countMatches(/Following.*project1/, lines) === 1,
      'Did not follow project1'
    );
    assert(
      countMatches(/Following nuget.*nuget1.*10.0.1/, lines) === 1,
      'Did not follow nuget1'
    );
    assert(
      countMatches(/Following.*project2/, lines) === 1,
      'Did not follow project2'
    );
    assert(
      countMatches(/Following nuget.*nuget2.*1.0.1/, lines) === 1,
      'Did not follow nuget2'
    );
    assert(
      countMatches(/Following nuget.*nuget3.*1.0.0/, lines) === 1,
      'Did not follow nuget3'
    ...
    assert(
      countMatches(/Parsing.*nuget1-10.schema/, lines) === 1,
      'Missing project1.schema'
    );
    assert(
      countMatches(/Parsing.*nuget2.schema/, lines) === 1,
      'Missing nuget2.schema'
    );
    assert(
      countMatches(/Parsing.*nuget3.component1.schema/, lines) === 1,
      'Missing nuget3.component1.schema'
    ...
    assert(
      countMatches(/Parsing.*project2.schema/, lines) === 1,
      'Missing project2.schema'
    );
    assert(
      countMatches(/multiple.dialog/, lines) === 3,
      'Missing multiple definitions'
    );
  });
  ...
  it('csproj-uierrors', async () => {
    ...
    let [merged, lines] = await merge(
      ['projects/project4/project4.csproj'],
      'project4.schema',
      true
    );
    assert(!merged, 'Merging should fail');
    assert(
      countMatches(/error|warning/i, lines) === 14,
      'Wrong number of errors or warnings'
    );
    assert(
      countMatches(/nokind does not exist/i, lines) === 1,
      'Missing nokind'
    );
    assert(
      countMatches(/nonExistentProperty/i, lines) === 8,
      'Wrong number of non-existent properties'
    );
    assert(
      countMatches(/order.nonExistentOrder/i, lines) === 4,
      'Wrong number of non-existent orders'
    );
    assert(
      countMatches(/missing \$schema/i, lines) === 1,
      'Did not find missing schema'
    );
  });
  ...
  it('csproj-config', async () => {
    console.log('\nStart csproj-config');
    let [merged, lines] = await merge(
      ['projects/project5/project5.csproj'],
      'project5.schema',
      true
    );
    ...
    assert(
      countMatches(/packages.config/i, lines) === 1,
  ...
  it('csproj-schema', async () => {
    ...
    assert(merged, 'Could not merge');
    let [merged2, lines2] = await merge(
      ['projects/project3/project3.csproj'],
      'project3-schema.schema',
      true,
      'project3.schema'
    );
    assert(
      countMatches(/error|warning/i, lines2) === 0,
      'Should not have got errors'
    );
    assert(
      countMatches(/using merged schema/i, lines2) === 1,
      'Should use merged schema'
    );
    assert(merged2, 'Could not merge');
    await compareToOracle(
      'project3-schema.en-us.uischema',
      'project3.en-us.uischema'
    );
  });
  ...
  it('csproj-import', async () => {
    ...
    let deletedPath = ppath.join(
      tempDir,
      'ImportedAssets/nuget3/stuff/nuget3-deleted.dialog'
    );
    await modifyFile(project, /1.0.0/, '1.0.1');
    await modifyFile(luPath, /intent/, 'intent modified');
    await modifyFile(
      ppath.join(tempDir, 'ImportedAssets/nuget3/nuget3.lg'),
      /template/,
      'template modified'
    );
    await modifyFile(
    ...
    await modifyFile(
      ppath.join(tempDir, 'ImportedAssets/nuget3/stuff/nuget3.dialog'),
      /dialog/,
      'dialog modified'
    );
    await modifyFile(jpgPath, /picture/, 'picture modified');
    let [merged3, lines3] = await merge(
      [project],
      'project3.schema',
      true,
      undefined,
    ...
    assert(
      countMatches(/warning/i, lines3) === 3,
      'Wrong number of warnings 3rd'
    );
    ...
    assert(
      countMatches('modified', lines3) === 0,
      'Missed deletion change 3rd'
    );
    assert(countMatches('conflicting', lines3) === 3, 'Missed conflicts 3rd');
    assert(
      (await fs.readFile(luPath, 'utf8')).includes('modified'),
      'Wrote file in check-only'
    );
    assert(
      (await fs.readFile(jpgPath, 'utf8')).includes('modified'),
      'Wrote file in check-only'
    );
    ...
    let [merged4, lines4] = await merge(
      [project],
      'project3.schema',
      true,
      undefined,
    ...
    assert(
      countMatches(/warning/i, lines4) === 4,
      'Wrong number of warnings 4th'
    );
    ...
    assert(
      countMatches('modified', lines4) === 1,
      'Missed deletion change 4th'
    );
    ...
    assert(
      (await fs.readFile(luPath, 'utf8')).includes('changed'),
      'Did not write file'
    );
    assert(
      (await fs.readFile(jpgPath, 'utf8')).includes('changed'),
      'Wrote file in check-only'
    );
  ...
  it('package.json', async () => {
    ...
    assert(
      countMatches('root-package.schema', lines) === 1,
      'Missing root-package.schema'
    );
    assert(
      countMatches('dependent-package.schema', lines) === 1,
      'Missing dependent-package.schema'
    );
    assert(
      countMatches('parent-package.schema', lines) === 1,
      'Missing parent-package.schema'
    );
    assert(
      countMatches('no-package.schema', lines) === 0,
      'Extra no-package.schema'
    );
    ...
    assert(
      await fs.pathExists(
        ppath.join(
          ...
          'dependent-package',
          'assets',
          'dependent-package.jpg'
        )
      ),
      'Incomplete assets copy'
    );
    assert(
      !(await fs.pathExists(
        ppath.join(tempDir, 'ImportedAssets', 'root-package')
      )),
      'Copied rootx'
    );
  ...
  it('nuspec', async () => {
    ...
    await fs.copyFile(
      'schemas/packageBase.json',
      ppath.join(path, 'packageBase.json')
    );
    await fs.copyFile(
      ppath.join(path, 'nuget1-10.schema.local'),
      ppath.join(path, 'nuget1-10.schema')
    );
    let cwd = process.cwd();
    try {
      process.chdir(path);
      ...
      assert(
        countMatches('nuget1.nuspec', lines) === 1,
        'Missing nuget1.nuspec'
      );
    } finally {
      process.chdir(cwd);
    }
