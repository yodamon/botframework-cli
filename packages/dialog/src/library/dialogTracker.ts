...
export class DialogTracker {
  ...
  async addDialog(dialog: Dialog): Promise<void> {
    try {
      ...
      if (
        dialog.schema() &&
        this.schemaPath &&
        dialog.schema() !== this.schemaPath
      ) {
        dialog.errors.push(
          new Error(
            `${dialog.schema()} does not match default schema ${
              this.schemaPath
            }`
          )
        );
    ...
    for (let dialog of this.dialogs) {
      if (dialog.save) {
        let filePath = root
          ? path.join(
              path.resolve(root),
              path.relative(process.cwd(), dialog.file)
            )
          : dialog.file;
  ...
  *typeMismatches(): Iterable<Definition> {
  ...
  private addDefinition(definition: Definition) {
    ...
    if (definition.id) {
        ...
        for (let old of this.idToDef.get(definition.id) as Definition[]) {
          if (
            ...
            (!old.kind ||
              ...
              definition.kind?.interfaces.includes(old.kind))
          ) {
            add = false;
      ...
      if (add) {
        ...
        if (definition.kind) {
          (this.typeToDef.get(definition.kind.name) as Definition[]).push(
            definition
          );
        }
      }
  ...
  private findDefinitionsAndReferences(
    elt: any,
    path: string,
    dialog: Dialog,
    ...
    if (Array.isArray(elt)) {
      let i = 0;
      for (let val of elt) {
        this.findDefinitionsAndReferences(
          val,
          `${path}[${i}]`,
          dialog,
          definition
