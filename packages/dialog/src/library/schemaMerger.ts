...
function walkJSON(
  elt: any,
  fun: (val: any, obj?: any, path?: string) => boolean,
  obj?: any,
  path?: any
) {
...
class Component {
  ...
  public isRoot(): boolean {
    return (
      ...
      (this.parents.length === 1 && this.parents[0].parents.length === 0)
    );
  }
...
export class SchemaMerger {
  ...
  // Files
  private readonly files: Map<string, Map<string, PathComponent[]>> = new Map<
    string,
  ...
  public constructor(
    ...
    this.output = output
      ? ppath.join(
          ppath.dirname(output),
          ppath.basename(output, ppath.extname(output))
        )
      : '';
  ...
  private async mergeSchemas(): Promise<any> {
      ...
      try {
        ...
        if (component.definitions && component.definitions.component) {
          ...
          if (!component.$schema) {
            ...
            this.parsingWarning(
              `Component schema ${component.$schema} does not match ${this.metaSchemaId}`
            );
          } else {
            this.validateSchema(component);
          }
          ...
          if (component.allOf) {
            this.parsingError(
              'Does not support allOf in component .schema definitions'
            );
          }
    ...
    if (!this.failed) {
      ...
      let finalSchema: any = {
        ...
        description:
          'These are all of the kinds that can be created by the loader.',
        oneOf,
        definitions: finalDefinitions,
      };
      if (this.debug) {
        await fs.writeJSON(
          this.currentFile + '.final',
          finalSchema,
          this.jsonOptions
        );
      }
    ...
    if (uiSchemas) {
        ...
        if (!fileName.startsWith(outputName + '.')) {
          ...
          for (let componentPath of componentPaths.reverse()) {
            try {
              ...
              if (!component.$schema) {
                ...
                this.parsingWarning(
                  `UI schema ${component.$schema} does not match ${this.metaUISchemaId}`
                );
              } else {
                this.validateUISchema(component);
              }
          ...
          if (!kindDef) {
            ...
            this.validateProperties(
              kindName,
              kindDef,
              locale[kindName].form?.properties
            );
            this.validateProperties(
              `${kindName}.order`,
              kindDef,
              locale[kindName].form?.order
            );
          }
        }
      }
      if (!this.failed) {
        for (let locale of Object.keys(result)) {
          ...
          this.currentFile = ppath.join(
            ppath.dirname(this.output),
            outputName + (locale ? '.' + locale : '') + '.uischema'
          );
          if (!this.checkOnly) {
            this.log(`Writing ${this.currentFile}`);
  ...
  private async copyAssets(): Promise<Imports | undefined> {
    let imports: Imports | undefined = this.failed
      ? undefined
    ...
    if (imports && !this.schemaPath && this.components.length > 0) {
      ...
      for (let component of this.components) {
        if (!component.isRoot()) {
          let exported = ppath.join(
            ppath.dirname(component.path),
            'ExportedAssets'
          );
          if (await fs.pathExists(exported)) {
            let used = new Set<string>();
            let imported = ppath.join(
              this.imports,
              'ImportedAssets',
              component.name
            );
            ...
            for (let path of await glob(
              forwardSlashes(ppath.join(exported, '**'))
            )) {
              let destination = ppath.join(
                imported,
                ppath.relative(exported, path)
              );
              ...
              try {
                let copy = true;
                let info: Import = {
                  definition: await hash.addHash(path),
                  path: destination,
                };
                let { unchanged, embeddedHash } = await hash.isUnchanged(
                  destination
                );
                if (hash.embeddedHash(path, info.definition) === embeddedHash) {
                  ...
                  this.warn(
                    `Warning copied conflicting ${path} to ${destination}`
                  );
            ...
            for (let path of await glob(
              forwardSlashes(ppath.join(imported, '**'))
            )) {
              if (!used.has(path)) {
    ...
    if (typeof schema === 'object') {
      if (schema.type === 'object') {
        ...
        return typeof schema.items === 'object'
          ? this.propertyDefinition(schema.items, property)
          : undefined;
  ...
  private toAbsoluteRef(ref: string, base: string): string {
    if (ref.startsWith('.')) {
      ref = forwardSlashes(
        `file:///${ppath.resolve(ppath.dirname(base), ref)}`
      );
    }
    return ref;
  }
    ...
    for (let [ext, files] of this.files.entries()) {
      for (let [file, records] of files.entries()) {
        ...
        if (same.length > 0) {
          this.failed = true;
          this.error(
            `Error multiple definitions of ${file} in ${winner.component.name}`
          );
  ...
  private async findParentDirectory(
    path: string,
    dir: string
  ): Promise<string> {
    path = ppath.resolve(path);
  ...
  private mergeInto(
    ...
    let extension =
      this.definitions[extensionName] ||
      this.metaSchema.definitions[extensionName];

    if (!extension) {
      this.mergingError(
        `Cannot extend ${extensionName} because it is not included`
      );
    } else {
        ...
        if (!definition.properties) {
          ...
          for (let property in extension.properties) {
            if (definition.properties[property]) {
              this.mergingError(
                `Redefines property ${property} from ${extensionName}`
              );
            } else {
              definition.properties[property] = clone(
      ...
      if (extension.hasOwnProperty('additionalProperties')) {
        if (definition.hasOwnProperty('additionalProperties')) {
          if (!canOverride) {
            this.mergingError(
              `Redefines additionalProperties from ${extensionName}`
            );
          }
        } else {
          ...
          if (val.$role) {
            ...
            if (!path) {
              if (expression.length > 0) {
                this.mergingError(
                  `$role ${val.$role} is not valid for component`
                );
              }
            } else {
              if (
  ...
  private sortImplementations(): void {
    for (this.currentKind in this.definitions) {
      ...
      if (this.isInterface(definition) && definition.oneOf) {
        definition.oneOf = definition.oneOf.sort((a: any, b: any) =>
  ...
  private splitRef(
    ref: string
  ): { path: string; pointer: string; name: string } {
          ...
          if (val.$kind) {
            ...
            if (this.roles(kind, 'interface').length > 0) {
              ...
              if (!hasImplementation) {
                this.mergingError(
                  `${path} has no implementations of ${val.$kind}`
                );
              }
            }
          }
      ...
      walkJSON(definition, (val, _, path) => {
        ...
        if (typeof val.additionalProperties === 'object') {
          verifyProperty(
            ...
            pathName(path, 'additionalProperties')
          );
        }
        if (val.patternProperties) {
          for (let pattern in val.patternProperties) {
            verifyProperty(
              val.patternProperties[pattern],
              pathName(path, `patternProperties/${pattern}`)
            );
          }
        }
        return false;
  ...
  private parsingError(err: Error | string): void {
    ...
    if (posMatch) {
      ...
      msg +=
        os.EOL +
        file.substring(pos - 40, pos) +
        '^^^' +
        file.substring(pos, pos + 40);
    } else {
