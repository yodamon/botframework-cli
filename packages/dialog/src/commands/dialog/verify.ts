...
export default class DialogVerify extends Command {
  ...
  static args = [
    {
      name: 'patterns',
      required: true,
      description: 'Any number of glob regex patterns to match .dialog files.',
    },
  ];
  ...
  static flags: flags.Input<any> = {
    ...
    schema: flags.string({
      char: 's',
      description: 'Default schema to use if no $schema in dialog file.',
    }),
  ...
  async execute(
    ...
    if (tracker.dialogs.length === 0) {
      ...
      for (let defs of tracker.multipleDefinitions()) {
        ...
        this.consoleError(
          `Multiple definitions for ${def} ${def.usedByString()}`,
          'DLG002'
        );
        for (let def of defs) {
      ...
      for (let def of tracker.missingDefinitions()) {
        this.consoleError(
          `Missing definition for ${def} ${def.usedByString()}`,
          'DLG003'
        );
      }
      ...
      for (let def of tracker.typeMismatches()) {
        for (let use of def.typeMismatches()) {
          this.consoleError(
            `Type mismatch ${def} does not match ${use}`,
            'DLG004'
          );
        }
      }
  ...
  consoleWarn(msg: string, code: string): void {
    this.warnings++;
    this.warn(
      `${this.currentFile ? `${this.currentFile} - ` : ''}Warning ${
        code || ''
      }: ${msg}`
    );
  }
  ...
  consoleError(msg: string, code: string): void {
    this.errors++;
    this.error(
      `${this.currentFile ? `${this.currentFile} - ` : ''}Error ${
        code || ''
      }: ${msg}`
    );
  }
